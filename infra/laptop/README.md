...
## DB 정기 백업 (2026-08-21 신설, #489)

이사 직후 랩탑엔 **DB 백업이 하나도 없었다** — `crontab` 0건, systemd timer 는 `hmb-tunnel-heal`
뿐이었다. 맥의 `~/.local/state/hmb/db-backups/`(8.9GB · 66개)는 정기 잡이 아니라 **배포 전 수동**
백업이 쌓인 것이라(`pre-v496-…`) **이식할 잡 자체가 없었다.** 그래서 새로 만들었다.

```bash
bash infra/laptop/install-db-backup.sh            # 설치(이미지 굽기 + 유닛 + 타이머)
bash infra/laptop/install-db-backup.sh --run      # 지금 1회 (서비스 경로로)
/opt/hmb/bin/db-backup.sh --status                # 세대·용량·최근 결과
```

| 항목 | 값 |
|---|---|
| 유닛 | `hmb-db-backup.service` + `.timer` (시스템 유닛 — `hmb-tunnel-heal` 과 같은 관용구) |
| 주기 | 매일 `19:00 UTC` = **04:00 KST** · `Persistent=true`(꺼져 있었으면 기동 시 따라잡음) |
| 보관 | `/var/backups/hmb/hmb-<UTC스탬프>.db` · **7세대** (`HMB_BACKUP_KEEP`) |
| 1회 크기·시간 | **727MB · 10~25초** (7세대 ≈ 5.1GB, 디스크 여유 227G) |
| 방식 | sqlite **온라인 백업 API**(`.backup`) — 스택을 안 세워도 정합. `cp` 는 WAL 때문에 조용히 어긋난다 |
| 이미지 | `hmb/sqlite3:local` (alpine + sqlite 를 **미리 구움**) |

### ⚠️ 왜 이미지를 미리 굽나

런북 P2-12 는 매번 `apk add --no-cache sqlite` 를 한다 = **백업이 네트워크에 의존**한다.
백업은 네트워크가 죽은 날에도 돌아야 한다.

### ⚠️ 설치 당일 실패경로 검증에서 **실제로 결함이 나왔다** — `integrity_check = ok` 는 백업이 됐다는 뜻이 아니다

첫 판은 P2-12 의 두 함정(ro 마운트 · 옛 파일에 검증 통과)만 막고 있었다. 그런데 실패경로를
찔러 보니 **세 번째 함정**이 있었다:

```
HMB_DB_FILE=nope.db ./db-backup.sh   →  exit 0 · 세대 1→2 · 4096 bytes · integrity=ok
```

`sqlite3` 는 **없는 파일을 열면 빈 DB 를 만든다.** 그래서 소스 경로가 틀려도 `.backup` 이
성공하고, 4096바이트 빈 DB 가 `ok` 를 받아 **정상 세대로 승격**된다. 무결성 검증은 파일이
**온전한가**만 보지 **무엇인가**는 보지 않는다 — P2-12 무음 실패와 정확히 같은 부류가
한 층 아래에서 재현된 것이다.

그래서 가드가 넷이다(전부 exit 1 · 세대 증가 0 · 인플라이트 잔재 0 으로 실측 확인):

| 가드 | 막는 것 |
|---|---|
| 소스 사전 확인 (`stat` on `/data/$DB`) | 없는 볼륨 · 없는 DB 파일 |
| `integrity_check = ok` | 손상·절단 |
| **테이블 수 ≥ `MIN_TABLES`(10)** | **빈 DB**(0 테이블). 라이브 실측 61 |
| **크기 ≥ 소스의 `MIN_RATIO_PCT`(50%)** | 절단된 백업 |

검증은 **반드시 인플라이트 이름에** 하고, 통과해야만 최종 이름으로 승격한다. 실패하면
인플라이트를 지우고 죽는다 — 그래서 **실패한 날 옛 세대를 잃지 않는다**(세대 정리도 성공 후에만).

### 한계 (다음 사람이 알아야 할 것)

**같은 기계 안의 백업이다.** DB 손상·잘못된 마이그레이션은 막지만 **랩탑 자체를 잃는 건 못 막는다.**
오프머신 사본이 필요하면 그건 별도 과제다.
