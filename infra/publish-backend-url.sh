...
#    더 나쁜 건 `exit "$rc"` = **`exit 0`** 이라, 이 스크립트를 직접 부르는 쪽 — 사람의 수동 실행
#    (status.sh 가 안내하는 경로)·e2e 하네스 — 에는 실패가 **성공으로** 나갔다는 것이다
#    (deploy-web.sh 는 이 스크립트를 부르지 않는다 — #514 재검증에서 호출자 전수 grep).
#    워치독만 무사했는데 그건 종료코드를 아예 안 읽고(`|| true`) 엣지를 직접 다시 조회하기
#    때문이다(publish_verified) — 즉 이 결함은 그 독립 검증이 **가려주고 있었다**.
rc=0
run_deploy || rc=$?
if [ "$rc" -ne 0 ]; then
