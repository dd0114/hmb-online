 ...
 * 파이프라인은 정확색 제거로 물러난다 — 배경이 남을지언정 캐릭터는 사실상 안 먹는다
 * (배경색과 정확히 같은 색이 캐릭터 안에 있는 경우는 예외 — SPEC §2 가 이를 금지한다).
 * 실패가 **눈에 보이는 쪽**(남은 배경)으로만 나게 하는 것이 설계 의도다.
 *
 ...
 * 몸통 내부에 구멍이 생긴다.
 * ⚠️ **부분 침식에는 눈이 멀다** — 캐릭터 일부가 연속적으로 지워져도 위상이 안 바뀌면
 * largestShare 는 높게 나온다(실측: 78% 소실에도 0.986). 최종 안전장치는 육안 확인이다.
