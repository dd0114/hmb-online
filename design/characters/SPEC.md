...
> ⚠️ **배경은 타협 항목이 아니다.** P2 파일럿에서 어두운 배경 위의 어두운 캐릭터(라그나)는 **어떤 설정으로도 분리되지 않았다** —
> 임계를 낮추면 배경이 남고 높이면 검은 머리·의상을 먹는다. 그 사이 안전값이 없다. 파이프라인은 이제 손상을 검출해 경고하지만, **고쳐주지는 못한다**.
> AI 생성기는 대부분 경계에 안티에일리어싱을 넣는데, 단색 배경 + AA 는 **캐릭터 외곽에 배경색 프린지**를 남긴다. 그래서 투명 배경이 최선이다.
