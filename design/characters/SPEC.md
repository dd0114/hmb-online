...
| 형태 | 근거 레퍼 | 규격 | 입력 |
|---|---|---|---|
| **아바타** | ref-1 | 64×64 / 32×32 / 16×16 + 팀 링(원형크롭, 지름 45px 기준 stroke 2px ≈ 4.5%) — blue `#3da9f1` / red `#ef4f44` | `__portrait` |
| **카드** | ref-2 | 226×425 (1:1.88), 금테 8px(카드폭 3.5%), 아트 상단 ~78% + 네임플레이트/별/설명판 | `__full` |
...
| 항목 | 규격 | 이유 |
|---|---|---|
| 포맷 | **PNG** 권장 (jpg/webp 도 받지만 파이프라인이 sips 로 PNG 변환) | 무손실·알파 |
| 배경 | **완전 투명(알파 0)** 최선. 불가 시 **단색 순수 배경**(#FF00FF 마젠타 또는 순백 #FFFFFF), 캐릭터에 그 색을 쓰지 말 것 | 자동 배경 제거 정확도 |
| 최소 해상도 | portrait **512×512 이상 정사각**, full **512×1024 이상 세로(1:2)** | 64px 다운스케일 시 8배 이상 확보 |
...
> **P2 파일럿 실측 결과(`PILOT-P2.md`, 2026-07-20)**: 자동 도트화는 **32px 이상 + 카드에서만 실용**, **≤16px 은 형태가 붕괴**한다.
> 파일럿 입력이 *이미 픽셀아트* 였는데도 그랬으므로 이건 **품질 상한**이다(일반 일러스트는 더 나쁘다).
> → **A안 권장이 실측으로 뒷받침됨.** 저해상 단계(16/8px)는 hero 가 직접 생성·손보정해 넣는 편이 낫다. hero 판정 대기 중(#104).
