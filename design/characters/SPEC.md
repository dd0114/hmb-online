...
> ⚠️ **왜 투명 배경인가.** 불투명 배경도 파이프라인이 자동 분리하지만, 안전 구간이 좁고 **소스마다 다르다**
> (P2 파일럿 자동 선택값: localTol 8~24 / globalTol 40~255 로 제각각). 낮으면 배경이 남고 조금만 넘으면
> 캐릭터의 어두운 머리·의상을 먹는다. 파이프라인이 매번 2축 탐색하지만, 투명 배경이면 이 위험이 아예 없다.
...
- `bgTol`(선택) = 배경 제거 임계 강제. 숫자면 양쪽 공통, `{"portrait":9,"full":8}` 로 개별 지정. 미지정 시 자동 탐색.
