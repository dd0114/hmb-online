...
> ⚠️ **왜 투명 배경이 필수인가.** 파이프라인은 불투명 배경을 **대신 해결해 주지 않는다.**
> 분리 임계값의 안전 구간이 소스마다 달라(P2 파일럿 육안 확정값: localTol 8~14 / globalTol 30~140 로 제각각)
> **자동 선택을 다섯 번 시도했고 전부 실패했다** — 지표마다 반례가 있다(PILOT-P2 §2).
> 그래서 불투명 배경으로 넣으면 **사람이 `bgTol` 값을 고르고 눈으로 확인해야 한다.**
> 지정하지 않으면 파이프라인은 보수적으로 처리해 **배경이 남는다**(캐릭터를 먹느니 배경을 남긴다).
...
```json
{
  ...
  "position": "FW",
  "stars": 5,
  "bgTol": { "portrait": { "localTol": 9, "globalTol": 40 }, "full": 8 },
  "signature": "#f7a051",
  "frame": "#c82813",
...
- `bgTol` = **불투명 배경일 때 사실상 필수**(투명 배경이면 불필요). 배경 제거 임계.
  형태: `8` · `{"localTol":9,"globalTol":40}` · `{"portrait":{...},"full":{...}}` (variant 별).
  고르는 법 = `out/<id>/report.json` 의 `tolDiagnostic` 스윕 표 참고 → 대조 시트로 육안 확인.
  **미지정 시 자동 선택하지 않는다** — 보수적 설정으로 처리해 배경이 남고 경고가 뜬다.
- `stars` = 별 개수(1~6). ref-2 라그나가 6성이라 6까지 받는다 — PRD 의 "5등급"과는 별개 축.
...
> **P2 파일럿 실측 결과(`PILOT-P2.md`, 2026-07-20)**: **32px 이상 + 카드는 자동 변환으로 충분**(3/3).
> **≤16px 은 명확한 자동 성공이 0/3** — 펭킹킹만 경계(△)이고 나머지는 무너진다.
> 전신 스프라이트는 ≤16px 에서 3/3 실패. 결정 변수는 해상도가 아니라 **실루엣 단순성과 색 대비**다.
...
## 4.5 hue 변형 규격 (LEGEND 부족분 — #121)

LEGEND 14명 > 캐릭터 12종이라 **부족분 2를 캐릭터 재사용 + hue 변형**으로 채운다(hero 확정, 매니저 규칙).

- **hue 변형 = 파이프라인에서 굽는다**(web CSS 아님). 근거: CSS `filter:hue-rotate` 는 선형근사라
  채도를 최대 32% 깎고, 팀 링·등급 프레임까지 물들이며, 피부톤 보호가 불가.
- 파이프라인 방식 = **진짜 HSL 회전**(`hueShift`): 채도·명도 보존 + **피부톤 대역 제외**(hue 18~48°/S 0.18~0.8/L 0.35~0.92).
- **카드는 아트에만 hue → 재합성**한다. 카드 완성본에 걸면 포지션 뱃지 색이 돌아 포지션 오인(FW→DF).
  프레임·시그니처는 변형 정체성이라 함께 시프트, 뱃지는 POS_COLOR 고정.
- 배정(결정론): 선수 id 정렬순 + 포지션 매칭. 슬롯 13=P143(MF)→`sail`+150°, 슬롯 14=P144(FW)→`ragna`+210°.
  각도·중복 캐릭터는 #121 이 최종 확정.
...
- [ ] PNG, 배경 투명(또는 캐릭터에 없는 단색 + AA 없는 하드 엣지)
...
## 변경 이력
- v0.1 (2026-07-20) 최초 — 레퍼 3장 + #103 실측 팔레트 기반.
- v0.2 (2026-07-21) **hero 확정**: 레퍼 시트의 12캐릭터를 **게임 자산으로 확정**(트레이싱 금지 대상 아님 — hero 생성물).
  → `dist/characters/` 로 발행(#121 LEGEND 매핑 소비). LEGEND 14명 부족분 2는 hue 변형(§8).
