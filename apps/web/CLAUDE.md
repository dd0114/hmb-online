...
- ⚠️ **D4 등급색 링(`gradeRingShadow`)은 계속 `GRADE_COLORS`(보라)다 — 일부러 그렇다.**
  LEGEND·GOLD 프레임은 육안 구분이 안 돼 **별 개수로만** 갈리는데(#187 D4, hero 위임 2026-07-26),
  카드 바깥 링이 그 유일한 즉시 식별자다. 여기까지 금색으로 "통일"하면 **레전드가 골드처럼 보인다.**
  "빛은 프레임을 따르고, 링은 등급을 말한다"가 두 색이 다른 이유다.
