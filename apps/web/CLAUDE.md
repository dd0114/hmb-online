...
- ⚠️ **도감은 이제 전신이다 — 정책이 뒤집혔다(#286 W3, hero 확정).**
  ...
  - ⚠️⚠️ **여기서 #285 와 정면으로 부딪힌다 — 아직 미해결이다(hero 게이트).**
    이 절은 원래 *"실효는 LEGEND·DIA 39명이고 나머지는 **아트 발행**이 필요하다"* 라고 적었는데
    **그건 틀렸다**(독립검증 W3 BL-1). 아트를 아무리 발행해도 위쪽 **#285 노출 정책**(다이아 미만은
    매핑이 있어도 화면에 그리지 않는다 — `icon-policy`)이 먼저 걸려 못 그린다. 문서가 "발행하면
    해결된다"는 잘못된 지도를 준 상태였다.
    - **실측**(도감 [전체], 미보유): DIA 이상 = `data-art-kind="full-art"` + 실루엣.
      GOLD 이하 = **`frame-only`, `<img>` 자체가 없다** → 필터를 걸 대상도 없다.
    - 그래서 hero 가 요구한 "전신으로 모으는 재미"는 **로스터의 26% 에만** 성립한다.
    - **미보유 잠금 표현만은 등급과 무관하게 통일**해 뒀다(자물쇠 오버레이 = 아트 유무 무관).
      같은 상태가 등급에 따라 두 그림으로 갈리던 것은 결함이라 그건 먼저 고쳤다.
    - **남은 결정은 hero 것**: ①#285 에 도감 예외를 둘지 ②AC 를 "DIA 이상만 전신"으로 좁힐지
      ③등급 임계를 내릴지. 코드로 조용히 봉합하지 말 것 — 두 hero 결정이 겹친 자리다.
