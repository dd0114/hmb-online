...
const S1 = 799; // save(= 유효슛 + 세이브 같은 틱)
const S2 = 1220; // goal
 ...
 * 없다** — 그런 계약은 시퀀서를 통째로 지워도 통과한다. 799 → 1220 은 자연 재생으로 **50초 이상**이라
...
test.describe("#421 W4 하이라이트 순서 재생", () => {
  ...
  test("d·e. 라이브 후반 — 상한 밖 장면으로 뛰지 않고, 따라잡으면 라이브를 이어 재생한다", async ({ page }) => {
    // 후반 10% 경과 = 상한 ≈ 144틱. 두 장면(799·1220)은 **아직 미래**다.
     ...
     * 리드인 착지(799−8−3스냅 = 788)가 그 아래라 **상한을 무시하는 변이가 그대로 통과한다**
  ...
  test("d2. 라이브 상한 **안**의 장면은 재생하고, 그다음 장면은 열릴 때까지 안 뛴다", async ({ page }) => {
    // 후반 60% 경과 = 상한 ≈ 864틱 → 799 는 열렸고 1220 은 아직이다.
    await openMatch(page, {
      state: "SECOND_HALF",
  ...
  test("g. 전반은 디폴트가 전체 재생이다(적용 범위 = 후반)", async ({ page }) => {
     ...
     * 70% 면 하이라이트 #1(799)이 상한 안이고 seek-to-now 는 그 뒤(≈1008)라, 시퀀서가 돌았다면
