...
test("f. 레벨업 행 → 후보 3장 → 선택 → 적용·축하 + 남은 대기 유도", async ({ page }) => {
   ...
   * 감쇠 설명. 그 다섯을 **양성으로** 단언하고 있던 것은 **`f` 와 `i` 둘뿐**이고(기준선 `3974ebe`
   * 실측: `f` 7건 · `i` 6건 · **`j`·`k` 0건**), 실제 diff 도 그 둘만 건드렸다. 그 단언들은 정책이
   * 바뀌어 **도달 불가**가 된 것이라 지우지 않고 *지금 참인 것*으로 다시 썼다(모듈 CLAUDE.md 표 #8).
   * ⚠️ 초판 주석은 여기 `f·i·j·k` 라고 적었다 — **거짓이다**(독립검증 FIN-1). `j`(순서)·`k`(core
   * 부재)는 이 축을 처음부터 안 봤고 지금도 **무접촉**이다.
   *
