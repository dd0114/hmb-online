...
export function GamePage() {
   ...
   * (앞의 두 문장은 실측 확인됨 — 첫 응답만 404 로 주면 그 방문 `offer_missed`·모달 0, 다음 진입에
   * 모달 1 + `offer_shown` 이고 `hmb.guide.practice.<uid>` 는 `null` 이다.)
   *
   * ⚠️ **그런데 이 가드에는 계약이 없다**(독립 검증 m-a): 지워도 죽는 테스트가 없다 — 방문 **중**
   * 리페치를 하네스로 만들지 못했기 때문이다(`refetchOnWindowFocus:false` · `retry:false`). 즉 위
   * 승격 억제는 *코드가 그렇게 생겼다*까지만 확인됐고 *그렇게 동작한다*는 아직 검정되지 않았다.
   * **지우기 전에 그 계약부터 만들어라**(목이 첫 응답 404 → 이후 200 + 명시적 무효화). 근거 없이
   * 지우면 한 방문에서 `offer_missed` 와 `offer_shown` 이 같이 나가고, 그게 D3 판단의 자를 흐린다.
   */
