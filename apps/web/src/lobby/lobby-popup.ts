/**
 * ⚠️ **현재 미사용**(#286 W2) — 삭제가 아니라 **화면에서 내린 것**이다(#106 선례와 같은 취급).
 *
 * 두 팝업의 트리거가 서로 다른 화면으로 갈라졌기 때문이다: 공지는 **홈**(진입 시), 원정
 * 리포트는 **게임 탭**([원정] 카드 클릭). 한 화면에서 겹칠 수가 없어졌으므로 우선순위를 매길
 * 자리가 없다. 다시 한 화면에 모이면 이 규칙(공지가 이긴다 · 온보딩 중에는 미룬다)을 그대로
 * 되살려 쓴다 — 테스트도 함께 남겨 둔다.
 */
/**
