 ...
 * [24시간 안 보기]를 누른 사이 노출 기간이 끝나면 **영영 못 본다**. 점검·보상 공지는
 ...
 *
 * 4. **억제는 팝업에만 적용된다** — 24시간 안 보기를 누른 공지도 목록엔 있다(`noticeCenterView`).
...
export function NoticeCenter({
   ...
   * 공지 팝업이 [닫기]/[24시간]으로 같은 저장소를 쓴다. 로컬 카운터로 자기 쓰기만 반영하면
  ...
  const toggle = useCallback(
    (key: string) => {
      ...
      // 펼친 순간 = 읽은 것. 팝업 [닫기]와 같은 범위(탭 세션)로 기록한다.
