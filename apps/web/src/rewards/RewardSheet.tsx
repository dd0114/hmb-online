...
import { presentSections } from "./registry";
...
export function RewardSheet({ bundle, matchId, subtitle, badge, badgeTone, onClose }: RewardSheetProps) {
  const sections = useMemo(() => presentSections(bundle), [bundle]);
    ...
    >
      ...
      {pick ? (
        ...
        <>
          ...
          <button type="button" className={styles.cta} data-testid="reward-confirm" onClick={confirm} disabled={ack.isPending}>
            확인
          </button>
        </>
      )}
