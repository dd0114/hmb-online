...
/**
 * POST /api/auth/register body — 서버 RegisterRequest.java 와 동일한 2필드.
 * 로그인 id 는 nickname 이 겸하므로 **여분 식별자를 만들지 않는다**(서버가 무시/거부).
 */
...
/**
 * POST /api/auth/login body — 서버 LoginRequest.java(nickname/provider/password).
 * guest·mock:* 경로와 **같은 엔드포인트·같은 바디 형태**고 provider 로만 갈린다.
 */
export function buildLocalLoginBody(input: {
  nickname: string;
  password: string;
}): LocalLoginRequest {
  return { nickname: input.nickname, provider: LOCAL_PROVIDER, password: input.password };
}

export const DUPLICATE_NICKNAME_MESSAGE = "이미 사용 중인 아이디입니다";
export const BAD_CREDENTIALS_MESSAGE = "아이디 또는 비밀번호가 올바르지 않습니다";
const GENERIC_AUTH_FAILURE_MESSAGE = "로그인에 실패했습니다";

/**
 * 서버 에러 → 화면 필드 에러 매핑(AC-A1). 코드는 AuthErrors.java 가 SoT:
 *   409 DUPLICATE_NICKNAME → nickname(= 아이디) 필드
 *   401 BAD_CREDENTIALS    → 폼 전역(어느 쪽이 틀렸는지 알려주지 않는다 — 계정 열거 방지)
 * 그 외(400 VALIDATION_ERROR 포함)는 폼 전역 일반 메시지. 서버 원문 message 는 신뢰하지 않고
 * 고정 문구를 쓴다(서버가 비밀번호 등 입력값을 에코해도 화면에 노출되지 않게 — AC-A2).
 */
export function localAuthErrorToFields(err: unknown): LocalAuthFieldErrors {
  ...
  if (err.code === AUTH_DUPLICATE_NICKNAME_CODE) return { nickname: DUPLICATE_NICKNAME_MESSAGE };
  ...
  if (isUnlabeledCode(err.code)) {
    if (err.status === 409) return { nickname: DUPLICATE_NICKNAME_MESSAGE };
