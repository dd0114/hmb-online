 ...
 *
 * ⚠️ **뒤집으면 계약 4건이 red 가 된다 — 그건 회귀가 아니라 설계다**(독립 검증 m3): 이 파일의 유닛
 * 1건 + `e2e/p504-d1-game-entry.spec.ts` ⑦ + `e2e/p504-onrail-telemetry.spec.ts` ③·③-b. 셋 다
 * "덱없음 자격자는 제안 대신 `offer_missed`"를 박제한 것이라, ① 로 가면 그 팔의 기대가 뒤집힌다
 * (제안이 뜨고 `offer_shown` 이 남는다). **코드가 아니라 그 네 곳의 기대값을 고쳐라.**
 */
