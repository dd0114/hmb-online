...
export type OnRailActionId =
  ...
  // ── S3 경기 ─────────────────────────────────────────────────────────────
  /**
   * 킥오프가 **성공**했다(`POST /api/matches/{id}/kickoff` 200 → 서버 `BRIEFING → GEN1`).
   * 경기 전 브리핑 스텝(`match-brief`)이 이 신호로 넘어간다 — 상태 폴링으로 흉내내지 않는 이유는
   * 이 파일 머리말과 같다("방금 그 행동을 했다"와 "원래 그 상태였다"를 구분해야 한다).
   */
  | "match-kickoff"
  // ── S5 성장 ─────────────────────────────────────────────────────────────
  //
