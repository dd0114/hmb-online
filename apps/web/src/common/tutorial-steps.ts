...
export const DECK_SETUP_STEPS: readonly TutorialStep[] = [
  {
    // ⚠️ **대상은 여전히 보드다.** 구 이유는 "버튼이 폭에 따라 두 자리 중 한 곳에만 떠서
    // (`auto-fill-top` / `auto-fill`) 직접 겨누면 반대쪽 폭에서 대상 부재 스킵"이었다.
    // #455 A3 이 그 산재를 없앴지만(경기장 우측 하단 하나) 대상은 안 옮긴다 — 그 버튼은
    // **빈칸이 있을 때만** 뜨므로 겨누면 "덱이 이미 찬 유저"에게 스킵이 된다. 축이 바뀐 것이지
    // 이유가 사라진 것이 아니다. 그리고 이제 버튼이 **보드 안**이라 링이 그것까지 감싼다.
    id: "setup-auto",
    targetTestId: "tactics-board",
    title: "선발은 자동으로 채웁니다",
    // hero: "자동완성 버튼 + 감독 한마디만 직접 타이핑" — 포메이션·슬롯 배치는 자동이다.
    body: "경기장 오른쪽 아래 [⚡ 자동 채우기]를 누르면 보유 선수로 빈 자리가 한 번에 채워집니다.",
    enabled: true,
    route: "/deck",
  },
  {
