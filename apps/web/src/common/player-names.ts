 ...
 * 짧은 이름이 실린 발행물은 <b>`data/players/players.v2.6.json`</b>(182/182 명 `shortName` +
 * 한글 표시명)이다. ⚠️ <b>`v2.5` 가 아니다</b> — 한때 여기 *"발행물(`players.v2.5.json`)에는
 * 182/182 명의 `shortName` 이 실려 있다"* 고 적혀 있었는데 그 파일의 `shortName` 은 <b>0/182</b>
 * (필드 자체가 없다)이고 이름도 <b>영문</b>이다. `v2.5` 는 #405(성장 재설계)가 먼저 발행한
 * <b>능력치 재롤</b> 층이고, 한글·`shortName` 층은 그 위에 `v2.6` 으로 올라갔다(발행 후 수정 금지
 * → 다음 버전, `data/CLAUDE.md`). 그리고 <b>서버가 지금 가리키는 파일이 `v2.5`</b> 라
 * (`server-java` 의 `application.yml` · `Dockerfile`) 이 문구가 틀린 채로 남으면
 * <b>"적힌 대로 스위치했는데 한 화면도 한글이 안 된다"</b> 가 된다(#406 W10 B-2).
 * <p>그 발행물이 도착해도 <b>서버가 `shortName` 을 실어 나르지 않는다</b> — 확인한 결손이 넷이다:
 * <ol>
