...
describe("우회 금지 — 이름 조회는 초크포인트로만", () => {
   ...
   *
   * <h3>⚠️ **테스트 파일에도 파일 단위 면제를 주지 않는다** (W8 결정)</h3>
   * `*.test.ts` 를 스캔 대상에서 통째로 빼는 안을 검토했고 <b>기각했다</b>. 이유는 하나다 —
   * 그 예외는 <b>지금 있는 파일이 아니라 앞으로 생길 모든 테스트 파일</b>에 적용되고, 그러면
   * "이 자리는 왜 초크포인트를 안 쓰나"를 <b>아무도 다시 묻지 않는다</b>. 반대로 표현 단위로
   * 등록하면 ①아래 "예외는 전부 실재한다" 검사가 매 실행마다 살아 있는지 재확인하고
   * ②<b>같은 파일에 새 위반이 생기면 표현이 달라 red 가 된다</b>(면제가 번지지 않는다).
   * W1c MAJOR-1 이 "테스트가 초크포인트를 안 써서 축이 미검정으로 남은" 부류였고, 클래스 단위
   * 면제는 정확히 그 상태를 <b>구조적으로</b> 만든다.
   *
   * ⚠️ 대신 알고 남기는 한계: 예외 키의 `<표현>` 은 **공백 접기 + 110자 절단**이라, 같은 파일에서
   * <b>글자까지 같은</b> 표현이 두 번 나오면 한 항목이 둘 다 면제한다. 그래서 예외로 남길 표현은
   * <b>그 파일에서 유일하도록</b> 이름을 잡는다(`RewardSheet` 의 `pickedGiven` 이 그 예다 —
   * 주석에 근거가 적혀 있다).
   */
  const EXEMPT = new Map<string, string>([
    ...
    [
      "rewards/RewardSheet.tsx::pickedPlayer?.name",
      "**초크포인트로 들어가는 인자**다(사다리 2단 `given` = 서버 정산 스냅샷 이름). 화면으로 나가는" +
        " 값은 `names.full(playerId, pickedGiven)` 이고, 그 계약은 `RewardSheet.names.test.ts` 가 잰다." +
        " 이름을 `pickedGiven` 으로 따로 뽑은 것은 이 예외 키가 **직접 렌더와 구별되게** 하기 위함이다.",
    ],
    [
      "match/player-stats-view.test.ts::p9.name",
      "**테스트의 기대값 단언**이고 `PlayerRow.name` = 이미 초크포인트를 지난 파생 행 모델이다" +
        "(`rowsFor` 가 `RosterMeta.short` 로 채운다 — playerId 조인이 아니다). 변수명이 `p9` 인 것은" +
        " 같은 파일의 `.map((r) => r.name)` 콜백과 이름이 겹쳐 그 줄들까지 오탐으로 잡히던 것을 푼 것.",
    ],
    [
