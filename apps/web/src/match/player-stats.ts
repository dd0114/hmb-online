...
function keeperAxis(line: PlayerStatLine, W: RatingWeights): number {
  ...
  //    이 가드는 `faced > 0` 과 같은 답을 낸다: `prior > 0` 이면 `faced = 0` 에서
  //    `shrunk = (0 + prior·E) / prior` 가 `E` 로 돌아와 **표시 기여가 0** 이고, `prior = 0` 이면
  //    `denom` 이 곧 `faced` 라 두 조건이 문자 그대로 같다. 그러니 이 줄을 **출하값에 기대어
  //    읽지 마라** — 가드가 막는 것은 `if (true)` 로 조건을 없앴을 때의 `0/0` 뿐이다.
  //    ⚠️ **"기여가 정확히 0"이라고 읽지 마라 — 부동소수에서 보편적으로 참이 아니다**(W1f minor-2).
  //    `prior·E / prior` 가 `E` 로 정확히 돌아오지 않는 조합이 있다 — 실측(`prior 1..200` ×
  //    `E 0..1` 0.01 간격 = 20,200 쌍): **2,237 쌍(11.1%)** 에 잔차, 최악이 `prior 3 · E 0.7`
  //    → `0.6999999999999998`(|잔차| `1.11e-16`).
  //    안전한 것은 **표시값**이다 — `ratingWithWeights` 의 `Math.round(x * 10) / 10` 이 그 잔차를
  //    흡수한다(위 쌍 × `scale ∈ {0,1,2.75,5.5,10,22,100}` = 141,400 조합에서 표시 위반 **0**).
  //    즉 잔차를 막고 있는 것은 계약이 아니라 **반올림**이고, 계약이 실제로 태우는 것은
  //    `E = 0.5` + `prior [0, 1, 4, 16]`(그 조합은 전부 float-exact)뿐이다.
