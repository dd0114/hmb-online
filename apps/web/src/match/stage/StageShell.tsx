  ...
  return (
    <div className={styles.shell} data-testid="stage-shell">
      ...
      */}
      ...
      >
        {/*
          ⚠️ 이 무대는 **관전(전·후반)과 종료 화면이 같이 쓴다**(FINISHED 도 `managing` 이 아니다).
          #406 W3 부터 여기에 유저용 **과거 전용 시크바**가 붙는다 — 라이브에서는 서버 시계가 미래를
          잠그고, 종료에서는 `clock === null` 이라 그 잠금이 저절로 풀려 전 구간 이동이 된다.
          그래서 결과 화면에 `reviewControls` 를 켜지 마라 — 같은 일을 두 번 하는 데다 돌려보기 묶음
          (장면 리스트·초/프레임 스텝)이 결과 패널의 세로 예산(#355)을 먹는다.
