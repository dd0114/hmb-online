 ...
 * ⚠️ **여기 적힌 총 건수는 두 번 낡았다 — 그래서 이제 안 적는다.** 한때 *"이 파일 63건"* 이었고
 * (재보지 않은 숫자 · 63 이었던 적이 없다) #406 W8 이 **52** 로 고쳤는데, 그 사이 파일이 자라
 * 지금은 **55** 다(#456 S2 재측정). 총 건수는 그 변이가 몇 건을 통과했는지와 무관하고 — 중요한
 * 것은 **죽는 3건이 전부 아래 `DENSE_LOG` 블록**이라는 사실(= 이 픽스처가 없으면 0건)이다.
 * 총 건수를 다시 적지 마라. 세는 즉시 낡고, 낡은 숫자는 다음 사람에게 거짓 근거가 된다
 * (apps/web CLAUDE.md 표 #11 이 이 트랙에 남긴 숙제 — 여기서 닫는다).
