...
/**
 * 디폴트로 하이라이트 순서 재생을 켤 하프. **지금은 비어 있다(#456 B1).**
 *
 * 구 값은 `[2]`(후반)였고 근거는 요구 원문 *"후반 입력 후 디폴트 경기 화면"* 이었다 — 그리고 그
 * 근거에는 **전제**가 하나 붙어 있었다: *"하이라이트는 **토글로 열어 둔다**"*. hero 가 #456 B1
 * 에서 그 토글을 무대에서 내리면서(*"하이라이트 토글 비활성 + 그 자리에 스킵"*) 전제가 사라졌다.
 *
 * 디폴트 ON 만 남기면 유저는 **끄는 버튼 없이** 릴이 도는 화면을 본다 — 장면 사이로 플레이헤드가
 * 튀는데 전체 재생으로 돌아갈 경로가 없다(#421 이관 발견). 그래서 여는 문을 같이 닫는다.
 *
 * ⚠️ **로직·부품은 살아 있다**(롤백 자산). 토글을 다시 노출하는 날 이 상수를 `[2]` 로 되돌리면
 * 그대로 살아난다 — 끄고 켜는 곳은 여전히 이 상수 하나 + `DEFAULT_ON_WHILE_LIVE` + 토글 버튼이다.
 */
export const HIGHLIGHT_DEFAULT_HALVES: readonly (1 | 2)[] = [];
