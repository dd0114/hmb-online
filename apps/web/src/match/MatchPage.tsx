...
export interface MatchPageProps {
   ...
   *
   * ⚠️ **출하에서는 `App.tsx` 가 채운다**(#456 S4 = 순차 보상 카드 `MatchRewardFlow`). 오랫동안
   * 아무도 안 넘겼고(설계 §9.2 는 #405 가 채운다고 적었지만 #405 는 `StageShell` 소유
   * `RewardSheet` + `!overlayOpen` 게이트로 착지했다) 그동안 CTA 는 `보상과 결과 보기` 였다.
   *
   * 기본값 `null` 은 **지우지 않는다** — 단위 계약(C2)이 "없어도 흐름이 완결된다"를 그 값으로
   * 재고, 이 화면을 다른 셸에 얹을 때 보상 층 없이 띄울 수 있어야 한다.
   */
