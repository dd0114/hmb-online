...
//
// 산출물(`dist/`)은 백엔드 0이라 **어느 정적 호스팅에도 그대로** 올라간다. 루트가 아니라
// 서브패스(`https://host/어떤경로/`)로 서빙하는 곳이면 base 를 넘겨야 에셋·라우터가 맞는다 —
// 값은 `HMB_BASE_PATH` 로 주고, 안 주면 루트(`/`) 라 로컬 `vite preview` 로 그대로 확인된다.
//
...
// SPA 딥링크(`/match/{id}` 새로고침·북마크). 정적 호스팅 다수가 없는 경로에 `404.html` 을 주므로
// index 를 복사해 두면 라우터가 이어받는다 — 이게 없으면 새로고침 한 번에 깨진 화면이 뜬다.
copyFileSync(join(dist, "index.html"), join(dist, "404.html"));
// Jekyll 계열 파이프라인이 `_` 로 시작하는 파일을 버리는 것을 끈다(에셋 이름 규칙이 바뀌어도 안전).
