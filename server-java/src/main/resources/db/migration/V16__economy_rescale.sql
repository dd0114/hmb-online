...
CREATE TABLE IF NOT EXISTS economy_rescale_v16 (
...
SELECT user_id, points * 9, 'economy_rescale_v16', 'v14', datetime('now')
FROM wallets
WHERE points > 0
  AND NOT EXISTS (SELECT 1 FROM economy_rescale_v16);
...
FROM wallets w
WHERE NOT EXISTS (SELECT 1 FROM economy_rescale_v16)
  AND NOT EXISTS (
...
INSERT OR IGNORE INTO economy_rescale_v16(id, applied_at) VALUES (1, datetime('now'));
