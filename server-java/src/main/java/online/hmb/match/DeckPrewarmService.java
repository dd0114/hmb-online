 ...
 *
 * <p><b>예산 가드</b>(P2-D8 정합): 저장 연타가 AI 콜 폭증이 되면 안 된다. 유저당 유효 prewarm 은
 * {@code deck_prewarm}(V20) 이 정확히 1행으로 강제하고, 갈아탈 때 직전 잡을 회수한다. 단 회수는
...
public class DeckPrewarmService {
    ...
    private void warm(String userId, PromptContextBuilder.BaseJob base) {
        ...
        if (base.baseId().equals(previous)) {
            jobQueue.enqueueBase(base.baseId(), base.context()); // 행이 사라졌을 때만 되살아난다(멱등)
            return;
        }
        txRunner.run(() -> {
            if (previous != null) {
