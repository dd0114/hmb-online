...
class NoticeAssetApiTest extends ApiTestBase {
     ...
     *
     * <p>상태코드를 고정하지 않는 이유: 미매핑 경로의 코드는 <b>이 계약의 관심사가 아니다</b>.
     * 이 계약이 지키려는 것은 <b>"삭제되지 않는다"</b> 뿐이고, 코드를 박으면 그 축이 바뀌는 날
     * <b>무관한 테스트가 깨진다</b>.
     * ⚠️ 실제로 그 축이 바뀌었다 — 예전엔 <b>500</b>(선재 이슈)이었고 #335 가 <b>404</b> 로 고쳤다.
     * 코드를 안 박아 둔 덕에 이 테스트는 그대로 살았다(그게 이 결정의 값어치다).
     */
    @Test
