 ...
 * 다른 테스트들(수령률 통계·감사 이력)이 <b>1행만 보는 조건</b>에서 돌게 된다.
 *
 * <p>⚠️ 정확히 적어 둔다(4차 독립검증 minor-2 가 내 과장을 잡았다): 그 상태에서도 <b>실제로는
 * 전부 통과했다</b>(역변이체 실측 17/0) — 수령률 단언은 자기 캠페인을 보낸 <b>직후</b> 목록을 읽어
 * 최신 1건이 곧 자기 것이고, 감사 단언은 목록이 아니라 {@code admin_ops_audit} 를 직접 센다.
 * 그러니 이 분리는 "약화돼 있던 것의 수복"이 아니라 <b>순서 의존이 생길 자리를 미리 없애는 위생</b>
 * 이다. 조건이 특별한 계약은 자기 방을 갖는다.
 *
