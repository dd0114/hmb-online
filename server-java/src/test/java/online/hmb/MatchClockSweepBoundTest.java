...
class MatchClockSweepBoundTest {
     ...
     * 똑같이 반환</b>해 변이체가 살아남는다(그런데 계약은 잡았다고 적혀 있었다).
     *
     * <p>⚠️ <b>중간에 시도한 2안</b>("예산 아래에서 차례로 끝나는 작업 여러 개", 700ms×4·예산 1s)에
     * 대해 한때 여기 *"같은 이유로 못 잡는다"* 고 적었는데 <b>범위를 잘못 쓴 문장이라 정정한다</b>
     * (독립 검증 델타 blocker). 정확히는 <b>변이의 모양에 달렸다</b>:
     * <ul>
     *   <li><b>가드 유지형</b>(아래 남은-예산 가드는 그대로 두고 {@code get} 인자만 작업당) — 2안은
     *       <b>못 잡는다</b>(C1 이 1.4s 에 반환해 임계 안). 그 변이에서는 가드가 초과분을 한 작업
     *       몫으로 자르기 때문이다.</li>
     *   <li><b>순수 작업당형</b>(가드까지 제거) — 2안도 <b>잡는다</b>(실측 출하 1.00s vs 변이 2.84s).
     *       그 구현에서는 가드가 아예 발화하지 않는다.</li>
     * </ul>
     * <b>채택안(아래)은 두 모양을 모두 잡는다</b>(각각 2.83s · 2.81s) — 그래서 2안이 "원리적으로
     * 안 된다"가 아니라 <b>채택안이 더 넓게 문다</b>가 참이다.
     *
     * <p>⚠️ <b>직렬성은 2안 계열에서만 필수다</b>(범위 정정, 델타검증 m12). 2안은 병렬로 돌리면
     * 웨이브가 뭉쳐 분리비가 1.42× 로 주저앉는다. 반면 <b>채택안은 두 번째 작업이 영영 안 끝나므로
     * "같이 끝나는" 상황 자체가 없다</b> — 실측도 직렬·병렬이 같다(출하 1508/1508 · 변이 2814/2821).
     * 아래에서 단일 스레드를 쓰는 것은 첫 작업이 예산을 거의 다 쓰게 <b>순서를 고정</b>하기 위해서지
     * 이 표본이 병렬에서 깨지기 때문이 아니다.
     */
    @Test
    void theBudgetIsForTheWholeSweepNotPerTask() {
        // 단일 스레드 = **순서 고정**(첫 작업이 예산을 거의 다 쓰고, 그 다음이 남은 예산을 받는다).
        // ⚠️ 이 표본은 병렬에서도 갈린다(둘째가 영영 안 끝난다) — 직렬이 필수인 것은 2안 계열이다.
