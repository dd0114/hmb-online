...
/**
 * V17(#209) 을 <b>이미 사람들이 쓰고 있는 DB</b> 위에서 검증한다.
 *
 * <p>이슈의 요구는 "기존 유저 무영향(가입 시점만 바뀐다)"이다. V17 은 튜토리얼 완료 플래그를
 ...
 */
class FlywayV17LegacyDataMigrationTest {
    ...
    void existingUsersAreBackfilledAsTutorialDoneAndNewOnesAreNot() throws Exception {
        Path db = Files.createTempFile("hmb-v17-legacy-", ".db");
        Files.deleteIfExists(db);

        // 1) 배포본 상태 재현: V16(#207 카탈로그 · #212 리스케일 포함)까지 적용된 DB + 플레이 중인 계정 2개.
        flyway(db, "16").migrate();
