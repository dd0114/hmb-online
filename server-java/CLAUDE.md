...
## 온레일 튜토리얼 — 쿠폰 · 스타터 재료 · 고정 매치 (#493 W6-v3, V43·V44 — #492 V42 와 겹쳐 개번)

hero 리플랜 v3: *"거의 정해진화면에서 유저가 선택할 여유가 없이 강제해야돼. 게임도 이겨야해. …
선수도 보유 선수말고 그냥 튜토리얼선수로. 그래야 시드값이 안바뀌어. … 모든유저가 같은 결과를 보는거야."*

### 무료 쿠폰(V43) = **1회성 권리 원장**. 지갑도 재화 원장도 아니다

- V33/V40 의 *"지급 표를 새로 만들지 마라"* 규율과 **충돌하지 않는다** — 그 규율은 **재화**에 관한
  것이고, 쿠폰이 나타내는 것은 잔액이 아니라 *"이번 한 번은 값을 안 낸다"* 는 권리다. 쿠폰이 소비되면
  **차감 자체가 일어나지 않는다**(원장에 0원 행조차 안 만든다). 지갑으로 흉내 내면 "5,000 G 를 줬다
  뺏는" 가짜 수급이 찍히고, 그 사이 잔액을 다른 데 쓸 수 있다 = 권리가 재화로 샌다.
- **멱등 2축**: 지급 = `uq_user_coupons_grant(user_id, type, grant_key)` + `INSERT OR IGNORE` /
  소비 = `UPDATE … WHERE used_at IS NULL` 의 **갱신 행 수**. 선검사(read-then-act)로 판정하지 않는다 —
  그 형태는 #286 BL-1 이 실측으로 뚫었다.
- **`type` 에 CHECK 를 걸지 않았다**. 종류가 늘 때마다 마이그레이션이 필요하면 "나중에 재사용"이라는
  목적이 깨진다. SoT = `CouponService.CouponType` enum, 모르는 문자열은 조회에서 안 잡혀 **혜택 없음**
  으로 안전하게 실패한다.
- 현재 3종: `FREE_ENHANCE`(잠재 다이스 골드) · `FREE_TRADE_RUSH`(단축) · `FIRST_TRADE_EPIC`(첫 오퍼
  등급 확정 — ⚠️ "무료"가 아니라 "결과 보장"이지만 같은 원자적 소비 규율이라 같은 표에 둔다).
- 소비는 **혜택을 주는 쪽과 같은 트랜잭션**에서만. 갈라 두면 "쿠폰은 썼는데 강화는 실패"가 되고 회수
  경로가 없다. `CouponService` 는 자체 tx 를 열지 않는다.
- 조회 = `GET /api/me.coupons`(additive, 종류별 **잔여 장수** · 0 인 종류도 키가 있다 — 클라가 "없음"과
  "모름"을 구분해야 무료 뱃지를 잘못 안 그린다).

### 스타터 재료 — 장수를 상수로 적지 마라

`TutorialStarterService` 가 가입 tx 안에서 지급한다(기존 유저 소급 지급 **없음** — 이 서비스는 가입
경로에서만 불린다 = 하위호환이 구조적으로 보장된다).

- **고정 카드 장수 = `1 + GrowthTuning.star.copies[2]`**(현재 3장). ⚠️ hero 원문은 "두개"인데 그건
  *재료가 되게 하라*는 뜻이고, 현행 계수에서 재료는 **여분 2장**이다 — 2장만 주면 승급이
  `INSUFFICIENT_MATERIALS` 로 막혀 "무조건 승급시킨다"가 **불가능**해진다. 그래서 계수에서 파생한다.
- **계약도 장수로 걸지 않았다**(`TutorialStarterTest`) — *"실제로 승급이 된다"* 로 건다. 숫자를 박으면
  계수를 조정한 순간 "테스트는 통과하는데 버튼은 막혀 있는" 상태가 된다.
- **"강화 1회 가능"의 정의 = 3지선다 1장 대기**(스탯이 오르는 유일한 경로, #405 W2b). 그래서 XP 를
  임의 숫자로 넣지 않고 `GrowthService.grantTutorialLevels` 가 **정산과 같은 함수**로 정확히 1레벨분을
  태운다(시드 접두 `tutorial:` — legacy 백필과 자리를 나눈다).
- 튜토리얼 순서가 계수에 걸려 있다: **승급(2★) → 잠재 해금 → 강화(다이스)**. 다이스는 `star < 2` 면
  `POTENTIAL_LOCKED` 다.

### 튜토리얼 고정 매치(V44 `matches.is_tutorial`) — **미리 구운 로그**를 적재한다

- **`mode` 값을 새로 만들지 않았다.** `mode` 는 보상 곡선·모드별 전적·리그/원정 분기가 전부 읽는 축이라
  값을 더하면 그 소비자들이 "모르는 모드"를 만나 조용히 폴백한다(= 보상 0 · 전적 누락).
  튜토리얼은 **연습경기의 한 종류**라 `mode='practice'` 그대로 두고 표식만 단다.
- **왜 매번 시뮬하지 않고 구웠나**: "반드시 승리"는 **엔진 버전에 걸린 성질**이다. 고정 입력·고정 시드로
  매번 러너를 태우면 엔진이 한 번 바뀌는 순간 튜토리얼이 조용히 패배로 뒤집히고, 그걸 잡을 계약을
  서버에 둘 수 없다(서버 테스트의 러너는 `FakeEngineRunner` 다). 구우면 그 위험이 0 이고 대기도 문자
  그대로 0 이다(AI 0 **+ 러너 왕복 0**).
- **그럼에도 기존 플로우 밖으로 나가지 않는다**: 자산은 `match_halves` 의 **같은 열**로 들어가므로
  관전 로그 서빙(verbatim)·결과·보상 봉투·정산·성장이 전부 평소 경로다. 분기는 딱 두 곳 —
  `enqueueHalf` 가 구운 인풋을 **`insertMaterialized`**(=#95 의 "콜 0 재사용" 경로)로 넣고,
  `simulateAndStore` 가 러너 응답 자리에 구운 결과를 끼운다. `prefetchBaseInputs` 도 건너뛴다
  (안 그러면 안 쓸 A 를 만든다 = "AI 0" 이 새는 자리).
- **`matches.seed` 에 구울 때 쓴 시드를 그대로 박는다** — `half_seed` 파생이 자산과 일치해야
  "이 로그는 이 시드로 재현된다"가 참이다. 자산·서버·굽는 스크립트가 **같은 식**(`Hashes.halfSeed`)을 쓴다.
- **1회 제한**(`TUTORIAL_ALREADY_PLAYED` 409): 구운 로그는 언제나 크게 이기므로 반복 생성이 열려 있으면
  승리 보상이 무한 발행된다. 판정은 **FINISHED 인 튜토리얼 매치 존재**이고 ABANDONED·FAILED 는 회수
  경로라 재시도를 막지 않는다.
...
## 온레일 관측 입구 — 클라가 사실을 보고하는 유일한 문 (#504 D2)

`POST /api/me/onrail-events` (`events/OnRailEventsController`). 이 서버의 이벤트 훅 10곳은 전부
**서버가 자기 동작 중에** 남긴 사실인데, 온레일(#493)은 브라우저 안에서만 도는 안내 계층이라
서버에 남는 흔적이 `matches.is_tutorial` 하나뿐이었다. 그래서 오픈베타 실유저 2명이 온레일을
한 명도 밟지 않은 것을 확인하고도 **"제안을 못 받았다"와 "제안을 받고 거절했다"를 DB 로 가를 수
없었다** — 세 가설의 서버 흔적이 완전히 같았다.

- **이벤트 6종**(`BusinessEvent.ONRAIL_*`): `offer_shown` · **`offer_missed`** · `accepted` ·
  `declined` · `step`(props `stepId`) · `done`. **V42 무변경**(그 표는 CHECK·FK 를 일부러 안 걸어
  종류 추가에 마이그레이션이 필요 없다 — 그게 "계측 추가 비용이 배포 비용이 되면 계측이 늘지
  않는다"의 실현이다).
- ⚠️ **`onrail_offer_missed` 가 이 웨이브 값어치의 대부분이다.** 제안 판정이 홈 타일 한 곳에만
  있어 하단탭 [게임]은 그것을 **평가조차 하지 않는데**(#504 D1), 그 우회의 크기를 재는 신호가
  이것뿐이다. 동선을 고치면 이 이벤트가 0 으로 떨어지는 것이 그 수정의 증거가 된다.
  - ⚠️ **이것이 세는 것은 "몇 명이 우회했나" 하나다 — 경로 분포가 아니다.** 초판이 `props.path`
    를 실었다가 **철회**했다: 보고 주체(web `GamePage`)는 `App.tsx` 의 `path="/game"` **한 곳**에만
    마운트되므로 거기서 읽는 pathname 은 진입 경로가 아니라 **도착 화면**이고 언제나 같은 상수다
    (그걸 단언한 e2e 는 어떤 우회 경로에서도 통과하는 **동어반복**이었다 — 독립검증 blocker-1).
    경로 분포를 보려면 **내비게이션 출처**를 실어야 하고, `CLIENT_ONCE_PER_USER` 에서도 빼야 한다
    (유저당 1행이면 분포가 애초에 안 생긴다) = 별개 웨이브.
  - 구 클라가 보내던 `path` 는 Spring 이 **모르는 필드로 무시**하므로 400 이 아니다(배포 순서 무관).
    계약이 그 무해함을 태운다(`OnRailEventsApiTest` ①).
- ⚠️ **위조 표면은 `CLIENT_REPORTABLE` 화이트리스트가 좁힌다.** `KNOWN` 으로 열면 클라가
  `match_finish` 같은 **서버 사실**을 위조해 퍼널을 오염시킬 수 있다(계약 = `OnRailEventsApiTest` ⑥,
  변이체로 사망 확인). 반대로 **`KNOWN` 에는 6종이 다 들어야 한다** — 안 들면
  `GET /api/admin/events?event=onrail_step` 이 400 이라 **기록해 놓고 못 읽는다**.
- ⚠️ **이 값들은 지표지 근거가 아니다.** 보상·권한·게이트에 쓰지 마라 — #493 W9 가
  `tutorial_complete`(클라 신고)를 완주 보상 판정에서 걷어내고 서버 사실로 옮긴 것이 그 교훈이다.
- **억제 축이 둘**: `CLIENT_ONCE_PER_USER` 5종은 `recordOnce`(유저당 1행, #496) · `onrail_step` 은
  반복이 의미라 그냥 append 하고 **스텝별 1회 좁힘은 클라가** 한다(새로고침을 넘겨야 하므로
  진행 상태와 같은 저장소에 산다). ⚠️ `onrail_step` 까지 `recordOnce` 로 좁히면 "어디서 이탈했나"가
  통째로 사라진다 — 그 방향의 변이는 "1행만 남는다" 계약으로는 안 죽어서 ⑤가 따로 있다.
- **훅이 컨트롤러인 것은 규칙이다**(#492) — 여기는 열리는 트랜잭션이 없어 계측이 본 동작을
  롤백시킬 위험이 **구조적으로** 0 이다. `BusinessEventHookPlacementTest.EXPECTED_HOOKS` 에
  이 파일이 훅 2개로 등록돼 있다(지우면 개수가 어긋나 red).
- ⚠️ `openapi.yaml` 엔 아직 없다(`docs/**` 는 이 모듈 밖 — `/api/growth/*`·`/api/away/*` 와 같은 상태).
- ⚠️ **`/event-board` 화면은 이 6종을 아직 못 보여준다**(#492 소유 · 화면 결정). 퍼널 칸이 안 늘어난
  것뿐 아니라 **스트림 필터 드롭다운도 7종 그대로**다(`eventboard/event-board-logic.ts:EVENT_TYPES`) —
  즉 `?event=onrail_*` 는 **API 를 직접 부를 때만** 쓸 수 있고 어드민이 화면에서 고를 수는 없다.
  (드롭다운을 늘리려면 그 목록을 **정확한 열거로 박제한 계약**(`event-board-logic.test.ts` +
  `e2e/p492-event-board-mock.spec.ts` 의 자체 목록·목 생성기)까지 같이 움직여야 해서 #492 웨이브다.)
- 계약 = `OnRailEventsApiTest`(8: **노출↔거절↔미노출 구별** · 미지 400 + 부수효과 0 · 401 ·
  유저당 1행 · 스텝 누적 · **서버 사실 위조 불가** · 조회 왕복 · **기록 실패해도 200**).
