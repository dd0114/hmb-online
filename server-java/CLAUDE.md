...
- 그래서 **정산에 도달하지 못한 매치**(킥오프 이후 포기·스톨 스윕)도 시도가 소모된 채 남는다.
  그게 옳은 방향이다 — 반대면 무르기를 반복해 시도를 안 쓰고 계속 지목할 수 있다.
...
### 계약
`AwayRevengeTest`(18) · `HomeNavBoardsTest`(12) · `DeckRequiredCodeTest`(4). **규칙 하나당 표본 하나**로
태운다 — 축이 다른 규칙을 한 픽스처에 겹치면 앞 분기가 뒤를 덮어 계약이 공허하게 통과한다(web W5 가
`WIN` + `AVENGED` + `attemptsUsed:1` 을 한 행에 뭉개 실제로 당했다). 변이체 20종 전수 사망 확인.
