...
- ⚠️ **`/api/away/*` 와 `/api/me/away-reports*` 는 openapi 에 아직 없다** — `docs/**` 가 이 세션의
  ...
  - additive: `GET /api/me` 에 `rating`, `MatchDetail` 에 `ownerName`(홈=매치 생성자 닉 — 관전자가
    홈을 자기 이름으로 오인하지 않게)
- ⚠️ **원정 보상은 economy 에 `away` 키가 없어 레거시 flat(=연습 값 승 500P)으로 폴백**한다
  (5R MIN-3). 동작은 정상이지만 "원정 보상 = 연습과 같다"는 **게임설계 결정이 어디에도 없다** —
  hero 결정 대기. 값은 `data/**` 소관이고 economy override 로 **무배포 교체**가 된다.
