...
- zip 검증 = 경로탈출(`..`) 거부 **두 층**(이름 검사 + 해제 경로 재확인) · 해제 **후** 총 바이트/
  ...
  매니페스트 4종 존재 + JSON 파싱. 실패는 **부수효과 0**(1단계에서 읽고 검증만, 통과 후에만 쓴다).
...
- ⚠️ **회수 기록은 `admin_ops_audit`(V18) 에 남는다**, `admin_catalog_audit` 이 아니라. 후자의
  `action` 엔 CHECK 제약이 있어 값을 늘리려면 **감사 테이블을 통째로 재작성**해야 하는데, 이력
  원장을 재작성하는 마이그레이션은 이 기능이 감수할 위험이 아니다(V18 은 주석에서 스스로 범용
  원장이라고 열어 뒀다). **대가 = 한 유닛의 이력이 두 원장에 나뉜다**(생성·수정은 카탈로그 원장).
...
- ⚠️ **알려진 부작용 — 회수한 번호는 재사용된다.** 채번이 `MAX(번호)+1` 이라 **가장 큰 번호**를
  회수하면 다음 생성이 그 번호를 다시 쓴다. 라이브 데이터는 안전하지만(참조 0 이어야 회수된다),
  `admin_catalog_audit` 은 그 번호의 옛 이력을 갖고 있어 **새 유닛 상세에 회수된 유닛의 이력이
  섞여 보인다**(스냅샷의 이름으로 사람은 구분할 수 있다). 고치려면 채번을 최고수위로 바꿔야
  하고 그건 스키마 변경이다 — 회수의 실사용 빈도(잘못 만든 직후)를 보면 지금은 감수한다.
