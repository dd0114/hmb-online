...
6. **디스크 규율 — 스크린샷 최소 + 끝나면 정리.** 프레임 단위 검증(순간이동·카메라 튐·프레임 유지)은
   **in-page rAF 루프로 `render()`/`cam()`/`cur()` 를 배열에 모아 반환**(메모리)해 수치로 판단하고,
   **프레임마다 PNG 저장 금지**. 눈확인용 스크린샷은 **결정적 2~5장만**. 저장 위치는 **세션 scratchpad**
   (`$OUT`, `/tmp/hmb-capture` 아님 — 그 폴더는 안 지워져 누적된다), 그리고 **작업 끝에 `rm -rf $OUT`** 로
   정리한다. (실적: 프레임 단위 스크린샷 덤프가 /private/tmp 에 1500+ PNG·400MB 누적 → 디스크 폭증.)
