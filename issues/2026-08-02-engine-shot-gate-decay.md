...
>
> ⚠️ **2026-08-03 정정 — 그 처방도 반증됐다(#407 N2+N3, engine@0.43.0). N1 은 켜지 마라.**
> N2 로 **박스를 채운 뒤에도** 감쇠는 축 B 를 판다(n60 N2+N1: 슛top1 94.5→97.3 · HHI
> 0.895→**0.948** · 비ST 슛 0.87→0.46). 볼륨 밴드까지 세게 걸면 박스 안 슛 **100%** ·
> HHI **~1.000** 으로 **Phase 2-A 기각 패턴이 그대로 재현**된다(n20 콜드 `f8 p.25 fl.05`:
> 슛 4.25 · 박스슛 100.0 · HHI 0.988). 상세 = `issues/2026-08-03-engine-box-arrival-runs.md` §3-3.
...
1. **볼륨 밴드(7.2–8.4)는 여전히 미달**(17.48). 이 웨이브가 만든 것은 *레버 두 개*지 캘리브레이션이 아니다.
   ...
   ⚠️ **2026-08-03 정정(#407 N2+N3, engine@0.43.0): 그 조건부 처방은 반증됐다 — N1 은 N2 위에서도
   켜지 못한다.** 박스를 채운 뒤에도 감쇠가 축 B 를 판다(§3-2 인용 블록의 정정 주석 참조).
   N2 자체도 파울 5.09→4.1~4.7(**n60**) 때문에 `enabled:false` 로 착지했다.
