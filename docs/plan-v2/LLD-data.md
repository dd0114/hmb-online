...
> 에픽: data · owned-glob `data/**`. 산출물은 **버전 파일**로 발행 — 소비자(server-java)는 파일만 읽는다.
>
> **⚠️ v2 발행됨(#84) — 아래 §2~§4 는 v1(가상 선수) 기준 이력이다.** 현행 = **실선수 172명**(인터내셔널 142 + 한국 30):
> - 상세·등급 기준: **`grade-mapping-v2.md`**. 이름·포지션·등급은 큐레이션 로스터(`data/players/roster.ts`)에서,
>   능력치 9종만 시드 RNG로 밴드 내 파생. 파일 = `players.v2.json`/`economy.v2.json`/`bots.v2.json`.
> - 분포: 선수 172(GK13/DF53/MF59/FW47), 등급 LEGEND14/DIA25/GOLD46/SILVER52/BRONZE35. ID `P001`~`P172`(재배정). 한국 유명선수 30명 포함(hero #84).
> - 실명 사용 허용(PoC) — **상용화 전 라이선스 해결 필수(백로그)**.
>
> (v1 이력) 실선수 금지: 이름 전부 가상(음절 조합 생성), ID `P001`~. 분포만 실축구 참고.
