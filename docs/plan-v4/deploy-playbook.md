...
**등록분 — 열차 없음.** (직전 = 엔진 열차 `engine@0.34.0` → **소진**: 2026-08-01 `v3.14` 로 발차. 라이브
engine@0.34.0 · Flyway v36 · 잔여 결함 **#388**(헤더 시계 0~44', apps/web 소유 — 표시 전용이라 롤백 안 함).
아래 접힌 원문은 이력.)

### 🚫 **상비 제약 — web 배포 동결 창** (#389 권씨, main 등록 2026-08-01)

**창**: `#389` 권씨 브랜치가 **main 에 머지된 뒤부터** 캐릭터 **오픈 전까지**.
**금지**: **CF Pages(web) 배포 전부** — 배포 사유·규모 무관. 핫픽스도 예외 아니다.

**왜**: 캐릭터가 DB 에서 **비활성**이어도 아트·이름은 `dist/chars/` 로 같이 나가고, Pages 는
**로그인 없는 공개 URL** 이다. 즉 `web` 이 나가는 순간 권씨의 **이름·전신 아트가 공개된다** —
서버 활성 여부와 무관하다. **비활성 석다이크로 같은 상태가 이미 확인된 선례가 있다.**

**절차**: 이 창 안에서 web 배포 요청이 오면 **발차하지 말고 이 제약을 먼저 소명**하고,
**hero 확인을 받은 뒤에만** 진행한다. (요청자가 main 이어도 마찬가지 — main 이 이 제약을 등록한 주체다.)

**⭕ 영향 없음**: **백엔드 도커 배포**(java·runner·executor)는 무관하다. `dist` 를 안 만진다.
⚠️ 단 `start-tunnel.sh` · `deploy-web.sh` · `publish-backend-url.sh` · **워치독 자가치유**는 전부
Pages 를 건드린다 — URL 이 바뀌면 자동으로 web 이 나간다. **터널 장애 대응이 곧 web 배포다.**
이 창에서는 그 경로도 유출 경로임을 인지하고, 이미 배포된 `dist` 스냅샷
(`~/.cache/hmb/dist-current`)이 **권씨 머지 이전 것인지** 먼저 확인한다 — `config.json` 만
갈아끼우는 `publish-backend-url.sh` 는 그 스냅샷을 그대로 다시 올리므로,
**스냅샷이 깨끗하면 터널 복구는 안전**하고 아니면 hero 확인 대상이다.
