...
| 구성 | web=Cloudflare Pages(정적) · 백엔드=**라이브 호스트**(현재 **윈도우 랩탑**) 도커 · 노출=Cloudflare quick tunnel · AI=호스트 구독 CLI |
| ⚠️ 라이브 호스트 | **맥 → 랩탑 컷오버 완료**(2026-08-19T08:28Z, #489/#472). 맥 = **롤백 자산**(정지 보존). 아래 §0.1 |

### 0.1 ⚠️ 맥에서 하지 말 것 — 워치독 재무장

맥의 자가복구 워치독은 컷오버 절차 **P0-7 로 정식 해제**됐다(plist 삭제). 이유는 그 절차의 문장 그대로다 —
*"안 끄면 두 머신이 같은 Pages 를 서로 덮는다. 배포 락은 머신 간 공유되지 않는다."*

- ❌ 터널 사고의 1차 대응으로 **맥에서 `bash infra/install-tunnel-heal.sh` 를 돌리지 않는다.**
  맥 워치독의 전파 기본값은 라이브 Pages(`hmb-online`)이고, 맥 설치기의 사전점검은 **경고만 하고 진행한다**
  (랩탑 설치기는 같은 ✗ 에서 타이머 기동을 거부한다). 결과 = 서비스 중인 랩탑을 맥이 덮는다.
- ✅ 맥 워치독 재무장은 **롤백을 실행할 때** 하는 일이다(그때는 라이브로 전파하는 것이 맞다).
- ⚠️ 맥 사전점검의 `✗ heal.conf 없음 — 전파가 기본값 'hmb-online'(라이브)로 나간다` 는 **참인 경고다.
  고치지 마라.** 처방 문구대로 `heal.conf` 를 만들면 롤백 시 전파가 `hmb-online-lab` 으로 새는데
  점검은 **✓ 를 찍는다**(참인 경고 → 거짓 안심). 근거·실측 = #123 2026-08-21 코멘트.
...
상한을 넘기면 `DEGRADED` 로 백오프한다 — 무한 재기동 방지 장치다. 그런데 회선이 계속 끊기는 날엔
상한이 금방 소진되고, 그때부터는 **터널이 죽어도 아무도 안 고친다**. launchd plist 는 `PATH`·`HOME`
만 넘겨서 **env 로는 런타임 조정이 안 되므로**, 워치독이 매 틱 읽는 노브 파일을 둔다:
```bash
bash infra/tunnel-heal.sh --budget    # 먼저: 지금 어느 축이 걸렸나 (아무것도 안 바꾼다)

printf 'HMB_HEAL_TRIES=8\n'                  > ~/.local/state/hmb/heal.conf  # 이 장애를 더 오래 붙잡는다
printf 'HMB_HEAL_MAX_INCIDENTS_PER_HOUR=6\n' > ~/.local/state/hmb/heal.conf  # 계속 죽는 날 플랩 가드 완화
rm ~/.local/state/hmb/heal.conf                                              # 회선 안정되면 원복
```
- ⚠️ **어느 축이 걸렸는지 보고 올려라** — `DEGRADED` 메시지와 `--budget` 이 축 이름을 그대로 말한다.
  세 축의 뜻은 위 §3.x 표(재시도 예산 / 장애 상한 / 폭주 방지선) 참조.
- ⚠️ 구 이름 `HMB_HEAL_MAX_PER_HOUR`·`HMB_MAX_HEALS_PER_HOUR` 는 계속 받지만 **이제 폭주 방지선**
  (기본 15)이다. 예전 완화책(`=6`)을 그대로 쓰면 방지선을 **조이는** 셈이라 의도와 반대다 (#505).
- **올려두고 잊지 마라** — 상한은 "터널이 반복해서 죽는다"를 사람에게 알리는 신호이기도 하다.
...
**주입 고장 실측(2026-07-26)**: 터널 강제 kill → **98초**. 프로세스는 살아있고 터널만 죽은
경우(2026-07-22 패턴)도 **53초**. 자세한 근거·설계 = `tunnel-resilience.md`.

⚠️ **실장애는 그보다 느리다 — 98초를 운영 기대치로 쓰지 마라** (#505). 주입 고장은 원인이 이미
사라진 뒤에 감지가 시작되므로 첫 시도가 곧바로 성공한다. 실장애는 원인(DNS·네트워크)이 아직
살아 있을 때 첫 시도가 뛰어서 **최근 4개 장애에서 첫 시도가 4/4 실패**했다 — 복구를 만드는 것은
언제나 재시도다. 실측 자동복구 MTTR = **5분 14초 / 7분 10초 / 9분 31초**, 그리고 자동복구가
끝내 안 돼 사람이 간 건이 1건(**13분 39초**, 2026-08-14).

그래서 재시도 예산을 **세 축으로 분리**했다(2026-08-14, #505) — 구조는 `MAX_HEALS_PER_HOUR=3`
한 노브가 "이 장애에 몇 번 시도하나" 와 "시간당 장애 몇 건까지 처리하나" 를 겸해서, 첫 시도가
구조적으로 실패하는데도 **여유가 한 번뿐**이었다:

| 축 | 노브(`heal.conf`) | 기본 | 뜻 |
|---|---|---|---|
| 재시도 예산 | `HMB_HEAL_TRIES` | 5 | 장애 **1건당** 연속 시도 |
| 장애 상한 | `HMB_HEAL_MAX_INCIDENTS_PER_HOUR` | 3 | 시간당 **서로 다른** 장애 처리 수(플랩 가드) |
| 폭주 방지선 | `HMB_HEAL_MAX_PER_HOUR` | 15 | 종류 불문 절대 시도 상한(#391 방지선, 정상 운전에선 안 걸린다) |

같이 들어간 것: ① **선행조건 게이트** — `api.trycloudflare.com` 이 안 풀리면 시도해도 100% 실패하므로
**예산을 쓰지 않고** 기존 터널도 죽이지 않고 다음 틱을 기다린다(그 상태가 600초를 넘으면 게이트를
무시하고 시도한다 — 게이트가 영구 차단이 되면 안 된다). ② **증거 보존** — cloudflared 실패 줄을
heal 로그에 복사하고 시도별 로그를 `~/.local/state/hmb/tunnel-logs/` 에 남긴다(단일 파일이라 다음
실행이 덮어써서 2026-08-14 2차 시도의 사유가 영구 소실됐다). ③ `status.sh` 가 **"돌고는 있는데 최근
자동복구가 실패했다"** 를 ✓ 가 아니라 경고로 띄우고, 남은 예산을 같이 보여준다.

```bash
bash infra/tunnel-heal.sh --budget      # 지금 자동으로 몇 발 남았나
bash infra/test-tunnel-heal.sh          # 예산·게이트·증거보존 계약 (주입 실패로 재현, 라이브 무접촉)
```
...
>
> ⚠️ **2026-08-13 재확인.** 그날까지 이 절 **뒤에** 승격 견적이 독립 절로 살아 있었고(현 §6.1),
> 그것 때문에 **매니저가 이미 닫힌 이 안건을 hero 에게 다시 올렸다.** 문서가 두 말을 하면
> 다음 세션도 똑같이 한다 — 그래서 그 견적을 **§6.1 "참고 이력"으로 접었다**(지우지는 않았다.
> 왜 안 했나의 근거는 남는 게 낫다). 승격 얘기를 다시 꺼내려면 **hero 결정부터** 받아라.
...
### 6.1 named tunnel 승격 견적 — **참고 이력**(2026-08-03 재검토 요청 시 산출)

> 🛑 **이 견적은 살아 있는 제안이 아니다.** 2026-08-03 에 "다시 검토해 달라"는 요청이 있어 산출했고,
> **2026-08-13 에 hero 가 재차 중단을 확정**했다(§6 의 "다시 제안하지 않는다"는 그대로 유효).
> 남겨 두는 이유는 **왜 안 했나의 근거**다 — 지우면 다음 사람이 같은 산출을 처음부터 다시 한다.
> ⚠️ 아래 "왜 다시 올리나"·"작업량·리스크"·"요건" 은 **그때의 문장 그대로**이며 **지금의 권고가 아니다.**
> 여기서 읽어야 할 것은 승격 절차가 아니라 **실장애 3건의 원인과 그 수정**(§6.1 마지막 두 절)이다.

#### 왜 다시 올렸었나 — 12시간에 터널 사고 3회 *(2026-08-03 시점 서술)*
