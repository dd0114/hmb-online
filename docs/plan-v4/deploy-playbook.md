...
**등록분 — 다음 열차 = `skipmode` 릴리스** (hero 예고, **최종 SHA 대기**). 아래는 SHA 없이 미리 확정한 것.

> **범위 = 풀 스택**: 백엔드(java·runner) **+ web 재배포**(스킵 + 브릿지 + 보상탭 + 선수기록 전부 탑재).
> web 동결은 이미 해제됐다(아래 참조) — 이번엔 web 을 **쏘는 게 목적**이다.

| # | 관문 | 내용 |
|---|---|---|
| 1 | **DB 백업 + 마이그레이션 리허설** | **V38·V39·V40 3건**. `.sql.conf` 짝파일 유무를 **먼저** 본다(있으면 비원자 → §8 전량). 라이브 사본에 3건을 **순서대로** 걸어 보고 행수·`integrity_check` 대조. ⚠️ 파괴 연산(`UPDATE/DELETE/DROP`)이 있으면 §0.5-7 대로 **무엇이 사라지는지** 읽고 hero 확인 |
| 2 | **#241 진행매치 0** | `engine@0.40.0 → 0.42.0` 범프 = 구 `resumeState` 거부. 발차 직전 재확인 |
| 3 | **economy override** | ⚠️ §0.6 — `mission.reward` 블록이 새로 필요하다. `/var/lib/hmb/economy.override.json` 은 **부분 병합이 아니라 문서 통째 교체**라, 현 override 를 그대로 두면 새 발행물의 `mission.reward` 가 **조용히 무시된다**. 반대로 지우면 지금 살아 있는 운영 조정(가입 젬 12,000 등)이 날아간다 → **새 발행물 기준으로 override 를 재작성**(§0.6 2-B) |
| 4 | **컨테이너 모듈 스모크** | 태그 전환 **전**. #385 절차(§0.8) — 모듈 로드 → `/health` → **`/simulate` 실왕복**. `configOverrides` 축도 같이 재확인 |
| 5 | **#396 프리플라이트** | 러너를 되돌릴 상황이 오면 `bash infra/preflight-runner-rollback.sh` 선행 |
| 6 | **실매치 완주 스모크** | 연습 1판 + **스킵 동작**(이번 열차의 목적) · 보상탭 · 선수기록. 고정 계정 `deploy-smoke`(§0.55) |
| 7 | **deploy-log** | 매니페스트 값으로 append |

**롤백 기준선**: 발차 전 `prev-live` 2개 고정(현재 java `2fc8a23d…` / runner `0fde29d1…`).
