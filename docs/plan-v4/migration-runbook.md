...
```
- **기대**: 마지막 줄 `OK`. 항목별로:
  - **P2-14 상당** — `integrity_check=ok` · flyway 최대 버전 · **행수 src=dst**(users·matches 를
    따로 재고 **둘 다 정수일 때만** 인정 — 묶어서 재면 절반만 나와도 통과한다) ·
    **sha256 두 시점 대조**(백업 직후 ↔ 볼륨 적재 후). 해시는 **스택 기동 전에** 잰다 —
    java 가 뜨면 WAL 체크포인트로 파일이 바뀌어 같은 내용에도 해시가 달라진다.
  - **P3-18/19/21 상당** — `AdminBootstrap admins=1` · `/internal/health` **토큰 없이 401 + 토큰
    있이 200**(401 만 보면 "경로가 살아있다"만 알고 "토큰이 통한다"는 모른다 — 이사에서 깨지는
    쪽은 보통 후자다) · runner `/health` 200.
  - **P5-28 상당** — 로그인 `isNew=false`(= DB 가 정말 넘어왔다는 증거) · `/api/me` 200 ·
    **`GET /api/deck` 200** · **매치 1판 완주**(킥오프 → 전반 스킵 → 감독시간 → 후반 스킵 →
    `FINISHED` → 결과 200).
  - **economy** — 파일 이송 + **`GET /api/admin/economy` 가 `source: OVERRIDE` ·
    `overrideFilePresent: true`**. 파일이 있다는 것과 서버가 그 값을 쓴다는 것은 다른 명제고,
    어긋나면 화면은 멀쩡한 채 숫자만 틀리는 무음 장애다.
  - **격리** — 라이브·growth 컨테이너 **ID·기동시각 완전 동일** · 라이브 볼륨 무변경.
- **실패 시**: 그 항목이 실이사에서도 실패한다. 정지 창을 잡기 전에 여기서 고친다.
- ⚠️ **이 리허설이 검증하지 않는 것**(통과했다고 이사가 끝난 게 아니다):
  - **라우터 스왑(P4)** — 테스터가 보는 URL 을 실제로 바꾸는 일이라 같은 머신에서 흉내낼 수
    없다(흉내내면 그게 라이브를 건드리는 것이다). P0/P1(사전 준비)·P4 는 리허설 밖이다.
  - **이미지 빌드** — 리허설은 **이미 있는 로컬 이미지**로 뜬다. 새 머신에서 처음 빌드할 때
    나는 실패(아키텍처 arm64/amd64 불일치, 빌드 캐시 부재, 의존성 원격 조회)는 여기서 안 난다.
  - **실제 네트워크 전송** — `rsync`(P2-15)를 타지 않고 같은 머신 안에서 볼륨→볼륨으로 옮긴다.
    전송 중 손상·중단·권한 소실은 검증 대상이 아니다(그래서 P2-15 의 재대조가 따로 있다).
  - **AI 모드 A** — executor 를 **`AI_EXECUTOR=stub`** 으로 띄운다(구독 세션을 리허설에 쓰지
    않는다). 실제 claude CLI 경로·구독 재로그인은 P3-20 에서 별도로 확인한다.
