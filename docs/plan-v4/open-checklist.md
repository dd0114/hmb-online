...
[ ] 상시 URL(선택): named tunnel / Pages 는 API토큰 방식 준비됨(#122)
```

**발견 이슈**: #125(livelock) · #165(백엔드 /version) · **#166(라이브AI 동시성 blocker)**.
**GO 판정 요약**: 인프라·배포 경로·라이브 AI 단일매치 = **green**. 잔여 GO 조건 = **B3 고지** +
**#166 동시성**(소규모 concurrency=1 이면 통과, 다수 동시매치 예상 시 선해결) + 도메인별 점검.
p3dep(인프라) 몫은 완료. 최종 GO 는 매니저/도메인 판정.
