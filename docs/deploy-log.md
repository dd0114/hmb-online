...
2. **하향 공지 미게시** — 4행이 어드민 override 값 → v2.7 정규값으로 내려앉았다(§6-3 표):
   `P182 오시야스 822→680`(−17.3%) · `P181 석다이크 812→673`(−17.1%) ·
   `P180 경니시우스 807→669`(−17.1%·**3명**·5장) · `P174 권씨 805→668`(−17.0%·**1명**).
   ⚠️ **이건 "보상"이 아니라 "고지"다** — 스펙 §6-3 은 이 4행에 **별도 보상을 배정하지 않았다**
   (보유자 있는 것은 권씨1·경니시우스3 뿐). 직전 세션 핸드오프가 *"보상 우편 — P174 1명 · P180 3명"* 로
   적은 것은 **고지 대상 수를 보상으로 옮겨 적은 것**이라 여기서 정정한다. 별도 보상을 줄지는
   **hero 결정 사항이고 아직 결정된 바 없다**.
