...
1. **은퇴 보상 우편 미발송** — `roster-v27-spec.md` §4 = 대체카드 **2,244장**(MULT=3, hero H6).
...
2. **하향 공지 미게시 + 하향 보상 미발송** — 4행이 어드민 override 값 → v2.7 정규값으로 내려앉았다(§6-3 표):
   `P182 오시야스 822→680`(−17.3%) · `P181 석다이크 812→673`(−17.1%) ·
   `P180 경니시우스 807→669`(−17.1%·**3명**·5장) · `P174 권씨 805→668`(−17.0%·**1명**).
   - **공지 = 3종**(권씨·경니시우스·오시야스 = 내려가는 카드 전부. 석다이크는 신규 오픈이라 하향이 아니다).
   - **보상 = 4명**(권씨 1 + 경니시우스 3). **오시야스는 보유자 0명이라 공지만 하고 보상 대상이 아니다.**
   ⚠️ 근거는 스펙이 아니라 **#450 hero 결정 4차**(`5193402946` §2)다 — `roster-v27-spec.md` §6-3 은
   이 4행을 *"고지 대상"* 으로만 적어서 스펙만 읽으면 보상이 없는 것처럼 보인다. **두 문서가 어긋나 있고
   hero 결정이 우선**이다. ⚠️ 다만 **보상의 형태·수량은 어느 쪽에도 적혀 있지 않다**(은퇴 보상의
   `MULT=3`·`sub(X)` 는 §4 = 은퇴 축 전용이라 여기에 그대로 쓸 수 없다) → **그 한 가지는 hero 결정 대기**.
3. **석다이크 오픈 공지 미게시**(P181 = 이 배포로 실제 오픈됨).
   ⚠️ hero verbatim 은 *"권씨랑 오시야쓰"* 였으나 실측상 **피해 최대는 경니시우스**(3명 5장 전원 덱 편성)라
   하향 공지는 **3종**으로 확장된 상태(#450 hero 결정 4차 §2).
