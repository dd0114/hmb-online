...
2. **무의미한 탭 한 번이 상대 팀 표에 영구히 가뒀다**(신규 minor-1, major-1 과 같은 증상·다른 경로).
   ...
   정의상 "고른 것"이 아니다 → **홈 픽스처**로 옮겼다(기본값 `home` == 내 팀 `home`, 유저가
   `away` 로 실제로 바꾼다 = **탭 한 번**으로 *따라간다*와 갈린다).
   ⚠️ **R2 가 여기 적었던 *"어웨이 픽스처에서는 구별 가능한 진짜 선택이 존재하지 않는다"* 는
   거짓이다**(R3 — 독립검증 major-1). 참인 것은 **초기 상태에서 탭 한 번으로는 만들 수 없다**는
   것뿐인데 문장이 픽스처 전체로 양화됐다. **2탭이면 만들어진다** — `away`(실변화 ①) →
   `home`(실변화 ②)은 둘 다 no-op 가드에 안 걸리는 진짜 선택이고, 착지가 *따라간다*와 다르다.
   R3 이 직접 잰 값(`meDelayMs 900`, 어웨이 픽스처):

   | 탭 | 도착 후 세그먼트 | 표 |
   |---|---|---|
   | 0회 | `{mine:"away", selected:"away"}` | `away-P014` 1행 / `home-P116` **0행** |
   | 2회(`away`→`home`) | `{mine:"away", selected:"home"}` | `home-P116` 1행 / `away-P014` **0행** (16초 뒤도 동일) |

   홈 픽스처를 고른 이유는 불가능이 아니라 **더 직접적이기 때문**이다. 그리고 **2탭 계약을
   어웨이 픽스처로 승격**했다(R3) — `apps/web/CLAUDE.md` #322 가 *"표본이 계약의 절반"* 이라고
   적은 자리가 어웨이 라운드이고, 표본 교체가 그 축을 홈으로 밀어 놓은 것을 되돌린다. 두 표본은
   대체가 아니라 **짝**이다(홈 = 1탭 최단 경로 / 어웨이 = #322 표본 축).
...
### W4 R3 — 독립검증 수습 (major 1 + minor 2)

R2 는 **PASS** 를 받았다(blocker 0). 이 라운드는 그 위의 major 1 · minor 2 를 닫는다. **런타임
결함은 다시 없었다** — major 는 R2 가 적은 절대 서술이 거짓이었던 것이고, **이 에픽에서 같은
모양이 세 번째다**(R1 MOTM "구조적 불가" → R2 ① / R2 "진짜 선택 부재" → 여기).

1. **major-1 — *"어웨이 픽스처에서는 구별 가능한 진짜 선택이 존재하지 않는다"* 는 거짓이다.**
   R2 가 세 곳(e2e 머리말 · 이 문서 · 커밋 `4edce85` 메시지)에 그렇게 적었다. 참인 것은
   **초기 상태에서 탭 한 번으로는 만들 수 없다**는 것뿐인데 문장이 **픽스처 전체로 양화**됐다.
   R3 이 어웨이 픽스처(`meDelayMs 900`)에서 직접 재현한 값:

   | 도착 전 탭 | 탭 직후 | 도착 후 세그먼트 | 표 |
   |---|---|---|---|
   | 0회 | — | `{mine:"away", selected:"away"}` | `away-P014` 1행 / `home-P116` **0행** |
   | 2회 `away`→`home` | `away` → `home`(둘 다 실변화) | `{mine:"away", selected:"home"}` | `home-P116` 1행 / `away-P014` **0행** |

   2탭은 **두 번 다 no-op 가드에 안 걸리는 진짜 선택**이고 착지가 *따라간다*와 명백히 다르다
   (16초 = 폴링 여러 바퀴 뒤에도 그대로).
   → **정정 3곳** + **표본 교체는 유지한다**(R2 의 `MU6` 킬 파워는 실재한다 — value-shopping 아님).
   홈 픽스처를 고른 이유는 불가능이 아니라 **1탭이라 더 직접적**이기 때문이다.
   → ⭐ **그리고 2탭 계약을 어웨이 픽스처로 승격**했다. 문장만 고치면 커버리지는 홈에 남는데,
   `apps/web/CLAUDE.md` #322 가 *"표본이 계약의 절반"* 이라고 적어 둔 자리가 정확히 **어웨이
   라운드**다(그 표본이 없어서 "홈은 내가 아니다"가 3개월 살았다). 두 표본은 대체가 아니라 **짝**이다.
2. **minor-1 — `flipTeams` 는 로그 자신의 `finalScore` 를 안 뒤집는다.** 뒤집힌 로그에서 골
   이벤트는 `home` 소속인데 `finalScore` 는 `{home:0,away:1}`(H1)·`{home:0,away:2}`(H2) 그대로다
   (실소비자 = `match-logic.ts:fallbackScore`). 목의 경고문은 *"매치 **메타**와 어긋난다"* 만
   말해 **로그 내부의 두 번째 불일치는 안 적혀 있었다**. 지금 유일한 소비 계약(MOTM)은 스코어를
   안 읽어 **무해**하다 → **고치지 않고 경고문에 명시**(뒤집으면 매치 메타·스코어바·타임라인이 같이
   움직여 이 목이 재려던 축이 흔들린다).
3. **minor-2 — `useTeamSegment` 세터가 매 렌더 새 신원이었다** → **`useCallback` + 함수형 갱신**.
   ⚠️ 신원만 고정하면 절반이다 — `team` 을 클로저로 잡는 형태는 **한 배치 안에서 두 번 부르면
   두 번째가 낡은 값과 비교**해 유저의 최종 의도를 삼킨다. 함수형 갱신이 그것까지 닫는다.
   ⚠️ 그리고 **이 축은 계약이 없으면 조용히 되돌아간다** — R2 형태로 되돌리는 변이가 기존 e2e·유닛
   **7건을 전부 통과**했다(아래 표 `MU-C` 의 e2e 열). → `src/match/useTeamSegment.test.ts` 신설(4건).

**변이**(CTRL green · CANARY red 선행, 판정 = exit code, 매 케이스 md5 복구 대조 — 5회 전부 일치):

| 변이 | 새 e2e (어웨이 2탭) | 새 유닛 (세터 4건) | 읽는 법 |
|---|---|---|---|
| CANARY 세터가 아무것도 안 한다 | **KILLED** | **KILLED**(1/4) | 하네스 생존 |
| **MU-A** 늦게 온 `myTeamSide` 가 `picked` 를 덮는다 | **KILLED** | — | ⭐ 승격한 계약이 실제로 문다 |
| **MU-B** no-op 가드 제거 | **SURVIVED**(의도) | **KILLED**(1/4) | 2탭은 no-op 축이 아니다 = 과잉 제약 아님 |
| **MU-C** R2 형태 복원(클로저 `team` · `useCallback` 없음) | **SURVIVED** | **KILLED**(3/4) | minor-2 는 e2e 로 못 잡는다 |
| **MU-D** 신원만 고정(비교는 클로저 `team`) | — | **KILLED**(2/4) | 두 축이 분리돼 있다 |

⚠️ **`-x` 가 새 계약을 가릴 뻔했다** — 1차 라운드는 describe 전체를 돌려 `MU-A` 가 **앞선** 홈
픽스처 계약에서 먼저 멈췄고, 그러면 승격한 계약이 죽는지는 **한 번도 안 재고** "KILLED" 로 적히게
된다. 그래서 새 계약만 `-g` 로 좁혀 **다시 돌렸다**(위 표는 그 값).

**게이트**(콜드, `uptime` 병기): `apps/web build` EXIT=0(load 30→34) · `vitest run apps/web`
EXIT=0 **2039 passed / 8 skipped**(R2 2035 + 새 유닛 4, load 33→63) · playwright **6스펙**
(`p403-result-players` · `p403-player-tab` · `p403-player-detail` · `p322-side-orientation` ·
`p322b-stats-team-header` · `p388-header-clock`) EXIT=0 **85 passed**(load 68→73). 실패 0.
⚠️ R2 의 *"6스펙 155 passed"* 와 **셋이 다르다**(R2 는 스펙 목록을 안 적었다) — 위 목록이 R3 의 셋이고,
고른 기준은 *"바뀐 파일의 소비자 전부(`p403-mocks` 를 import 하는 3스펙) + 이 라운드가 되돌린 #322 축"* 이다.
⚠️ **손대지 않은 것**: `p403-player-detail.spec.ts:96` 부하 민감 플래키(#436, 이번 실행에선 green) ·
`p233-second-half-score.spec.ts` 선행 red(#433, 게이트에 안 넣었다).
