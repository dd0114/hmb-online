...
2. **MOTM 을 `home` 사이드에서만 못 찾는 변이가 살아남았다**(minor-1). 그리고 그건 픽스처 선택이
   아니라 **구조**다: 평점이 10.0 에서 포화해 동점자가 여럿이고(아래 "남은 것") `pickMotm` 의
   마지막 tie-break 가 **키 오름차순**이라 `away:*` 가 언제나 이긴다 = **MOTM 이 home 인 실로그
   표본은 만들 수 없다**(팀 라벨을 통째로 뒤집어도 tie-break 가 다시 away 를 고른다).
   → 탐색을 `player-stats-view.motmRowOf` 로 **순수 계층에 내리고** 거기서 home/away 표본을
   직접 먹인다. 프롬프트가 제안한 "목 로그 변형"은 위 이유로 **성립하지 않아 따르지 않았다**.
...
⚠️ `motmRowOf` 의 `home` 항 제거는 **e2e 로는 여전히 SURVIVED** 하고(2번의 구조적 이유) 유닛이 죽인다 —
그 사실 자체가 이 계약이 순수 계층에 있어야 하는 근거다.
