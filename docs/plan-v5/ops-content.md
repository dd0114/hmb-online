...
### 7.2b 유닛 회수 (#210) — 알려진 대가 두 가지

둘 다 **고칠 수 있지만 대가가 더 큰** 자리라 기록하고 감수한다. 되돌리려면 여기부터 읽어라.

1. **이력이 두 원장에 나뉜다.** 회수 기록은 `admin_ops_audit`(V18) 에 남고 생성·수정은
   `admin_catalog_audit` 에 남는다. 후자의 `action` 에 CHECK 제약이 있어 값을 늘리려면
   **감사 테이블을 통째로 재작성**해야 하는데, 이력 원장 재작성 마이그레이션은 이 기능이
   감수할 위험이 아니다.
2. **회수한 번호가 재사용된다.** 채번이 `MAX+1` 이라 가장 큰 번호를 회수하면 다음 생성이 그
   번호를 다시 가져간다. **라이브 데이터는 안전하다**(참조 0 이어야 회수되므로 남의 카드가
   바뀌지 않는다). 다만 카탈로그 원장이 그 번호의 옛 이력을 갖고 있어 **새 유닛 상세에 회수된
   유닛의 이력이 섞여 보인다**. 고치려면 채번을 최고수위(high-water mark)로 바꿔야 하고 그건
   스키마 변경이다 — 회수의 실사용 빈도(잘못 만든 직후)를 보면 지금은 비용이 크다.
