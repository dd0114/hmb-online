...
> **웹을 다시 배포**해야 한다. 유닛은 **아트**가 배포에 묶여 있다(등록 자체는 이미 무배포다 — §7.0).
> 이 문서는 그 배포 의존을 끊는 설계다. **W1 = 공지 이미지 업로드**(소형, §1~§6),
> **W2 = 유닛 아트 핫로드 + 회수**(중형, §7). 둘 다 구현·검증 완료(§5.5 · §7.5).
...
### 7.2b 유닛 회수 (#210) — 처음 감수했던 대가 두 가지를 **hero 지시로 해소**

1차 구현은 둘을 "기록하고 감수"했는데, hero 가 **둘 다 고치라고** 했다(2026-07-30). 회수 기능은
그때까지 라이브에 배포된 적이 없어(main 에 엔드포인트가 없었다) **회수된 유닛이 0** 이었다 —
지금까지의 유닛 정리는 전부 비활성화였다. 그래서 마이그레이션·동작 변경의 소급 위험이 없었다.

| 처음의 대가 | 왜 감수했나 | 어떻게 고쳤나 |
|---|---|---|
| **이력이 두 원장에 나뉜다** (회수만 `admin_ops_audit`) | `admin_catalog_audit.action` 의 CHECK 를 넓히려면 **테이블 재작성**이 필요하고, 이력 원장을 재작성하는 마이그레이션의 위험을 그 기능이 지기엔 컸다 | **V32 재작성**으로 CHECK 에 `unit_purge` 추가 → 생성·수정과 **한 원장**. 계약 = `FlywayV32CatalogAuditRebuildTest`(**마이그레이션 전에 심은 행**의 내용 보존 · **인덱스 4개** 재생성(부분 조건까지) · 두 유니크가 실제로 잠그는지 · 새 action 수용 · 오타 거부) |
| **회수한 번호가 재사용된다** | 채번을 최고수위로 바꾸는 것이 스키마 변경으로 보였다 | **`meta_kv.unit_id_high_water`** 로 해결(새 표 불필요). 회수가 비운 번호를 수위에 남기고 채번이 그걸 함께 본다 |

**왜 재작성이 안전한가**(⚠️ `DROP TABLE` 이 들어 있다): 데이터를 **변환하지 않고** 전 컬럼을 복사하고,
`.sql.conf` 를 두지 않아 **Flyway 트랜잭션 안에서 원자적**이며, 이 표를 FK 로 참조하는 표가 없다.
인덱스는 표와 함께 사라지므로 **넷 다 다시 만든다**(V14 **셋** + V15 하나).
배포 시 §0.5 체크 1·7 에 걸리므로 **백업이 안전망**이다.

> ⚠️⚠️ **여기서 blocker 를 하나 냈다**(독립검증). 초판이 V14 를 "둘"로 오독해
> `uq_catalog_audit_idem`(대상별 멱등, 4개 액션의 유일한 DB 백스톱)을 빠뜨렸고 — 더 나쁜 것은 —
> **계약이 `containsExactlyInAnyOrder(3개)` 로 그 손실을 박제**하고 배포 확인 커맨드가 "3이면 정상"
> 이라 보고했다. 즉 **고치려 하면 테스트가 막는** 상태였다. 계약이 구현을 검증한 게 아니라 복사한 것이다.
> 교훈 둘: ①**인덱스를 세는 계약은 원본 마이그레이션을 직접 읽고 세라**(구현을 보고 세면 오독이
> 그대로 계약이 된다) ②**재작성 검증은 마이그레이션 전에 행을 심어야** 한다 — 끝난 DB 에서 새 행을
> 왕복시키면 "기존 행 보존"을 한 번도 검사하지 않아 `INSERT..SELECT` 에서 컬럼을 빼도 통과한다(실측).

**실패 기록의 트랜잭션 경계**도 같은 라운드에서 걸렸다: 거절(409)을 실패하는 트랜잭션 **안에** 쓰면
롤백이 그 기록을 같이 지운다. 성공은 삭제와 한 트랜잭션(안), 실패는 그 밖 — 방향이 반대다.

⚠️ **수위를 올리는 곳은 회수 한 곳뿐**이다(생성은 올리지 않는다 — 살아 있는 번호는 `players` 가 이미
갖고 있어 중복 기록이다). 그래서 "이 키가 있다 = 회수된 번호가 있다"가 성립하고, 운영 진단에 쓸 수 있다.

### 7.5 W2 독립검증 (2026-07-30) — 1차 **FAIL** → 수습 후 재검증 **PASS**

1차 판정은 blocker 1 · major 4 였다. **기계 게이트는 전부 green 이었는데도** 그랬다는 것이 요점이다 —
수치가 아니라 **계약이 무엇을 구속하는가**가 문제였다.

| # | 지적 | 실체 | 조치 |
|---|---|---|---|
| **BLOCKER** | 매니페스트 JSON 손상이 **고아 리비전 디렉토리**를 남긴다 | 파싱이 **쓰기 뒤에** 있어 "다 쓴 뒤 400". 최대 64MB, 회수 동사 없는 보관소라 영구 누수. 하필 **zip 을 몇 번 고쳐 올리는 것이 정상 사용 패턴** | `extract()` → `read()`(검증만) + `write()`(통과분만)로 분리, 파싱을 쓰기 앞으로. 계약은 DB 행이 아니라 **디렉토리 수** |
| **MAJOR-1** | §7.2 base 전환에 **계약 0** | 기본값을 `CHARS_BASE` 로 되돌리는 변이체가 **1334건 전부 통과**. 그 상태 = 이미지가 전부 웹 오리진 404 = 전 아트축 사망. e2e 는 백엔드가 없어 **구조적으로** 못 잡는다 | URL 조립 3축 + `resolveTile` + 롤백 방향 단언 |
| **MAJOR-2** | 활성 리비전인데 파일 없음 → **폴백 경로 소멸** | `index()` 가 DB 만 보고 200 → 매니페스트 전부 404 → 화면 통째로 이니셜. **플레이북이 반대로 적혀 있었다** | 서버가 파일 존재 확인 + web 빈 번들 재폴백(두 층) + 문서 정정 |
| **MAJOR-3/4** | reason 필수·DB 롤백에 **계약 0** | W1 에서 지적받아 고친 것을 **아트 번들엔 코드만 복사**했다 | 형제 계약을 그대로 이식(트리거 주입 포함) |

재검증에서 **MIN-A**(부분 유실은 아직 폴백 안 됨 — `index()` 가 매니페스트 1종만 확인)와
**MIN-B**(비-zip 문구에 계약 없음)가 새로 잡혀 둘 다 닫았다.

> ⚠️ **정정 — 변이체 보고를 정확히 적어라.** 구현 세션이 "변이체 A(파싱 순서 되돌리기)가 테스트를
> 죽였다"고 보고했으나 **사실이 아니었다**: 그때 실제로는 파싱 순서와 백스톱을 **동시에** 되돌렸다.
> A 단독은 **살아남는다**(백스톱이 흡수 = equivalent mutant). 정확한 문장은 **"한 층만 지우면 안
> 죽고, 두 층 다 지우면 죽는다"** 이고, 그게 심층방어가 작동한다는 증거다. 이 구분을 뭉개면 나중에
> 백스톱만 지운 사람이 "변이체가 죽으니 계약이 있다"고 오판한다.

살려 둔 것(근거): **MIN-2** `purge` 에 busy 재시도 없음 — 트랜잭션이 통째 롤백돼 **데이터는 안전**하고
대가는 운영자가 409 대신 500 을 본다는 것뿐이다(회수는 잘못 만든 직후 1회 쓰는 동사다).
**`write()` 2층 경로 재확인**은 API 레벨에서 단독 변이체가 살아남지만 1층에 단독 계약이 따로 있다.
