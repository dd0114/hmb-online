...
|---|---|---|
| **이력이 두 원장에 나뉜다** (회수만 `admin_ops_audit`) | `admin_catalog_audit.action` 의 CHECK 를 넓히려면 **테이블 재작성**이 필요하고, 이력 원장을 재작성하는 마이그레이션의 위험을 그 기능이 지기엔 컸다 | **V32 재작성**으로 CHECK 에 `unit_purge` 추가 → 생성·수정과 **한 원장**. 계약 = `FlywayV32CatalogAuditRebuildTest`(행 내용 보존 · **인덱스 3개** 재생성 · 새 action 수용 · 오타 거부) |
...
인덱스는 표와 함께 사라지므로 **셋 다 다시 만든다** — 특히 V15 부분 유니크를 빠뜨리면
`unit_create` 멱등 백스톱이 조용히 사라져 #207 B1(같은 키 동시 생성이 유닛 2개)이 되살아난다.
