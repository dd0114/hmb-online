# 게임화면(관전) 레이아웃 안 — 경기장면 고정 + 정보 시트 + 뷰어 SoT 수렴

> ⚠️ **#284(2026-07-29) 로 정보 토글이 폐기됐다** — 시트는 상시이고 탭 구성은 매치 상태가 정한다.
> 이 문서에서 "토글"이라고 적힌 자리는 §3 현행 규칙이 이긴다(아래 그림·표는 #169 당시 이력).
...
├────────────────────────────────────────┤
│  📊 통계    📜 로그    📝 후반지시   ⏸ │  64px   [C] 토글바 (#284 에서 삭제)
└────────────────────────────────────────┘  844
```
...
- ~~**기본 상태 = 토글 전부 off** → 무대만(R3).~~ → **#284: 시트는 처음부터 열려 있다**(§3).
...
### 2.5 데스크탑 (≥1024px) — 시트 대신 우측 도크  ⚠️ **폐기(§9.10)**

> hero 리뷰 결과 이 안(우측 도크)은 **폐기**됐다 — 무대가 좌로 쏠려 화면이 비대칭이었다.
> 확정안: **데스크탑도 폰과 같은 형태**(무대 가운데, 시트·토글 하단). 아래 그림은 이력으로 남긴다.
...
> ⚠️ **#284 에서 토글이 폐기됐다(hero 결정, 2026-07-29).** 아래 "구 규칙"은 이력으로 남긴다 —
> 되살리려는 변경이 오면 그게 왜 폐기됐는지부터 읽어야 한다.

### 현행 (#284)

- **토글바 없음.** 정보 시트는 **상시**이고, 유저는 탭만 고른다.
  - 폐기 이유: 하단 토글바와 시트 탭바가 **똑같이 생긴 줄 두 개**로 화면에 동시에 떴다(hero 캡처
    `issue1.png`). hero: *"탭 구조면 그 안에서 조정하면 되지 껐다켰다 할 필요가 없다."*
- **무엇이 탭인지는 매치 상태가 정한다**(유저 설정이 아니다) — `tabsFor(state, statePanel)`:
...
- **`후반 지시` 는 전반에만.** 감독시간엔 `감독` 탭이 **같은 입력을 프리필된 채로** 갖는다(§3.1) —
  둘을 같이 띄우면 같은 문장을 고치는 칸이 둘이 되고 어느 쪽이 이기는지 알 수 없다.
  후반·종료엔 서버가 409 로 막으므로 애초에 낼 곳이 없다.
- **표시 순서와 기본 선택은 다른 축**이다. 순서는 통계·로그·후반지시(익숙한 순서), 기본으로 열리는
  건 **로그**(hero 확정 — 관전 중 볼 게 있어야 한다. 통계는 초반에 0-0/0슛이라 빈 표처럼 보인다).
  단 상태 패널(감독·결과)이 있으면 **그게 먼저** — 지금 해야 할 일이 정보 탭을 이긴다.
- 활성 탭 1개만 렌더(높이 고정 = 무대 보호). 시트가 비는 상태는 이제 없다(통계·로그는 항상 있다).
- **접근성**: 시트 탭은 `role="tablist"`. 무대는 `aria-live="polite"`로 주요 이벤트 자막을 읽어준다
  (골/카드만 — 소음 방지).
- 계약: `stage-state.test.ts`(탭 구성·기본 탭) + `e2e/p284-info-tabs.spec.ts`(실화면) +
  `e2e/match-stage.spec.ts`(무대 생존 — 이건 안 뒤집혔다).

### 3.1 후반 지시 미리작성 (#284)

- 대상은 **팀 전체 + 로스터 전원**(선발 11 + 벤치). 목록의 출처는 `match.userDeckSnapshot` —
  서버가 `POST /prompts{scope:player}` 의 playerId 를 같은 스냅샷으로 검증하기 때문(현재 덱으로
  만들면 전반 중 덱을 고친 유저에게 **400 나는 칩**을 보여준다). 스냅샷 없는 구 매치만 덱 폴백.
- 형태는 **대상 칩 + `PromptBlock`**(덱·감독시간과 같은 부품). 감독시간처럼 보드를 통째로 넣는 안은
  기각 — 위에서 경기가 도는 시트라 세로가 없다.
- **저장 버튼이 없다.** 타이핑이 멎으면 자동으로 서버에 간다(`useHalftimeDraft`).
  - hero 는 "감독시간이 끝날 때 알아서 저장"을 제안했으나 **기각**: 만료는 서버 이벤트이고
    (`MatchClockSweeper` — *"아무도 보고 있지 않은 매치도 … 후반 시뮬로 넘어간다"*,
    `sweep-interval-ms: 1000`), 탭이 닫혀 있으면 저장할 주체가 없다. 열려 있어도 전이 후 도착한
    저장은 409. → **유저가 확실히 화면에 있는 동안** 보낸다. 체감은 같다(버튼을 안 누른다).
  - 대가: 저장이 조용하므로 **실패도 조용해질 수 있다** → 상태 줄(`brief-save-status`)이 필수다.
- 적어둔 문장은 감독시간 `감독` 탭에 **프리필**된다. 붙는 기준은 **playerId**(슬롯 아님) — #276 이
  자리 바꾸기를 열었으므로 슬롯 인덱스는 움직인다. 사람에게 한 말이 자리를 따라가면 안 된다.
- 화면 복원의 원천은 **로컬 초안**(`hmb.match.halftime-draft.<id>`)이다. 서버에 프롬프트 **읽기 API 가
  없어서**(`MatchDetail` 에 필드 부재 — #253 이 남긴 것과 같은 구멍) 되읽을 수 없다.
  ⇒ **기기 간 이어쓰기는 안 된다**(폰에서 적고 데스크탑에서 열면 칸이 비어 보인다). 다만 **후반 반영은
  된다** — 서버에는 저장돼 있다. 읽기가 열리면 이 층을 갈아끼운다.
- 서버가 **빈 문자열을 400** 으로 막아 저장했던 지시의 **삭제가 불가능**하다(덮어쓰기만). 감독시간이
  그 사실을 안내한다(`halftime-cleared-warning`). server-java 에 "빈 값 = 삭제" 를 요청 중.

### 구 규칙 (#169~#244, **폐기** — 이력)

- ~~3토글은 서로 독립(`stats`, `log`, `brief` 각각 boolean). 동시 on 허용.~~
- ~~켜진 토글이 시트의 탭이 된다. 0개면 시트 자체가 없다(= "기본은 경기장면만").~~
- ~~토글 상태를 `localStorage`(`hmb.stage.toggles`)에 기억.~~
- ~~토글은 `role="switch" aria-checked`.~~
- 데스크탑 도크(켜진 패널이 세로로 함께)는 그 전에 이미 폐기됐다 — §2 확정안 참조.
...
### 9.10 데스크탑 = 폰의 넓은 버전 (hero 결정, 2026-07-22)

hero 리뷰: **폰은 그대로 OK. 데스크탑 우측 도크(360px)는 폐기** — 무대가 왼쪽으로 쏠려 비대칭이었다
(도크 시절 무대 중심이 화면 중심에서 180px 어긋남). 확정 형태 = **무대 가운데 + 하단 시트(탭) + 하단 토글바**,
즉 폰과 같은 구조에 폭만 여유 있는 버전.

- CSS: 데스크탑 미디어쿼리에서 `grid-template-columns`(도크) 제거 → `minmax(0,1fr) auto` 행 배치.
  무대는 `justify-self/align-self: center` + 비율 유지(높이에 맞춰 폭 결정). 시트는 하단, `max-height: 42svh`
  (데스크탑은 **세로**가 무대 크기를 정하므로 시트가 화면을 삼키면 무대가 급격히 작아진다).
- 넓은 화면에서 값이 양 끝으로 흩어지지 않게 패널 내용 폭을 묶었다(통계 560px · 로그 720px · 시트 1040px, 전부 가운데).
- **회귀 가드**: `match-stage.spec.ts` 데스크탑 케이스가 이제 **무대 중심 = 뷰포트 중심(±4px)** 과
  **시트가 무대 아래·화면 폭**임을 단언한다(도크로 되돌아가면 실패한다).
- 실측(1280×800, 통계 on): 무대 548px 폭·중심 640 = 뷰포트 중심 640, 시트 y=411(무대 바로 아래)·폭 1280.

**이 과정에서 실캡처로 잡은 버그**: 점유율 막대가 **폭 0으로 접혀 아예 안 보였다**(`.bar` 는 스탯 행의
그리드용이라 flex 컨테이너인 점유율 행에서 `flex-grow` 가 없었다). 폰·데스크탑 양쪽에 있던 문제였고
캡처를 눈으로 보고서야 드러났다 — 좌표·수치만 봤으면 못 잡았다(§2-2 규율의 실증).

### 9.11 데스크탑 무대 1.5배 · 시트 고정 (hero 리뷰 2026-07-23)

hero 프리뷰 확인 후 2건:

**(1) 무대가 너무 작다 → 1.5배.** 데스크탑은 **세로**가 무대 크기를 정하므로(비율 유지), 시트를 줄인 만큼
무대가 커진다. 정보 시트를 `clamp(150px, 20svh, 260px)` 로 낮췄다 → 1280×800 실측 **무대 폭 548 → 820 (1.50배)**,
가운데 정렬 유지(중심 640 = 뷰포트 중심).

**(2) 로그가 쌓이며 시트가 자라 무대가 점점 작아진다 → 고정.** 원인은 데스크탑 시트가 `max-height` 만
두고 높이는 **콘텐츠가 정했다**는 것. 로그 라인이 늘수록 시트가 자라고 그만큼 무대가 줄었다(hero 관찰이 정확).
→ 시트에 **고정 `height`** 를 부여했다. 내용은 패널 안에서만 스크롤한다(최신 하단 유지).

- 높이는 탭 종류로만 갈린다(**콘텐츠와 무관**): 정보(통계·로그·후반지시) = 낮게(무대 우선),
  상태(감독·결과) = `clamp(260px, 40svh, 420px)`(교체·프롬프트·결과표를 실제로 조작하는 자리).
  판정은 순수 함수 `sheetHeight()` + 단위테스트.
- 고정 시트(≈160px)에 지표 6개가 스크롤 없이 들어가도록 **데스크탑 통계를 3열**로 바꾸고 여백을 조였다
  (QA 뷰어 HUD 와 같은 그리드 idiom).

**회귀 가드 추가**: `match-stage.spec.ts` g — 로그 라인이 실제로 늘어난 뒤에도 무대 bbox(폭·높이·y)가
**불변**임을 단언한다. 뮤테이션 확인: 옛 `max-height: 42svh` 로 되돌리면 이 테스트가 **실패**한다(무대 높이 변화 검출).

### 9.12 정보 시트 26svh (hero 최종 미세조정, 2026-07-23)

로그를 더 보이게 정보 시트를 `clamp(150px, 20svh, 260px)` → **`clamp(170px, 26svh, 340px)`**.
무대와 시트는 세로를 나눠 쓰므로 무대는 그만큼 줄어든다 — hero 가 트레이드오프를 수용한 조정이다.

| 항목 | 20svh | **26svh(확정)** |
|---|---|---|
| 시트 높이(1280×800) | 160px | **208px** |
| 무대 폭 | 820px (1.50×) | **746px (1.36×)** |
| 로그 보이는 줄 | 5~6줄 | **8줄** |
| 통계 6지표 | 3열, 여백 빠듯 | 3열, 여유 |
