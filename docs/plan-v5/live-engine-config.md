...
**재생 경로에는 이제 throw 가 하나도 없다**(독립검증 M-A 로 마지막 하나까지 정리). 초판은 런타임
비용 상한(`assertAffordable`)을 *"이 값이 러너를 재우는가는 시간이 지나도 답이 안 바뀐다"* 는 근거로
재생에 남겼는데 **그 전제가 거짓이었다**: 상한은 `matchMinutes × 60000 / msPerTick` 이고 `msPerTick`
은 **구조값**이라 오버레이가 아니라 배포된 base 에서 온다(Tier C 0.25초 물리로 가면 1000→250,
허용 상한이 900분→225분). 그러면 어제 통과한 오버레이가 오늘 진행 중 매치를 죽인다 = B2·B3 과 같은
구조다. 그래서 재생에서는 그 경로만 **버리고 base 값으로 되돌린다** — base 는 정의상 상한 안이라
러너를 재우는 위험은 그대로 막히고, **작성에서는 여전히 400** 이다(운영자가 지금 고칠 수 있는
유일한 시점).
...
> ③은 서버가 매치별로 통제하는 값이라 배포와 무관, ④는 **도달 가능하지만 작성 스모크가 막는다**
> (`{"matchMinutes":0}` — 구조 게이트는 통과시키고 2시드 스모크가 400 을 낸다. ⚠️ "작성 게이트가
> 막으니 재생은 안전하다"는 논거는 이 웨이브에서 **세 번 뒤집혔다**(B2·B3·M-A). 여기서만 참인
> 이유는 실사고에 base `msPerTick` 이 커지는 배포가 필요해서이고, 그건 구조 변경이라
> `config.version` 이 오른다. 무조건 참으로 만들려면 `ticksPerHalf === 0` 도 상한과 대칭으로
> 버리면 된다 — **머지 후 처리**(6차 m-A, 회귀 위험 0).
> **문제는 ①이다** — `SimStateSchema` 의 필수 필드는 엔진 열차가 늘려 왔고(#279 가 `plan`·`phase`·
> `intents` 를 한 번에 필수로 넣었다), 스키마만 넓히는 변경은 경기가 bit-identical 이라
> `config.version` 을 올릴 이유가 없다. 즉 **B3 가 "노브 삭제는 엔진 열차의 정상 활동"이라며 막은
> 것과 정확히 같은 논리**가 스키마 진화에도 적용되는데, 그 축에는 아무 장치가 없다.
> **이 웨이브가 만든 결함은 아니지만**(이 브랜치가 그 스키마에 넣은 것은 `.optional()` 필드 하나뿐),
> "throw 가 하나뿐"이라는 문장이 다음 사람에게 그 축을 안 보게 만든다. 별건 = **#395**.
...
- `T-R9` 런타임 비용 상한(`MAX_HALF_TICKS`) — 러너를 재우는 값은 **작성에서** 거부, 상식적 실험
  범위는 통과. ⚠️ 초판은 여기에 *"재생 경로에도 남아야 한다"* 고 썼는데 **`T-R13` 이 그것을 뒤집었다**
  (M-A — 그 근거가 거짓이었다). 계약의 정본은 `T-R13` 이다.
...
- `T-R13` 비용 상한은 **작성 400 / 재생 버림**, 그리고 버려도 **다른 노브는 살아남는다**. 독립검증 M-A.
...
- ⚠️ **`POST /config/validate` 는 무인증이고 최대 14.3초 걸린다**(6차 m-B). 실측: `matchMinutes`
  ...
  뒤에 선다**. `MAX_HALF_TICKS` 는 *박히는* 값을 막지 이 표면을 막지 않는다. 러너의 노출 등급은
