...
> 확정되지 않은 것(=hero 컨펌 대기)은 **§6 컨펌 항목**에 모아 뒀다.
...
메시지함이 새로 만드는 것은 **"보낼 것과 받을 것"의 상태 2개(테이블)와 그 전이**뿐이다.
...
- **멱등**: 같은 `Idempotency-Key` 재전송 → **200** `{applied:false, campaignId, targetCount}`(아무것도
  ...
  비교 대상 = `audience` · 대상 목록 · `payload` · `expiresAt`. `title`/`body`/`reason` 은 비교하지
  않는다(오타 수정 재전송을 막게 된다 — 돈을 움직이는 필드가 아니다).
...
- **만료된 미수령은 목록에서 제외한다.** 행은 남는다(감사·CS 대응). 근거: 수신함은 할 일 목록이고,
  이제 아무것도 할 수 없는 항목은 "받을 수 있는 줄 알았는데?" CS 만 만든다. **수령한 메일은
  목록에 남는다**(만료돼도) — 무엇을 받았는지는 유저가 되짚을 수 있어야 한다.
...
- **홈 헤더 뱃지 전용 조회는 만들지 않는다** — `GET /api/me` 응답에 `mail:{unread:n}` 한 필드를
  더한다. 홈이 이미 부르는 유일한 호출이라 왕복이 늘지 않는다(인덱스 COUNT 1회).
...
## 5. 화면 위치 — 2안 (hero 컨펌 대상)

목업 = `docs/plan-v5/mock/mailbox/index.html` (로컬, 390px). 두 안이 **같은 목록·상세 화면**을
쓰고 **진입점만** 다르다.

| | **A. 홈 헤더 봉투 아이콘**(main 권장) | **B. 내 정보 탭 안** |
|---|---|---|
| 진입 | 홈 헤더, 닉네임 옆 ✉️ + 숫자 뱃지 | 하단탭 [내 정보] → 목록 안 "메시지함" 행 |
...
**main 권장 = A**, 근거: 첨부가 **만료되는 자산**이라 발견성이 곧 손해와 직결된다. 공지(놓쳐도
손해 없음)와 다른 축이다. 비용은 아이콘 한 개이고, 그 자리(닉네임 옆)의 헤더 증가는 이미 실측으로
+8px 이 확인된 자리다.

> A 를 택해도 **[내 정보]에 같은 진입점을 하나 더 두는 것은 자유**다(같은 화면으로 간다).
> 반대는 성립하지 않는다 — B 만 두면 홈에서 보상이 왔다는 사실 자체가 보이지 않는다.

---

## 6. hero 컨펌 항목 (§5 목업과 함께)

1. **위치 A / B** (권장 A).
2. **네이밍**: "메시지함" vs "우편함" (문서·목업은 **메시지함** + ✉️ 로 그림).
3. **만료 기본값**: 발송 시 미지정이면 **무기한**(권장) vs 기본 14일 강제.
4. **만료된 미수령을 목록에서 숨긴다**(권장) vs "만료됨" 회색 항목으로 남긴다.
