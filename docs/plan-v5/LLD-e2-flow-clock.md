...
>
> ⚠️ **정정(#416, 2026-08-03)**: 위의 *"남은 오차는 `paceRate` 가 흡수한다"* 는 **더 이상 사실이 아니다.**
> #365 이후 `paceRate` 는 **프로덕션 호출부가 0건인 휴면 함수**다(`VisualPlayback.tsx` = *"배율은
> 건드리지 않는다(고정 배속만)"*). 운영 창은 러너가 주는 `playbackMs` 라 흡수할 오차가 애초에 없고,
> 폴백(`half-real-ms`) 경로에는 **배율 보정이 없다** — 그래서 잘림이 시작되는 지점은 배율 상한 1.6 이
> 아니라 **1.0** 이다. 이 문장을 근거로 창을 줄이면 하프 끝이 잘린다(#416 이 실제로 그 오판을 한 번 했다).
...
되돌림은 드리프트 폭(하프의 12%)을 넘는 **의도적 점프에만**.
*(⚠️ #365 이후 `paceRate` 는 제품 경로에서 빠졌다 — 창이 그 하프의 실제 재생 길이라 흡수할 오차가
없다. 위 문장은 #216 시점의 기술이다. 상세 = 이 문서 §2 의 #416 정정.)* 서버 시계가 주는 값은 **스냅샷 인덱스**이고
