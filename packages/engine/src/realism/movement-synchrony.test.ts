 ...
 * (그 대조값은 비교 대상 노브가 01d959e 에서 삭제돼 **출하 도구로는 재현 불가** — 원 측정치와
 *  스윕 표는 이슈 #147 코멘트에 있다.)
 * ## 0.18.0(#178) 주의 — 이 골든이 **나빠진 것처럼 보이는 것은 아티팩트다**
 * 마크 진동(±3m 왕복) 픽스로 weightedR 0.756→0.773 · rigidPct 57.9→59.6 으로 "동조가 늘었다".
 * 진동은 인접 수비수끼리 방향이 제각각인 **반동조 노이즈**였으므로 그것을 제거하면 정렬도가
 * 올라간다 — 즉 개선된 개인 움직임이 사라진 게 아니라, 지표를 낮추던 버그가 사라진 것이다.
 * (관객이 본 것은 "개성 있는 움직임"이 아니라 제자리 진동이었다.) 진동 지표는 mark-jitter.test.ts.
 *
