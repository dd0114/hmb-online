 ...
 *
 * REALISM_SEEDS(20 시드 = 팀-경기 40 표본)는 팀당 슛의 표본표준편차가 ~5 라 **표준오차 ≈0.8**이다.
 * 밴드 경계(12–14, 실측이 13.4~14.0 으로 상단에 붙어 있음)와 인접 사다리 rung(0.30↔0.34)의
 * 실제 차이가 그 노이즈보다 작아, 코드가 안 바뀌어도 판정이 뒤집힌다(#182 에서 실증:
 * 같은 config 가 n=20 에서 14.65/비단조, n=40·60 에서 13.54/전구간 단조).
 * 가드는 이 시드로 재서 SE 를 절반으로 줄인다 — **밴드·단조성 기준 자체는 그대로**, 측정만 정밀하게.
 */
