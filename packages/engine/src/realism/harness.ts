...
/**
 * 회귀 가드용 확장 시드(40 = REALISM_SEEDS ×2). (#182)
 *
 * 쓰는 이유는 **사다리 단조성 하나** 때문이다. REALISM_SEEDS(20 시드 = 팀-경기 40 표본)는
 * 팀당 슛의 표본표준편차가 ~4-5 라 **표준오차 ≈0.7**인데, 사다리의 인접 rung 간격(0.30↔0.34)이
 * 그보다 작다 → 코드가 안 바뀌어도 순서가 뒤집힌다. #182 에서 실증: 같은 config 가
 * n=20 에서 `0.30→13.53, 0.34→13.28`(역전), n=40·60 에서는 전 구간 단조.
 *
 * 주의(사실관계): **밴드(12–14)는 n=20 에서도 통과한다**(실측 13.53). 즉 이 시드 상향은
 * 밴드 초과를 피하려는 게 아니라 사다리 플래키니스를 없애려는 것이다. 밴드·단조성 **기준
 * 자체는 하나도 바꾸지 않았고**, 측정 표본만 늘려 SE 를 줄인다.
 */
export const GUARD_SEEDS: string[] = [...REALISM_SEEDS, ...REALISM_SEEDS.map((s) => `7${s}`)];
