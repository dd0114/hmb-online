 ...
 *
 * ⚠️ **표본이 8시드인 이유**: 4시드에서는 이 사다리가 단조가 아니다(실측
 * 21.08 · 23.44 · 22.24 · 16.73 · 12.15 — 중간 두 칸이 뒤집힌다). 같은 사다리가 8시드에서는
 * 단조다. 라인 산포는 경기 전개에 민감한 양이라 4시드로는 인접 rung 을 분해하지 못한다 —
 * 아래 "보고" 블록의 20시드 주석과 같은 부류의 문제이고, 이 트랙이 반복해 걸린
 * **"두 점(또는 적은 표본)으로 추세를 읽는"** 함정이다.
 */
...
describe("S3-B L3 — 결과 지표 후보 4종은 **게이트가 아니다** (신호 없음을 박제한다)", () => {
  const ks = [0, 0.5, 1.0];
  const outs = ks.map((k) =>
    measureShapeOutcome(
      cfg((c) => {
  ...
  it(`오프사이드 라인 SD · 라인 이동 · 허용 슛 거리 · 라인 뚫림% 는 이 축의 함수가 아니다 — 라인SD ${outs
    .map((o) => o.offsideLineSdM.toFixed(2))
    .join("/")} · 이동 ${outs.map((o) => o.lineStepMeanM.toFixed(2)).join("/")} · 슛거리 ${outs
    .map((o) => o.concededShotDistP50M.toFixed(2))
    .join("/")} · 뚫림 ${outs.map((o) => o.behindLinePossPct.toFixed(2)).join("/")}`, () => {
    // ⚠️ **이 테스트는 값을 걸지 않는다.** 스코프 단계에서 선언한 후보 3종(라인SD·라인이동·
    // 허용슛거리)과 구현 중 추가한 1종(라인 뚫림%)을 전부 사다리로 재 봤고, 어느 것도 이 축에
    // 단조 반응하지 않았다(라인 뚫림%는 `refMode` 를 바꾸면 크게 움직여서 **라인 높이의 대리
    // 변수**임이 드러났다 = 응집이 아니라 높이를 재는 자다).
    //
    // 신호 없는 게이트에 계수를 맞추는 것이 트랙 D 의 함정 #4 라, **걸지 않고 보고만 한다**
    // (S3-A §2-2 가 위협 레인 점유를 같은 이유로 계약에서 뺀 선례). 이 테스트가 존재하는 이유는
    // 다음 사람이 "왜 결과 지표 계약이 없나"를 묻지 않게 하기 위해서다 — **찾아봤고 없었다.**
    expect(outs).toHaveLength(3);
    for (const o of outs) {
      expect(Number.isFinite(o.offsideLineSdM)).toBe(true);
      expect(Number.isFinite(o.behindLinePossPct)).toBe(true);
    }
  });
});
