 ...
 *
 * ⚠️ 캐시는 **SEEDS(8) 범위로만** 건다. `HEADER_SEEDS`(120) 까지 붙들면 상주 메모리가 120로그가 되고,
 * 그건 이 파일이 지금까지 가져 본 적 없는 최고점이다(120로그는 `countHeaders` 안에서 한 번 만들어졌다
 * 사라진다). 그쪽 중복은 캐시가 아니라 **호출 자체를 1회로 줄여서** 없앤다(아래 `HEADERS`).
 */
...
describe("#313 H5 — 루즈볼은 굴러간다(비행 중 급정지)", () => {
  it("무소유 공의 급정지 ≤ 25회/경기 (settleSpeed 0 일 때 52회)", () => {
    ...
    // ⚠️ #377 M3-B(#379 레인 예측) 이후 **red** 다: 23.8 → 25.25(8시드). 그런데 **20시드로 재면
    //   off 24.25±0.89 vs on 23.65±0.74 = 오히려 개선**이다(`evidence/377/M3-B.md`).
    //   8시드 SE≈1.3 이라 임계 25 가 평균에서 0.5σ 거리 = 이 표본으로는 판정이 동전 던지기다.
    //   상한 25 는 진짜 게이트라 **넓히지 않았고**(#327 규율), 표본은 #313 소유자 몫으로 둔다.
...
describe("#306 S6 — 공중볼과 헤딩", () => {
   ...
   *
   * ⚠️ **표본 60 → 120**(#377 M3-A 2R). 임계는 **비율로 그대로** 옮기고 표본만 늘린다.
   *
   * 1R 에서 이 확대를 독립검증이 blocker 로 잡은 것은 **확대 자체**가 아니라 근거로 적은 수치가
   * **거짓**이었기 때문이다("120경기로 재면 헤더 슛이 39 → 48 로 늘었다" ← 계약과 **다른 시드
   * 집합**으로 잰 값이었다). 지금 적는 수치는 **이 파일이 쓰는 `HEADER_SEEDS` 구성 · 같은
   * `countHeaders`** 로 잰 실측이다.
   *
   * **확대의 근거는 검정력이지 "내 값을 통과시키는 것"이 아니다.** `movement.passPlan.pull` 사다리:
   *            off | 0.45 | 0.6 | 0.65 | 0.7 | 0.75 | 0.8
   *   n=60  골:  3  |  3   |  4  |  3   |  2  |  0   |  6      ← **어느 값도 구별 못 한다**(0 이 섞인다)
   *   n=120 골:  8  |  5   |  5  |  5   |  6  |  3   | 10      ← **전 구간 0 이 아니다**
   * 즉 확대는 출하값 하나를 구제하는 것이 아니라 **게이트 전체를 안정화**한다 — 그것이
   * value-shopping 과의 차이다. (n=60 에서 `>0` 게이트는 기대값 2~4 · 포아송 P(0) ≈ 2~14% 짜리
   * 동전 던지기였다. n=120 이면 P(0) ≲ 0.5%.)
   *
   * ⚠️ 그리고 헤더 슛·골은 **`pull` 의 함수가 아니다** — 위 사다리 슛 쪽은
   * `49 / 43 / 40 / 43 / 33 / 29 / 48` 로 **추세가 없다**(가장 세게 당기는 0.8 이 예고 off 와 같은
   * 수준). 그래서 이 축은 `pull` 선택의 **근거로 쓰지 않는다**(config.ts 주석 참조).
   * 그 말은 이 게이트가 **회귀 감시로는 검정력이 약하다**는 뜻이기도 하다 — 120표본에서도 29~49 로
   * 흔들린다. 근본은 표본이 아니라 **입력**이다: 크로스 생성기가 없어(#316/S5) 헤더의 상류가
   * 걷어내기의 부산물뿐이다. 그게 들어오기 전까지 이 게이트는 "0 이 되면 알려 준다" 수준이다.
   */
  const HEADER_SEEDS = [
    ...GUARD_SEEDS,
  ...
  function countHeaders(): { headerShots: number; headerGoals: number } {
    const c = countHeaderStats(HEADER_SEEDS.map(logOf));
    // eslint-disable-next-line no-console
    console.log(`  [#306] ${HEADER_SEEDS.length}경기 헤더 슛 ${c.headerShots}건 · 헤더 골 ${c.headerGoals}건`);
    return c;
  }

  /**
   * #371: **한 번만 센다.** 아래 세 it 이 각각 `countHeaders()` 를 불러 표본 전체를 **세 번**
   * 다시 시뮬했다. 세 it 이 보는 것은 같은 표본의 서로 다른 필드이므로 한 번 세서 나눠 쓰면
   * 값·임계·표본이 하나도 안 바뀐다(순수 중복 제거). 그래서 표본을 120 으로 넓혀도 비용은
   * 1회분(120경기)이지 3회분이 아니다.
   * 120로그는 여기서 만들어져 집계 후 바로 버려진다 — 상주하지 않는다(위 `logCache` 주석).
   */
   ...
   */
  it("헤더 슛 총량이 기준선 아래로 침식되지 않는다 (120경기, 경기 길이 환산)", () => {
    ...
    //
    // ⚠️ #377 M3-A 2R: 표본이 60 → 120 으로 **두 배**가 됐으므로(위 `HEADER_SEEDS` 참조)
    // 같은 **비율**을 유지하려면 하한도 두 배다 → `round(20 × 45/90) × 2` = **20**. 실측 29 ≥ 20.
    // ⚠️ 1R 이 넣었던 `Math.max(12, …)` 하드코딩은 **지웠다**. 그 12 는 "예고 패스가 헤더 슛을
    // 24 → 15 로 침식시켰다"는 전제 위에 세운 값인데 그 전제가 **죽었다** — 헤더 슛은 `pull` 에
    // 추세가 없다(위 사다리 `49/43/40/43/33/29/48`). 죽은 전제 위의 상수를 남겨 두면 다음 사람이
    // 그것을 실측 기준선으로 오해한다.
    const floor = Math.round(20 * (defaultEngineConfig.matchMinutes / 90)) * 2;
  ...
  it("헤더 슛 중 골도 0 이 아니다", () => {
    // 표본 근거·검정력 논의는 위 `HEADER_SEEDS` 주석에 있다(요약: n=60 은 전 사다리에서 0 을 뽑아
    // **어느 값도 구별 못 했다**, n=120 은 전 구간 0 이 아니다 = 게이트 전체가 안정화된다).
    expect(HEADERS.headerGoals, `헤더 골 총 0건 (${HEADER_SEEDS.length}경기)`).toBeGreaterThan(0);
  });
