...
describe("#314 ⓐ 걷어내기", () => {
  it(`걷어내기가 팀당 5–12.5회/경기 나온다 (현재 ${cur.clearances.toFixed(2)})`, () => {
    ...
    //   레인 예측 세기(rung = `vision.laneRead.{pull, maxStepM}` 동시 배율)를 흔들어도 방향이
    //   따라오지 않는다(= 그 축의 함수가 아니다). **이 파일의 표본(n16)과 60시드가 서로 다른
    //   그림을 준다는 것 자체가 검정력 문제의 증거다** — 그래서 둘을 같이 적는다:
    //
    //     rung        n16     n60
    //     off        5.31    5.17±0.21
    //     0.5x       4.69    5.02±0.20
    ...
    //     4x         4.84    5.05±0.21
    //
    //   n16 은 0.5x 에서 최저였다가 2x 에서 최고로 되올라가고, n60 은 1x 가 최저다 — 두 표본이
    //   극점 위치조차 공유하지 않는다(단조도 아니고 부호도 안정적이지 않다).
    //   게다가 **기준선 자체가 밴드 바닥(5.0)에서 0.8 SE** 위다(16시드 SE≈0.40). 임계가 아니라
    //   **검정력**이 문제이므로 #314 소유자가 표본/관찰량을 정해야 한다 — M3-B 는 안 건드렸다.
    //   (재표본 이관 = **#399**. 이 파일의 임계·표본은 그 이슈가 소유한다.)
