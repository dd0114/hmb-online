...
describe("#353 롤백 등가 — 노브를 되돌리면 0.27.0 동작 규칙으로 돌아간다(해시 박제)", () => {
  ...
  const GOLDEN: Record<string, string> = {
    ...
    // #377 M3-B → dbb362ab · e8563fac · (seed3 불변).
    // #377 S3-B(engine@0.39.0) 재기록 — 레거시 노브 셋은 **hold 압박 규칙만** 되돌린다.
    //   공유 수비 라인·오픈플레이 레스트디펜스는 그 스위치 밖에서 계속 돌므로 여기 값이 움직인다.
    //   계약 내용은 그대로다(조용한 드리프트 금지).
    // #407 N4(engine@0.41.0) 재기록 — 1대1 예외는 hold **EV** 의 항이고 레거시 노브 셋
    //   (`keepBase=1` 등)이 되돌리는 것은 hold 의 **압박 반응**이라 서로 사거리가 다르다.
    //   이 계층의 스위치는 `chain.hold.oneOnOnePenalty=0` 이다. 계약 내용은 그대로다.
    //   0.40.0 값 = 0e24b839 · 4b54a605 · ec7b86bf → **뒤 두 개가 움직였다**(첫 매치에서는
    //   1대1 판정이 한 번도 안 걸렸다).
    // #407 ⑦(engine@0.42.0) 재기록 — `rules.offside.callProb` 0.013 → 0.045 는 **심판 판정**이라
