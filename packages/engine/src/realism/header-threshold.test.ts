...
describe("#357 헤더 임계 분리 — 필드 임계가 공중 경로를 끄지 않는다", () => {
  ...
  it("③ 두 임계가 같으면 분리 이전과 동일한 동작(롤백 경로) — 최종 해시 비트 동일", () => {
    ...
    // #371: 여기 있던 `expect(hashes(0.12)).toEqual(shared)` 는 **같은 입력을 두 번 돌려 자기와
    // 비교**하는 8경기(≈4초)짜리 no-op 이었다(같은 헬퍼·같은 인자라 정의상 항상 통과). 재현성
    // 자체는 `determinism.test.ts` 가 80회로 지키는 계약이므로 여기서 중복으로 살 이유가 없다.
    // 이 it 이 실제로 거는 판정은 **아래 split ≠ shared** 이고 그건 그대로다.
