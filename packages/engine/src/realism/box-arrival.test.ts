...
describe("#407 N2 게이트 — 조건 밖에서는 런을 한 건도 발행하지 않는다", () => {
  ...
  it("중앙 출신은 후보가 아니다 — **밖에 서 있던 사람만** 안으로 들어온다", () => {
    // ⚠️ 이 줄이 없으면 기제가 자기 발등을 찍는다: 이미 중앙인 미드필더(LCM, base 횡편차 12.2m)가
    // 전진도에서 앞서 먼저 뽑히고, 그러면 **박스 앞 슈팅 자리에 있던 비ST 슈터를 박스 안으로
    // 치운다**. 20시드 짝 대조(정렬키·나머지 노브 고정) lat0 → lat16: 비ST 슛 **0.73 → 0.90** ·
    // 슛 HHI 0.914 → 0.891 · 박스ST% 82.7 → 80.7 — 즉 이 줄이 **지키는** 방향이다.
    // ⚠️ 초판 주석의 "0.82 → 0.43" 은 **오귀속이라 철회했다**(#407 독립 검증 blocker-1) — 그
    // 0.43 은 `holdTicks` 24·유클리드 정렬 팔의 값이고 `minBaseLatM` 단독 효과가 아니다.
    // 이 줄과 아래 `describe B` 의 `minBaseLatM` 대조 주석은 **같은 수치**여야 한다
    // (한 파일 안에 폐기본과 정정본이 공존하던 것이 blocker-1 의 실체다).
    // 4-3-3 base 횡편차: LW/RW·LB/RB 20.4m · LCM/RCM 12.2m · CB 6.8m · CM/ST 0
    // → 출하 문턱 16m 은 윙어·풀백만 통과시킨다.
    const { state } = synthState(F3_X, true);
...
describe.skipIf(!LADDER)(`#407 N2 사다리 — 정원이 박스 인구를 정한다 ${LADDER_TAG}`, () => {
  it("maxRunners 0(=off) → 1 → 2 에서 **박스 안 비ST 인구**가 단조 증가한다", () => {
     ...
     *   **이 기제의 노브들은 앞항을 실질적으로 못 민다.** 통제하는 것은 러너가 박스에
     *   **서 있는 시간·인원**뿐이고, 출하 팔(h12 r2)이 만드는 인구 증가분은 게이트틱당 **+0.27**
     *   (0.46 → 0.734, **n20 `e407-boxpop`**)에 그쳐 유입은 5.13 → 5.83(**n20 `e407-diversity`**)
     *   으로 거의 안 움직인다. ⚠️ 두 수는 **프로브도 단위도 다르다**(게이트틱당 인원 ↔ 팀-경기당
     *   수신) — 한 줄에 쓸 때는 반드시 출처를 명기한다. 뒷항은 도착 지점 xG 가
     *   큰 쪽이 이기는 경쟁이라 **ST 가 구조적으로 이긴다**. 즉 수신은 이 노브들의 **하류**이고
     *   그 사이에 **상류 배급 결정**이 끼어 있어 노브를 더 밀어도 평평해진다
     *   (n60 h12 1.04 · h16 1.22 · h20 1.08).
     *   (대안 가설 "수비가 러너를 예측해 막는다"는 아블레이션으로 **배제**: `runReadFrac=0`
     *    에서도 수신이 안 오른다.)
     *
     *   ⚠️ **초판이 여기 적었던 "유입 5~6/팀-경기 고정 · ST 박스수신 4.5~5.2 상수"는
     *   반증됐다**(#407 독립 검증 blocker-2). 아래는 전부 **n20 `e407-diversity`, 출하 커밋
     *   `41d5981` 콜드**(BASE = `boxArrival.enabled=false`): 인구를 크게 올리는 config 팔에서는
     *   유입이 **2.5~3.9배로 따라온다** — `variety.defenderOverlapProb .4`+`overlapBaseLine .8`
     *   총량 **12.75** · ST **10.38** · 팀 폭 **48.0(밴드 안)** / 전역 `attackWidthReach −0.10`
     *   총량 **19.95** · ST **11.95** · 폭 37.1(BASE 는 5.13 · 4.58 · 46.8). **유입은 구조적
     *   상수가 아니다** — 그 팔들이 못 쓰이는 이유는 유입이 아니라 1대1 11.45 · 박스슛 64.7 ·
     *   파울 2.67 등 **다른 밴드의 붕괴**다. 어느 팔에서도 안 변하는 것은 **"누가 받고 누가
     *   쏘는가"** 뿐이다(overlap 팔에서 유입이 2.5배가 돼도 박스ST **81.4%** · 슛top1 **94.0%** ·
     *   골top1 **93.7%** ↔ BASE 89.3 / 95.1 / 95.4).
     *   ⚠️ 이 계약이 자를 옮긴 근거는 **범위 한정된 쪽**(= N2 노브의 증가분으로는 유입이 안
     *   움직인다)이고 그 사실은 여전히 참이다. "유입은 못 만든다"로 읽으면 **다음 웨이브가
     *   M 규모 배급 생성기로 오스코핑된다** — 상세 = 노트 §3-5.
     *
     *   반면 **인구는 이 노브의 정의 그 자체**다 — 러너가 박스에 서 있는 틱 수. 배급이 개입하지
     *   않으므로 병목이 없다. 노브가 죽으면 여기가 **먼저** 무너진다.
     *
     * ## ⚠️ 왜 `maxRunners` 이고 왜 rung 이 셋뿐인가 (간격 > SE 규율)
     * 두 노브 다 **포화한다**. **n20 `e407-boxpop`** 인구 실측(단위 = 게이트틱당 비ST 인원):
     *   `holdTicks` 0 **0.460** → 8 **0.680** → 12 0.734 → 16 **0.723**  ← 8 위로 SE 아래·역전
     *   `maxRunners` 0 **0.46** → 1 **0.60** → 2 **0.73** → 3 0.729 → 4 0.804 ← 2 위로 평평/역전
     * (⚠️ `holdTicks` 행은 수습 커밋에서 콜드 재측정했다 — 초판의 `0 → 0.471` 은 **12시드 사다리
     *  harness** 의 rung 0 값이라 n20 프로브 행에 섞이면 안 된다. `maxRunners` rung 3·4 는
     *  탐색기 값이고 계약이 쓰는 구간이 아니라 재측정하지 않았다. **계약이 무는 0/1/2 삼각형은
     *  아래 `arm()` 이 매 실행 직접 재고 로그로 찍는다** — 그게 이 절의 권위 수치다.)
     * `holdTicks` 8·12·16 이나 `maxRunners` 2·3·4 를 rung 으로 쓰면 **SE 아래 간격에 계약을
     * 거는 것**이고 그게 정확히
