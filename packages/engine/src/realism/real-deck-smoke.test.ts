 ...
 * 인데 **슛은 1:0** 이었다. 그래서 슛 하한 래칫을 **같이** 둔다(아래).
 *
...
describe("T0 · 붕괴 케이스 1경기 — 하프가 죽지 않았는가", () => {
   ...
   *
   * **절대 하한을 의도적으로 낮게(3) 잡았다.** 실측은 0.07 에서 이 시드가 `home 7 · away 8`
   * (5시드 약팀 6~8)이고 붕괴값 0.197 에서는 `1:0` 이었다 — 즉 3 은 정상 대비 절반 이하라
   * 계수 튜닝의 통상 진폭에 걸리지 않고, 붕괴는 확실히 잡는다. **밴드가 아니라 하한**인 이유는
   * T0 이 매 커밋 게이트라서다: 상한까지 걸면 튜닝 웨이브마다 red 가 되어 신호가 죽는다.
   * 볼륨 밴드는 T1(`shot-frequency.test.ts`)이 본다.
   *
   * 대조군(벤치마크 평균) 대비 관계식이 더 좋겠지만 그건 경기 5판을 더 돌려야 해서
   * T0 ≤1분 예산과 충돌한다 — 관계식 판정은 T1 전량 스캔이 갖고 있다.
   */
  it("양 팀이 슛을 만든다 — #370 부류의 슛 붕괴 하한(정상 7~8, 붕괴값에서는 1:0)", () => {
