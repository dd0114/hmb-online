...
describe("롤백 스위치 (#147 W3)", () => {
  ...
  // engine@0.26.0(#320 공 물리 속도벡터 재작성 + 그에 따른 볼륨 재보정 gv 24→11 · xgBase
  // 0.42→0.36)로 두 상수 모두 갱신. 공 물리는 시야 스위치와 무관한 **엔진 코어**라 vision-off
  // 경로도 당연히 함께 움직인다 — 위 #307/0.25.0 과 같은 성격의 정상 갱신이다.
  const ROLLBACK_HASH = "711682a2";
  ...
  // engine@0.26.0 #320 공 물리 속도벡터 재작성 + 볼륨 재보정 → **77fe81c1** (전부 위와 같은 이유).
  const ROLLBACK_HASH_MARKED = "77fe81c1";
