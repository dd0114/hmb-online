...
describe("롤백 스위치 (#147 W3)", () => {
  ...
  // engine@0.25.0 볼륨 재보정(chain.goalValue 12→24 · contest.xgBase 0.195→0.42 ·
  // onTargetBase 0.235→0.21 · controlRange 3.5→5.0)으로 두 상수 모두 갱신. 전부 **전역 노브**라
  // vision-off 경로의 경기 전개도 함께 움직인다 — 아래 "이 대조가 말하지 않는 것" 항목과 같은 성격의
  // 정상 갱신이다(스위치가 시야 계층만 끈다는 **의미**는 이 파일 아래 구조 계약이 따로 지킨다).
  // #314(행동·의도 계층: 걷어내기 신설 · 런 오더/의도 게시판 · 수비 블록 추종 · GK 스위퍼 라인)
  ...
  // 시야 롤백 경로도 함께 움직인다. 26781828 → 21c1b46f.
  // #377 M3-A(#369 예고 패스)도 같은 성격이다 — 예고 게시·읽기는 **오프더볼 목표**를 움직이는
  // 전역 변경이라 시야 롤백 경로(`vision.enabled=false`)에서도 똑같이 걸린다. 이 계층의 스위치는
  // 따로 있다(`movement.passPlan.enabled`). 21c1b46f → 아래 상수.
  // #377 M3-A **2R** 로 다시 이동한다. 두 가지가 겹쳤고 둘 다 롤백 스위치 밖이다: ①`pull`
  // 0.7 → 0.75(전역 오프더볼 목표) ②`hash.ts` 가 **데드볼 상태(`stoppage`·`setPiece`)를 흡수**
  // 하기 시작했다 = 해시 **공식**이 바뀐 경우다(#279 S1 과 같은 성격 — 동작이 아니라 관측 범위가
  // 넓어졌다). 5fdcb658 → 아래 상수.
  ...
  // #407 N4(engine@0.41.0) 재기록 — 같은 이유(롤백 스위치 밖의 전역 변경). hold EV 의 1대1 예외
  //   (`chain.hold.oneOnOnePenalty`)는 **볼 소유자 결정 코어** 변경이라 `vision.enabled=false`
  //   에서도 똑같이 걸린다. 이 계층의 스위치는 따로 있다(그 노브를 0 으로). b8b7834e → 아래 상수.
  // #407 ⑦(engine@0.42.0) — 오프사이드 호출 게이트(`rules.offside.callProb` 0.013 → 0.045)는
  //   **심판 판정**이라 `vision.enabled` 와 무관하게 돈다. 이 계층의 스위치는 그 노브 자체다
  //   (0 으로 두면 깃발이 하나도 안 오른다 — `realism/offside-call.test.ts`).
  //   ⚠️ **이 상수는 안 움직였다** — 이 매치의 기하 오프사이드 롤이 0.013~0.045 창을 한 번도
  //   안 밟았다는 뜻이다(#316/#239 때와 같은 부류의 기록). 아래 marked 는 움직였다.
  const ROLLBACK_HASH = "1d8a173d";
  ...
  //    "main 비트동등 복원"이 아니다.
  // #357(가치 역전 해소: chain.goalValue 9.4→22 · contest.shootXgThreshold 0.07→0.197 ·
  // onTargetBase 0.21→0.19)도 같은 성격이다 — 전부 **전역 볼륨 노브**라 vision-off 경로의
  // 전개도 함께 움직인다(81c322bb → b2f4a92b · b248b6d4 → 아래 상수).
  ...
  //
  // ⚠️ #316/#239 에서는 **두 상수 모두 안 움직였다** — 이 매치들에 1대1 판정도, 자기 박스의
  ...
  // #370 되돌림 → 3a11b097.
  // #377 M3-A(#369 예고 패스) → 347d54a5.
  // #377 M3-A **2R**(`pull` 0.75 복귀 + 해시가 데드볼 상태를 흡수) 재기록 — 위와 같은 이유.
  ...
  // ⚠️ #407 N4(engine@0.41.0) 에서는 **이 상수만 안 움직였다**(plain 은 움직였다). 그 매치에서
  //   1대1 판정이 한 번도 안 걸렸다는 뜻이다 — #316/#239 때와 같은 부류의 기록이라 남겨 둔다.
  // #407 ⑦(engine@0.42.0) 재기록 — 오프사이드 호출 게이트(위 ROLLBACK_HASH 와 같은 이유).
  //   ⚠️ **이번엔 marked 만 움직였다**(plain 은 불변) — #407 N4 때와 정확히 반대다. 두 변형이
  //   서로 다른 전개를 타므로 창에 걸리는 롤도 다르다. 7d1ea7a7 → 아래 상수.
  const ROLLBACK_HASH_MARKED = "348a3356";
