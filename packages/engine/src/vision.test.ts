...
describe("롤백 스위치 (#147 W3)", () => {
  ...
  const ROLLBACK_HASH = "618e0a35"; // #313/#306/#312 공 물리 재작성 — 아래 재기준 사유 참조.
  ...
  //    "main 비트동등 복원"이 아니다.
  const ROLLBACK_HASH_MARKED = "1c13ba12"; // #279 사슬 채택 → #313/#306/#312 공 물리 재작성으로 재기준.

  // 재기준 사유(#313 루즈볼 굴림 · #306 공중볼 · #312 세기/정확도, 2026-07-30):
  // 이 세 변경은 **롤백 스위치가 끄는 층(시야)과 무관한 전역 물리**다 — 공이 굴러가고(settleSpeed),
  // 패스 세기가 선수마다 다르고(passSpeedMin/Max), 조준에 오차가 있고, 띄운 공은 헤딩 경합을 한다.
  // 게다가 `hashState` 가 비행 상태(kind·delivery·speed·목표)를 새로 흡수한다(§5-6). 그래서
  // vision-off 경로의 해시도 같이 움직인다. 이 상수의 목적은 "레거시와 같다"가 아니라
  // **"롤백 경로가 조용히 드리프트하지 않는다"** 이므로(#181 재정의) 정당한 재기준이다.
