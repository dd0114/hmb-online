...
export const defaultEngineConfig: EngineConfig = {
  ...
  movement: {
    passPlan: {
      ...
      // 하는가"** 의 문제였다(§2.5 새 노브 레지스트리 기준).
      //  ① 초판 0.45 — "런 오더보다 약하게"라는 *말*에서 고른 값이다. 변이체 킬(끄면 경기가
      //     달라진다)은 green 인데 **읽은 리시버가 도착 예정 지점에서 오히려 멀어지고 있었다**.
      //  ② 0.75 — 효과는 났지만 **헤딩을 침식**했다(아래 표). 헤더 골이 출하 표본에서 0 이 됐고,
      //     그걸 표본 확대로 넘기려던 것을 독립검증이 blocker 로 잡았다(정당한 지적이다).
      //
      // 세 축을 같이 봐야 한다. 세게 당길수록 ⓐ예고 효과는 커지고 ⓑ목표가 튀어 제자리 진동
      // (#178/#185)이 늘고 ⓒ패스가 잘 이어져 **공중볼이 줄어 헤딩이 마른다**.
      ...
      //   (예고 off = 24 / 3. 즉 헤딩 침식은 0.7 에서도 있다 — 아래 참조.)
      // → **0.7** = 세 축을 동시에 만족하는 유일한 값. 관찰량을 반사실 팔(`readBase=1`)이 아니라
      //   **출하 config 안의 READ vs UNREAD 대조**로 바꾼 것이 이 선택을 가능하게 했다(독립검증 m1).
      //
      // ⚠️ **선언된 회귀**: 헤더 슛이 60경기 24 → 15(−37%) 로 준다. 이 웨이브가 원인이고
      // (`passPlan.enabled=false` 는 직전 커밋과 비트 동일), 원인은 "패스가 잘 이어져 걷어내기·
      // 롱볼 경합이 준 것"이다. 숨기지 않고 `ball-physics.test.ts` 의 헤더 슛 래칫을 **조이는 것**
      // 으로 다음 침식을 잡는다(10 → 12). 근본 해소는 #316/S5(크로스 생성기 부재) 소관이다.
      enabled: true,
      readBase: 0.35,
      readAttrSwing: 0.3,
      pull: 0.7,
