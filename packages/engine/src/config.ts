...
export interface EngineConfig {
  ...
  rules: {
    ...
    offside: {
      ...
      toleranceM: number;
      /**
       * offsideTrap on 인 수비팀에 대해 **심판이 쓰는 판정선만** 이만큼(m) 상향한다.
       *
       * ⚠️ **출하 0 이다(#377 S3-C).** 0.39.0 까지 이 값(2.5)이 트랩의 **유일한** 구현이었다 —
       * 선수는 한 명도 안 움직이고 심판만 편향됐다. 이제 기하가 `movement.defLine.trap` 으로
       * **물리적으로** 움직이므로, 여기에 값을 더하면 **이중 계상**이다.
       *
       * ⚠️ **0 보다 크면 잠복 결함이 되살아난다** — 이 보정을 적용하는 곳은 `checkOffside`
       * **하나뿐**이고, 같은 라인을 쓰는 다른 소비자들(`through.ts:throughPassOptions` 의 온사이드
       * 게이트 · `chain.ts` 진단)은 **보정 없는** `offsideLineProg` 를 쓴다. 즉 값이 0 이 아니면
       * **패스 생성기는 "온사이드"라 믿고 찌른 공을 심판이 깃발로 잡는다**(불일치 = 이 값 그대로).
       * `offsideLineProg` 가 "손복사본 금지 · 단일 출처"로 만들어진 함수인데 소비처가 갈라져 있다.
       * 이 성질은 `realism/offside-trap.test.ts` T9 가 계약으로 박제한다 — 다음 사람이 이 노브를
       * 올리면 거기서 걸린다.
       *
       * 노브를 **지우지는 않는다**: `trap.enabled=false` + 이 값 2.5 = 0.39.0 재현(아블레이션 팔).
       */
      trapBiasM: number;
      /**
  ...
  chain: {
    ...
    mode: "weighted" | "chain";
    /**
     * `passDirectness`(#361 T1) 배수를 롱 옵션 EV 에 적용하는가. false = 배선 이전 동작.
     * 나머지 M2 배선 3종은 각자 스위치가 있는데(`press.trigger.enabled` · `duty.enabled` ·
     * `fatigue.recoveryEnabled`) 이것만 없어서 **"배선 전부 off"를 config 로 재현할 수 없었다**
     * (독립검증 m3). 아블레이션의 한 팔이 재현 불가면 귀속 분해가 검증 불가가 된다.
     */
    passDirectnessEnabled: boolean;
    ...
    holdPenalty: number;
    /**
     * **슛의 거리 게이트를 "후보 삭제"에서 "결정단 감쇠"로** (#407 N1, 0.41.0).
     *
     * ## 무엇을 대체하나
     * `chain.ts` 의 슛 생성기는 `distToGoalM > contest.shootRange` 면 후보를 **아예 안 만들었다**.
     * 그 하드 컷이 두 가지를 동시에 했고 둘 다 대가가 컸다:
     *  - **와이드 슛을 원리적으로 0 으로 만든다.** 와이드 레인 문턱은 피치폭 68÷5×1.5 = **20.4m**
     *    (횡오프셋)인데 골 중앙까지의 유클리드 거리는 횡오프셋보다 항상 크거나 같으므로,
     *    `shootRange`(19) < 20.4 이면 와이드 슛의 확률은 **정확히 0** 이다. 튜닝이 아니라 산수다.
     *  - **볼륨을 그 노브로 내리면 박스 편중이 폭증한다.** 박스 밖 슛이 통째로 사라져서다
     *    (Phase 2-A 3안: 박스 안 슛 41.2% → 89.9/99.7/100%, hero 기각).
     *
     * 그래서 생성은 **넓게**(안전 상한 `genMaxM`) 하고, 억제는 `candidateEv` 의 shoot 분기에서만
     * 한다. 먼 슛은 **EV 경쟁에서 지는 것이지 존재하지 않는 것이 아니다.**
     *
     * ## ⚠️ `xgAtPoint` 는 건드리지 않는다 (이 설계의 핵심)
     * Phase 2-A 는 `contest.shootDistanceFactor`(= xG 자체의 거리 계수)를 키워 같은 일을 하려다
     * **슛당 xG 0.117 · 전환 10.7% · 스로인 24.5** 로 기각당했다. xG 는 "이 슛이 들어갈 확률"이라는
     * 결과 모델이고 여기 감쇠는 "이 선수가 이 슛을 고를 만한가"라는 **결정 모델**이다. 섞지 말 것.
     * 계약 = `realism/shot-distance-decay.test.ts`("감쇠 노브를 돌려도 같은 지점 xG 가 bit-identical").
     *
     * ## `contest.shootRange`/`shootXgThreshold` 는 은퇴하지 않았다
     * 두 노브는 **다른 소비처에서 계속 산다**: `shootRange` = 1대1 부스트 자격(`oneOnOneShot`) ·
     * weighted 롤백 코어의 슛 가중 · 1대1 진단(`realism/one-on-one.ts`). `shootXgThreshold` =
     * **질 게이트로 생성기에 그대로 남는다**(저xG 슛을 걷어내는 역할은 거리와 다른 축이고,
     * 그 사다리가 `shot-frequency.test.ts` 의 계약이다). 이 블록이 대체한 것은 `chain.ts` 슛
     * 생성기의 **거리 컷 한 줄**뿐이다.
     */
    shootDistance: {
      /**
       * false = 0.40.0 하드 게이트(`distToGoalM > contest.shootRange` 면 후보 없음) 그대로.
       * **롤백 스위치** — `hold.oneOnOnePenalty=0` 과 같이 끄면 0.40.0 과 bit-identical 이고,
       * 그 해시가 계약으로 박제돼 있다.
       */
      enabled: boolean;
      /**
       * 후보 **생성**의 안전 상한(m). 게이트를 완전히 없애면 사슬 탐색 후보 수가 늘어 노드 예산
       * 컷오프 지점이 달라지고 비용도 는다 — 그래서 "레버"가 아니라 **넉넉한 상한**으로 둔다
       * (`contest.shootRange` 보다 충분히 크게). 여기서 볼륨을 잡으려 하면 그건 하드 게이트의 재발이다.
       */
      genMaxM: number;
      /** 이 거리(m)까지는 감쇠 없음(계수 1.0). 박스 근처 슛의 결정은 0.40.0 과 같게 둔다. */
      freeM: number;
      /**
       * `freeM` 초과 1m 당 슛 EV 의 **골 항** 감쇠(선형). **이것이 볼륨 레버다** — 값↑ → 팀당 슛↓.
       * 사다리 계약 = `shot-frequency.test.ts`(HMB_LADDER).
       */
      perM: number;
      /**
       * 감쇠 계수의 하한. 0 으로 두면 아주 먼 슛이 EV 0 이 되어 **하드 게이트와 같아진다** —
       * 이 값이 "먼 슛이 선택지에서 사라지지는 않는다"를 지킨다.
       */
      floor: number;
    };
    /**
    ...
    hold: {
      ...
      minKeep: number;
      /**
       * **"확실한 슛" 예외**(#407 N4, 0.41.0) — 홀더가 1대1(`decision.oneOnOneShot` 이
       * `detail="one_on_one"` 을 낸 상태)이면 hold EV 의 유지 항에서 이만큼을 **더** 뺀다.
       *
       * ## 왜 필요한가
       * `p_keep` 은 압박에 반응하므로 **자유로울수록 hold 가 최적**이다. 그런데 1대1 은 정의상
       * "반경 안에 비-GK 상대가 없다" = `p_keep` 최대다. 그래서 hold 계열 노브(`holdPenalty`)를
       * 볼륨 레버로 쓰면 **#316 의 1대1 이 먼저 죽는다** — Phase 2-A 안 A(`holdPenalty` −2.0)에서
       * `one_on_one` 이 팀-경기의 95.8%에서 **0건**이 됐고 그것이 그 안의 기각 사유였다.
       *
       * ## 왜 곱이 아니라 뺄셈인가
       * hold EV 의 유지 항 `V(here) − holdPenalty` 는 음수가 될 수 있다. 계수(<1)를 곱하면 음수는
       * **0 쪽으로 올라가** 부호가 뒤집힌 보정이 된다. `holdPenalty` 와 같은 자에 놓인 **가산
       * 페널티**여야 항상 같은 방향으로 작동한다.
       *
       * ## 1대1 판정은 재발명하지 않는다
       * `decision.oneOnOneShot`(=`contest.oneOnOneClearM`/`oneOnOneXgMult`/`shootRange`)이 SoT 고,
       * `chain.ts` 는 **루트에서 이미 부른 그 결과**를 쓴다(hold 후보는 루트에서만 생성되므로
       * 가상 도착 지점에서 1v1 기하를 재는 일이 없다 — #316 이 기각한 함정을 그대로 피한다).
       * 0 = 예외 없음(0.40.0 동작) = **롤백 스위치**.
       */
      oneOnOnePenalty: number;
    };
    /**
     * **스루패스 — 공간 타깃 패스 후보**(#377 트랙 D M3-C, W0 §2-C).
     *
     * ## 무엇이 없었나
     * `passOptions` 는 동료 **개체**만 후보로 낸다 → 조준점이 언제나 리시버의 (미래) 발밑이라
     * 리드 거리 p50 이 **3.48m** 였다(스루패스는 10~25m). `ActionCandidate` 는 S2(#279)부터
     * 좌표가 1급이라 표현은 가능했는데 **그 좌표를 만드는 생성기가 없었다.**
     *
     * ## EV 식은 안 바뀐다
     * 공간 후보도 `p × V(도달 상태) + (1−p) × V(턴오버)` 그대로 평가된다. 새로 생긴 것은 `p` 안의
     * **경주** 뿐이다 — 러너가 먼저 닿나, 수비가 먼저 닿나(`raceBase`/`raceGainPerTick`).
     * 그래서 별도의 "스루패스 가중치" 노브가 없다(있으면 EV 와 이중 계상이 된다).
     *
     * ## 왜 M3-A 뒤인가
     * 러너가 서 있으면 언제나 수비가 먼저 닿아 EV 가 낮고 후보가 **한 번도 안 뽑힌다**(W0 §3 이
     * "C 먼저"를 기각한 근거). 예고 패스(#369)가 리시버를 **미리 출발**시켜 놓았기 때문에 성립한다.
     *
     * ⚠️ 계수는 **러프 기본값**이다(#377 방침). 총량 밴드 재보정은 **트랙 T** 소관이다.
     */
    throughPass: {
      /** false = 공간 타깃 후보 없음(#377 M3-C 이전 동작). 롤백 스위치·변이체 킬 대조군. */
      enabled: boolean;
      /**
       * 조준점을 상대 **오프사이드 라인 뒤** 몇 m 에 두나. 라인 정의는 `checkOffside` 와 같은
       * 자(뒤에서 2번째 상대)를 쓴다 — 다르면 "찔렀는데 깃발이 오른다"가 오차만큼 상시 발생한다.
       */
      behindLineM: number;
      /** 러너 판정: 그 동료의 **목표**가 현재 위치보다 골 쪽으로 이만큼 앞서 있어야 한다(m). */
      minRunGainM: number;
      /** 리드 거리(러너 → 조준점) 하한(m). AC 의 "10~25m 구간"이 이 두 노브다. */
      minLeadM: number;
      /**
       * 리드 거리 상한(m). 넘으면 잘라서 조준하고, 그 결과가 라인 뒤가 아니면 후보를 버린다.
       *
       * ⚠️ **가드레일이 아니라 주 레버다.** 리드는 `clamp(러너 속도 × (공 도달 틱 + 계획 창),
       * minLeadM, maxLeadM)` 인데, 출하값에서 채택된 후보의 리드 실측(8시드 · 12건)이
       * `25.0 ×8 · 23.5 · 23.4 · 20.2 · 18.7` 로 **상한이 67%(8/12)를 차지한다** = 대부분의
       * 스루패스가 이 클램프에 **닿아 있다**. 그러므로 이 값을 만지면 조준점이 그대로 따라
       * 움직이고(리드 ↑ → 라인 뒤 판정 통과 ↑ · 경주 여유 ↓), 스루패스 빈도와 성공률이 같이
       * 바뀐다. 트랙 T 재보정에서 "상한이니 안전하다"고 두면 안 되는 노브다.
       */
      maxLeadM: number;
      /**
       * **보수성 노브**(hero 게임설계 항목 — W0 §5-3). 러너가 최근접 수비보다 **이만큼 틱 먼저**
       * 조준점에 닿아야 후보로 낸다.
       *
       * ⚠️ hero 회신이 없어 **보수(성공 가능성이 높을 때만 찌른다)** 를 기본값으로 잡았다.
       * *"스루패스가 실패해 골킥이 되는 그림이 자주 보여도 괜찮다"* 로 정해지면 **이 값만**
       * 0 이나 음수로 내리면 된다 — 코드는 안 바뀐다.
       */
      minMarginTicks: number;
      /** 경주 계수의 절편 — `raceFrac = clamp(raceBase + raceGainPerTick × 여유틱, 0, 1)`. */
      raceBase: number;
      /** 여유 1틱당 경주 계수 증가분. 여유가 클수록 성공확률이 오른다. */
      raceGainPerTick: number;
    };
  ...
  /**
   * `duty`(밸런스 / 공격 가담 / 수비 안정 / 연결고리) 배수. (#366 T5)
   *
   * UI 셀렉트 → `tactical-patch` → `SimPlayer.duty` 까지 전부 배선돼 있는데 **읽는 코드가 0건**
   * 이었다 = 유저가 고른 열거형이 무효. 결판은 **(a) 배선한다**(#366 권장 — 이미 노출된 UI 라
   * 빼는 비용이 더 크다).
   *
   * ⚠️ **자연어 우회와 이중 계상된다.** 같은 지시가 프롬프트 문장으로도 합성돼
   * `forwardRunFreq`·`supportDepth` 를 올린다. 그래서 배수를 일부러 **얕게** 잡는다 —
   * 1.0 에서 ±0.3 안쪽. 이 값들은 러프 기본값이고 정밀 조정은 트랙 T 소관이다.
   */
  duty: {
    /** false = 미배선(0.31.0 이전 동작) — 롤백 스위치·변이체 킬 대조군. */
    enabled: boolean;
    /** 전진 런 배수. */
    forwardRunMult: { defend: number; support: number; attack: number };
    /** 공 지원 당김 배수. */
    supportPullMult: { defend: number; support: number; attack: number };
  };

  /**
   * 압박(press) — 팀 지시가 실제로 걸리는 자리. (#361 T1)
   *
   * `pressingScheme.triggerLine` 은 계약·UI·프롬프트까지 전부 배선돼 있는데 **엔진 소비자가
   * 0건**이었다(#359/#360 진단). 즉 유저가 "하이프레스로 가자"를 골라도 경기가 **비트 단위로
   * 동일**했다. 여기서 게이트 하나를 열어 로우블록/하이프레스를 가른다.
   */
  press: {
    trigger: {
      /** false = 게이트 없음(0.31.0 이전 동작) — 롤백 스위치·변이체 킬 대조군. */
      enabled: boolean;
      /**
       * 게이트 여유(진행도). 트리거선 바로 위에서 압박 담당이 매 틱 붙었다 떨어졌다 하지 않게
       * 하는 완충이다(경계 근처 채터링 방지).
       */
      marginProgress: number;
    };

    /**
     * **압박 유닛**(#377 트랙 D S3-A · #350 · #362 · #303) — "한 명만 붙고 나머지는 구경"의 해소.
     *
     * ## 무엇이 없었나 (구조 사실)
     * 압박 담당은 **팀당 정확히 1명**이었다 — `assignPresser` = 트리거 게이트 + `closestToBall`
     * 이 전부고, "커버"라는 개념이 코드에 **자리가 없었다**. 나머지 수비수는 자기 마킹 대상과
     * 블록 위치로만 판단했다. 게다가 **위험도라는 축이 아예 없어서**, 하프라인에서 공을 잡은
     * 것과 우리 박스 앞에서 잡은 것이 수비 반응 면에서 **완전히 같은 코드 경로**였다
     * (hero 실관전 제보 #350 의 실체).
     *
     * ## ⚠️ 목표값 `볼 10m 안 수비수 ≥ 2.0` 의 근거 (임의 임계가 아니다)
     * 이 값이 게이트로 정당한 이유는 로드맵에 적혀 있어서가 아니라, **기존 노브로는 도달할 수
     * 없기 때문**이다. HEAD(`221c673`)에서 수비 블록 축을 사다리로 흔든 8시드 실측:
     *   `movement.defendCompactX` 0.16 → 0.32 → 0.60 → 0.90 에서
     *   볼 10m 안 수비수 = **1.322 → 1.405 → 1.578 → 1.523** (1.6 근처에서 **포화**)
     * 즉 2.0 에 도달했다면 그건 **새 기제 때문일 수밖에 없다** — 그래서 절대 임계가 자기충족이
     * 아니다. (같은 사다리에서 `ppda` 는 3.96/4.42/3.87/4.83 로 **추세가 없어** 게이트에서 뺐다.)
     *
     * ## 위험도 → 인원 매핑은 프리킥 벽의 관용구를 **그대로 재사용**한다
     * `setpiece.ts:wallCountFor` 가 이미 "위협거리 → 인원(near~far 선형)"을 한다(0.25.0).
     * 새 관용구를 만들지 않는다 — 이름·형태·클램프까지 같은 모양이다.
     *
     * ## `intensity` 는 이제 연속값이다 (#362)
     * 구 소비는 `pressAggression × intensity > 0.15` **불리언 문턱 하나**였고, 기본값
     * 0.5 × 0.55 = 0.275 에서 이미 참이라 **올리는 방향으로는 효과가 0** 이었다. 이제 두 축에
     * 연속으로 들어간다 — **인원**(`intensityCountGain`)과 **압박 사거리**(`intensityRangeGain`).
     * ⚠️ 불리언 문턱은 **지우지 않았다** — 유닛 밖(롤백 경로)에서 그대로 쓰인다.
     *
     * ## 세기 기본값은 러프하다 (hero 게임설계 메모)
     * 현 체감을 크게 흔들지 않는 쪽으로 잡았다. hero 가 "더 몰아쳐라 / 덜 나가라"를 원하면
     * **`countNear`·`intensityCountGain` 두 노브만** 움직이면 되고 코드는 안 바뀐다
     * (M3-B `vision.laneRead.pull` 과 같은 취급).
     */
    unit: {
      /**
       * false = 0.37.0 이전 동작(압박 담당 1명 · 목표 오염 그대로) — **롤백 스위치**이자
       * 변이체 킬 대조군. `221c673` 과 **bit-identical** 이어야 한다.
       */
      enabled: boolean;
      /**
       * 위험거리 = **공 → 우리 골** 거리 + `wideWeight` × 횡오프셋(m).
       * 벽의 `threatDistance` 와 같은 정의다 — 각도 대용(삼각함수 금지 §5-4).
       */
      wideWeight: number;
      /** 위험거리가 이 값 이하면 유닛 총원 = `countNear`. */
      dangerNearM: number;
      /** 위험거리가 이 값 이상이면 총원 = `countFar`. 사이는 선형. */
      dangerFarM: number;
      /** 위험 구역(우리 박스 앞)에서의 유닛 총원(압박 담당 포함). */
      countNear: number;
      /** 먼 곳(상대 진영)에서의 유닛 총원. 1 = 구동작(압박 담당만). */
      countFar: number;
      /**
       * 커버 후보가 공에서 떨어져 있어도 되는 최대 거리(m) — `intensity` 로 스케일된다.
       * ⚠️ 구 `movement.pressRange`(22m) 는 주석에 "압박 발동 거리"라고 적혀 있지만
       * **압박 배정에서 한 번도 읽히지 않았다**(`pressureCountAt` 의 기본 반경으로만 쓰인다).
       * 그 주석이 약속한 의미를 여기서 처음 실제로 쓴다. 그쪽 값·동작은 손대지 않았다.
       */
      rangeM: number;
      /** `intensity`(0..1) → 인원 배수 폭. 총원 × (1 + gain × (intensity − 0.5)). */
      intensityCountGain: number;
      /** `intensity` → 압박 사거리 배수 폭. 같은 형태. */
      intensityRangeGain: number;
      /**
       * 커버가 자기 블록 자리를 버리고 레인 지점 쪽으로 가는 비율(0..1).
       * **0 = 플라시보 팔**(역할은 배정하되 선점만 0) — A4 의 대조군이 이 값 하나로 만들어진다.
       */
      coverLanePull: number;
      /**
       * 커버가 맡는 레인 구간의 길이(m, 공에서부터).
       *
       * ⚠️ **이 절단이 "커버"와 "가로채기"를 가른다.** 레인 전체에 투영하면 커버가 리시버 쪽
       * 끝에 가서 서고, 그건 압박 담당을 **지원하지 않는** 그림이다(실축의 second defender 는
       * 첫 수비수 옆·뒤를 받친다). 공에서 이 거리까지의 구간만 맡게 잘라서, 커버가 압박 담당을
       * 받치면서 동시에 그 각을 지운다.
       */
      coverLaneReachM: number;
      /**
       * 레인 위협의 기준거리(m) — 리시버가 우리 골에서 이보다 가까우면 그 차이만큼 위협.
       * 위협 = `max(0, dangerRefM − 리시버↔우리골) + max(0, 전진이득)`.
       *
       * ## ⚠️ 여기서 **두 번** 틀렸고, 두 번 다 게이트 계측이 잡았다 (기록해 둔다)
       *
       * ① 초판은 위협을 *오직* **전진 이득**(이 패스가 공을 우리 골에 얼마나 더 가깝게 만드나)
       *    으로 정의하고 하한을 6m 로 뒀다. 그러면 **hero 제보가 말하는 바로 그 상황에서 기제가
       *    죽는다** — 공이 이미 우리 박스 앞이면 "6m 더 가까워지는 패스"는 기하학적으로 없다.
       *    4시드 실측(위험 구역 <25m): 틱당 **4.77명이 이득 부족으로 탈락, 살아남은 레인 0**.
       * ② 그래서 하한을 0 으로 내렸다(= "뒤로 가는 패스만 제외"). **여전히 죽었다** — 같은
       *    구역에서 (배정을 시도한 틱만 추려) 상대 아웃필더 **9명 중 8.96명이 탈락**했다.
       *    이유는 산수다: 공이 우리 박스 앞에 있을 때 그 공을 가진 선수가 보통 **가장 깊은**
       *    공격수라, 나머지 전원이 공보다 우리 골에서 멀다 = 전진 이득이 전부 음수다.
       *
       * → **방향 게이트 자체가 틀린 개념이었다.** 박스 앞에서 위험한 패스는 컷백·횡패스이고
       *   그건 "전진"이 아니다. 위협은 **수신 위치가 얼마나 위험한가**(절대)여야 하고, 전진
       *   이득은 있으면 더해 주는 **가산 항**이지 게이트가 아니다. 그래서 방향 게이트를 없애고
       *   위협 하한(`minThreatM`) 하나만 남겼다.
       *
       * ⚠️ 분모를 조심해라 — ①에서 ②로 갈 때 처음 본 수치(4.77)는 **배정을 시도조차 안 한 틱**
       * (압박 담당 없음·루즈볼)을 포함한 평균이라 신호가 절반으로 희석돼 있었다. `want > 0` 으로
       * 조건부로 보고 나서야 "9 중 8.96" 이 보였다.
       */
      dangerRefM: number;
      /** 커버를 붙일 최소 위협(m). 이 아래면 막을 값이 없다고 본다. */
      minThreatM: number;
      /** 커버가 "닿는다"고 보는 레인까지 최대 거리(m). 이 밖의 레인엔 자리를 안 버린다. */
      reachM: number;
      /**
       * 이미 이만큼(m) 막혀 있는 레인엔 겹치지 않는다.
       * ⚠️ M3-B(#379)가 실측으로 확인한 게이트와 **같은 성질·같은 자**(`laneDangerOn`)다 —
       * 없으면 도달비용 항 때문에 "내가 이미 서 있는 레인"이 언제나 이겨 선점이 제자리가 된다.
       */
      coveredM: number;
      /** 레인 도달비용 가중(가치 = 위협 − 레인거리 × 이 값). `chooseMarkTarget` 과 같은 관용구. */
      laneCostWeight: number;
      /**
       * **지원(support) 슬롯** — 막을 레인이 없을 때 유닛의 나머지가 서는 자리.
       *
       * ## 왜 이 역할이 필요한가 (실측이 시켰다)
       * 커버 섀도만으로는 유닛이 요청한 인원을 못 채운다. **위험한 레인이 희소하기 때문**이다 —
       * 4시드 게이트 실측(위험 구역, 배정을 시도한 틱): 요청 인원 `want` **2.0** 인데 실제로
       * 살아남은 레인은 **0.65** 뿐이었다(나머지는 위협 부족 6.9 · **이미 막힘 1.45** · 사거리 밖
       * 0.59 · 가치 ≤0 0.54). "이미 막힘"을 풀면 겹쳐 서게 되므로 그건 답이 아니다.
       *
       * 그래서 두 번째 역할을 둔다: **끊을 레인이 없으면 압박 담당을 받치러 간다.** 이게 hero
       * 제보(#350)가 문자 그대로 말한 것이다 — *"한 명만 붙어 있어도 나머지가 구경을 해"*.
       * 실축에서도 두 번째 수비수의 기본 임무는 특정 패스를 끊는 것이 아니라 **커버**다.
       *
       * ⚠️ 이 역할이 볼 10m 안 수비수(A2 의 자[尺])를 직접 올린다는 점을 숨기지 않는다. 다만
       * 순서가 반대였다면 문제였을 것이다 — 지표를 맞추려고 역할을 만든 것이 아니라, **유닛이
       * 요청 인원을 못 채운다는 실측**이 먼저 있었고 그 결손의 축구적 해답이 이 역할이다.
       */
      /** 지원 슬롯이 공에서 자기 골 쪽으로 떨어지는 거리(m). */
      supportGapM: number;
      /** 지원 슬롯끼리의 횡 간격(m) — 압박 담당·서로 위에 겹치지 않게. */
      supportSpreadM: number;
      /** 지원이 자기 블록 자리를 버리고 슬롯으로 가는 비율(0..1). `coverLanePull` 과 같은 형태. */
      supportSlotPull: number;
    };
  };

  /**
   * 틱당 기본 피로 증가(0..1 스케일). 질주/압박 시 가중.
   * ⚠️ `fatigue.recoveryEnabled=false`(롤백 경로)에서만 쓰인다 — 기본 경로는 `fatigue` 블록이다.
   */
   ...
   *
   * ## 무엇이 문제였나
   * 구 모델은 `fatigue += fatiguePerTick × exertion` **단조 증가**뿐이었다(엔진 전체에서 대입이
   * 한 곳, 회복 항 0, 하프타임 리셋 없음). `1/0.0009 = 1112틱` 이면 1.0 포화 →
   * **경기의 79.4% 구간에서 전 아웃필더가 fatigue = 1.0(전원 동일)** 이었다.
   * 그리고 `exertion` 의 `active` 는 **매 틱 소유자 1명 + 압박자 1명**뿐이라 사실상 전원 1.0 —
   * **누가 얼마나 뛰었는지가 피로에 반영되지 않았다.**
   *
   * fatigue 는 장식이 아니라 이동속도(×0.55) · xG(×0.70) · 드리블(×0.60) · 공중경합(×0.75) ·
   * 사슬 EV 에 **곱해지는 승수**다. 즉 경기의 79% 가 "전원 최대 피로"라는 상수 위에서 돌았고,
   * 하프타임 교체·컨디션(Phase 2)·`stamina`(#337)가 닿을 자리가 구조적으로 없었다.
   *
   * ## 무엇으로 바꿨나
   *  ① 부하를 **그 틱에 실제로 이동한 거리**로 잰다(최대속도 대비 비율). 서 있으면 안 는다.
   *  ② **회복 항**이 매 틱 깎는다. 그래서 곡선이 포화가 아니라 **오르내린다**.
   *  ③ **데드볼 정지 틱**은 회복 배수가 붙는다(숨 고르기) — 구 모델은 정지 중 피로 갱신 자체가 없었다.
   *  ④ **하프타임 회복**(절대 감소) — 후반이 전반과 같은 피로에서 시작하지 않는다.
   *
   * ⚠️ 계수는 **러프 기본값**이다(#377 방침: 구현과 계수 조정 분리). fatigue 가 5개 소비자에
   * 곱해지므로 이 값들은 볼륨을 움직인다 — 정밀 보정은 트랙 T 소관이다.
   */
  fatigue: {
    /** false = 구 모델(단조 증가, `fatiguePerTick` 사용) — 롤백 스위치·변이체 킬 대조군. */
    recoveryEnabled: boolean;
    /** 부하 계수. 그 틱 이동거리 / `speed.maxPerTick` (0..1)에 곱한다. */
    exertionPerTick: number;
    /** 인플레이 틱당 기본 회복량. */
    recoverPerTick: number;
    /**
     * **누진 회복** 계수 — 회복량 = `recoverPerTick × (1 + recoverFatigueGain × fatigue)`.
     *
     * 왜 필요한가: 단순 상수 회복으로는 "평균 부하는 옛 모델만큼 무겁게" 와 "아무도 1.0 에
     * 박히지 않게" 를 **동시에** 못 만든다(실측: 평균 0.727 을 만들면 종료 시점 산포가 0.000 —
     * 즉 전원이 다시 1.0). 지쳤을수록 더 빨리 회복하면 평형점
     *   f* = (부하 − r) / (r × gain)
     * 이 1 아래에 생겨서, 많이 뛴 선수와 덜 뛴 선수가 **서로 다른 평형**에 앉는다.
     * 0 이면 상수 회복(이 필드 이전 동작).
     */
    recoverFatigueGain: number;
    /** 데드볼(정지) 틱의 회복 배수 — 실축의 "숨 고르기". */
    deadBallRecoverMult: number;
    /** 볼 소유자·압박 담당의 추가 부하 배수(구 모델의 1.6 을 계승). */
    activeMult: number;
    /** 골키퍼 부하 배수(구 모델의 0.3 을 계승). */
    gkMult: number;
    /** 하프타임에 한 번 빼는 절대 회복량(0..1). */
    halfTimeRecover: number;
    /** 회복이 도달할 수 있는 하한(0 = 완전 회복). */
    floor: number;
  };
  ...
  movement: {
    /**
     * **예고 패스 읽기**(#369 — hero: *"받는 쪽이 패스하는 사람의 생각을 예측, 둘이 링크되듯이.
     * 선수들은 동료들과 훈련을 했기 때문에 미리 한 단계 앞서 움직인다"*).
     *
     * ## 무엇이 없었나
     * `intents` 는 **찬 그 틱에만** 게시됐고(#314), 틱 순서상 오프더볼 결정이 볼 소유자 결정보다
     * **앞**이라 리시버는 언제나 *반응*만 했다. 그래서 패스가 리시버 **발밑**으로만 갔다
     * (리드 거리 p50 3.48m — 스루패스는 10~25m). 스루패스는 없는 게 아니라 **성립할 순간이
     * 만들어지지 않았다.**
     *
     * ## 어떻게 만드나
     * 캐리어의 사슬은 이 틱에 이미 최상위 패스 후보를 정수 EV 로 계산해 뒀다. 안 찼어도 그걸
     * **예고로 게시**하고(`Intent.kind = "pass_plan"`), 동료가 **능력만큼** 읽어 도착 예정 지점으로
     * 미리 출발한다. 새 예측기를 만들지 않는다 — 리시버가 같은 탐색을 다시 돌리면 비용만 22배다.
     *
     * ## 텔레파시 방지
     * **아군만 읽는다.** 상대의 예측은 별개 축(#379)이고 정보 출처가 다르다(게시판이 아니라 기하).
     *
     * ## RNG
     * 읽기 판정은 **시드 노이즈**(`varietyNoise`)다 — RNG 스트림을 **한 번도 소비하지 않는다**.
     * #369 가 경고한 "후보 수에 비례한 RNG 소비 → 재개 계약 취약"을 구조적으로 회피한다.
     */
    passPlan: {
      /** false = 예고 게시·읽기 없음(#369 이전 동작). 롤백 스위치·변이체 킬 대조군. */
      enabled: boolean;
      /** 읽기 확률의 바닥(능력 50 기준). */
      readBase: number;
      /** 능력((mental+positioning)/2)이 0→100 일 때 읽기 확률에 주는 가감폭. */
      readAttrSwing: number;
      /** 읽은 리시버의 목표를 도착 예정 지점 쪽으로 당기는 비율(0..1). */
      pull: number;
      /** 예고의 수명(틱). 캐리어가 마음을 바꾸면 낡은 예고가 남지 않게 짧게. */
      expireTicks: number;
    };
    ...
    /**
     * **공유 수비 라인 보정의 강도**(0..1) — #377 S3-B 에서 처음 실제로 소비된다.
     *
     * ⚠️ 구 주석은 "base 로 복귀하는 비율"이었는데 **읽는 코드가 0줄이었다**(3시드 최종 해시가
     * 0 / 0.5 / 1.0 에서 전부 동일 = `8c1af96c 3bfd7771 f3049b84`). `dead-knobs` 레지스트리에도
     * 미등록이라 그 사실이 아무 데서도 부정되지 않았다. 이제 `teamplan.ts:applyDefensiveLine`
     * 의 유일한 세기 노브이고 **LIVE 로 등록**돼 있다.
     *
     * 0 = 배정은 하되 보정 0 = **플라시보 팔**(라벨이 아니라 기제가 원인임을 가르는 대조군).
     *
     * ⚠️ **이 슬라이더를 올리면 `ball-physics` 의 "무소유 급정지"가 따라 오른다** — 트랙 T
     * 재보정자가 알아야 할 성질이다(S3-B 독립검증 m3). 60시드 실측(`unownedDeadStops` 정의
     * 그대로, 상한 25):
     *   OFF(defLine+rest off) **25.07** · k=0.25 **24.45** · k=0.65(출하) **25.27** · k=1.0 **27.03**
     * (20시드도 같은 방향: 24.80 · 23.80 · 25.45 · 26.65.) 즉 **출하값은 안전지대의 가장자리**이고
     * 라인을 더 조이면 그 축이 임계를 넘는다. **기제는 규명하지 않았다** — 여기 있는 것은 관측된
     * 동반 관계뿐이다. 이 노브를 올릴 거면 `#399` 소유 계약인 `ball-physics` 급정지를
     * **같이 재고** 넘어가라(그 임계·표본은 이 웨이브가 안 건드렸다).
     */
    lineDiscipline: number;
    /**
     * **공유 수비 라인**(#377 S3-B · 로드맵 W5-2). 백4가 서로를 보고 한 줄을 유지한다.
     *
     * ## 왜 "절대 당김"이 아니라 "상대 보정"인가 (이 설계의 핵심 근거)
     * 구조는 라인이 아니라 **독립된 스프링 4개**였다 — 수비 x = `base + (lineX − base) ×
     * defendCompactX(0.32)` 라 **68%가 자기 포메이션 슬롯**이고, 4-3-3 슬롯은 풀백을 CB보다
     * **6.3m 앞**(0.22 vs 0.16 × 105)에 둔다. 한 수비수의 위치를 **다른 수비수와 비교하는 코드가
     * 0줄**이었다.
     *
     * ⚠️ 그리고 **투영을 100%로 올려도 라인이 안 생긴다**(8시드 실측): `defendCompactX=1.0` 이면
     * 백4의 목표가 문자 그대로 **같은 점**인데 산포는 여전히 **7.60m**, 후속 당김(마크·유닛·roam·
     * runOrder)까지 전부 아블레이션해도 **7.20m** · FB↔CB **5.90m ≈ 슬롯 오프셋 6.3m 그대로**다.
     * 이유는 지연이다 — 라인 목표는 공을 1:1로 따라가는데(최대 18 m/tick) 선수는 ~2–3 m/tick라
     * **전원이 같은 속도로 뒤처지고 초기 오프셋이 보존된다**. 뒤진 선수에게 "더 빨리 뛰어라"는
     * 불가능한 주문이고, **앞선 선수에게 "기다려라"** 가 가능한 주문이다. 그래서 기준선이
     * 팀계획 라인(절대)이 아니라 **멤버 자신들의 평균**(상대)이다.
     *
     * (같은 실측이 `defendCompactX` 가 이 축의 레버가 아님도 보여준다 — 5-rung 사다리
     *  0.16 · 0.32 · 0.50 · 0.70 · 0.90 → 산포 10.62 · 9.92 · **10.68** · 8.92 · 7.27 로 **비단조**다.
     *  두 점만 보면 단조로 보인다 — S3-A 가 "단조 레버"로 기록한 것이 그 착시다.)
     */
    defLine: {
      /** false = #377 S3-A(0.38.0) 동작. **롤백 스위치** — 비트 동일이어야 한다. */
      enabled: boolean;
      /** 라인 멤버 판정: base 진행도가 이 값 이하인 아웃필더(0..1). 백4 0.16/0.22 ⊂ · 미드 0.40 ⊄. */
      memberProgressMax: number;
      /** 이 인원 미만이면 라인을 잡지 않는다(압박 유닛이 데려간 뒤 1명 남은 상태 등). */
      minMembers: number;
      /**
       * 역할 깊이차 보존 비율(0 = 완전 평평, 1 = 현 슬롯 오프셋 그대로).
       * 실축 라인도 완전 평면은 아니라 0 이 정답이 아니다 — 러프 기본값, 조정은 트랙 T.
       */
      roleOffsetKeep: number;
      /**
       * **응집 밴드(m)** — 라인이 *실제로 서 있는 곳* 기준 이 거리 밖으로는 목표를 보내지 않는다.
       * 로드맵 W5-2 의 `blockLineRangeM` 이 이것이다.
       *
       * ⚠️ **밴드 안에서는 아무것도 안 한다** — 마킹·압박이 정한 자리를 근거리에서 흔들지 않고
       * **이탈자만** 되돌리는 외과적 제약이다.
       */
      blockLineRangeM: number;
      /**
       * `computeTeamPlan` 의 라인 높이 권한(피치 길이 비) — 구 하드코딩 상수 `LINE_HEIGHT_RANGE`
       * 를 config 로 승격한 것(§2-4). **죽은 슬라이더 소생**(#361 계열).
       *
       * ⚠️ 실측: 구 값 0.2 에서 `team.defensiveLineHeight` 전 구간(0.2 → 0.9)의 라인 높이 권한이
       * **3.7m** 다(오프사이드 라인 25.52 → 29.23m). 유저의 "하이라인/로우블록"이 사실상 안 닿는다.
       * hero 가 더/덜 원하면 **이 값만 움직이면 된다** — 다른 계수는 안 건드려도 된다.
       *
       * 롤백(`enabled=false`)이면 구 상수 0.2 를 쓴다(비트 동일 보장).
       */
      heightRangeX: number;
      /**
       * 밴드의 기준점.
       *  - `"members"`(출하) = **멤버들이 실제로 서 있는 위치의 평균**.
       *  - `"planLine"` = 팀계획 `plan.lineX`(공에 매인 절대 좌표).
       *
       * ⚠️ 이 두 팔의 차이가 이 웨이브의 **설계 주장 그 자체**다. 실측(8시드): 라인 멤버의 목표는
       * 이미 촘촘한데(산포 4.40m) **선수는 자기 목표에서 평균 7.65m 뒤에** 있다 — 즉 산포는 목표에
       * 있지 않고 **지연**에 있다. 절대 기준점은 "전원이 못 따라가는 같은 점"을 줄 뿐이라 지연
       * 편차를 못 건드리고, **실제 위치** 기준이라야 앞선 선수를 세워 뒤진 선수를 기다리게 할 수
       * 있다. `"planLine"` 은 그 "안 되는 쪽"을 config 로 재현하는 팔이다 — 아블레이션의 한 팔이
       * 재현 불가면 귀속 분해 자체가 검증 불가가 된다(#377 M2 m3).
       */
      refMode: "members" | "planLine";
      /**
       * **오프사이드 트랩**(#377 S3-C · 로드맵 W5-3) — 라인을 **조건부로, 잠깐** 밀어올린다.
       *
       * ## 0.39.0 까지 트랩은 "약한" 것이 아니라 **없었다**
       * `team.offsideTrap` 의 유일한 소비처는 `contest.ts:checkOffside` 였다 — **아무도 안 움직이고
       * 심판의 판정선만** `rules.offside.trapBiasM` 만큼 옮겼다. 유저/AI 가 트랩을 요청해도 선수는
       * 한 명도 다르게 서지 않는다. 여기서 그 지시가 **실제 위치**를 갖는다.
       *
       * ## 왜 "무차별 상향"이 아니라 **거리 게이트**인가 (이 설계의 핵심 실측)
       * 20시드 계측(`realism/trap.ts`, 출하 config). 공↔우리 골 거리 버킷별로 —
       *
       * | 버킷 | 수비 틱% | 어깨(4m) 인원 | 라인 뒤 상대 | **뚫린 비율** |
       * |---|---|---|---|---|
       ...
       *
       * **기회(어깨 인원)는 25–40m 에서 위험지역과 사실상 같은데(0.841 vs 0.858) 대가는 1/3,
       * 40–60m 이면 1/20 이다.** 무차별 하이라인은 이걸 못 고른다 — `defensiveLineHeight=0.9` 는
       * <25m 구간의 뚫린 비율을 **65.18%** 로 폭발시킨다. 그래서 트랩은 **어디서 거는가**가 전부다.
       *
       * ## 왜 이진 on/off 가 아니라 **연속 세기**인가
       * 목표를 매 틱 켰다 껐다 하면 #178(마크 당김 진동)이 그대로 재현된다 — 목표가 뒤로 갔다
       * 앞으로 오면 선수는 제자리에서 왕복한다. 그래서 세기를 트리거 강도에 **연속**으로 매고
       * (`releaseSmooth`), 상태(hold/cooldown)를 **저장하지 않는다**. 무상태인 두 번째 이유는
       * 와이어다 — `SimState` 에 필드를 늘리면 `packages/server` 의 `SimStateSchema` 가 그것을
       * **조용히 버려**(zod 는 미선언 키를 버린다) 하프 재개에서 무음 유실이 된다(#154/#241 계열).
       *
       * ⚠️ **계수는 전부 러프 기본값이고 조정은 트랙 T.** hero 가 "트랩을 더 세게"를 원하면
       * `stepUpM` 만, "덜 위험하게"를 원하면 `minBallDistM` 만 움직이면 된다.
       *
       * ⚠️ 출하 픽스처는 `team.offsideTrap: false` 라 **이 블록은 출하 벤치마크에서 발화하지 않는다**
       * (골든 무이동). `dead-knobs` 레지스트리에 **조건부 LIVE** 로 등록돼 있고, 레버성은 트랩을
       * 켠 상태에서 검정한다.
       */
      trap: {
        /** false = #377 S3-B(0.39.0) 동작. **롤백 스위치** — 비트 동일이어야 한다. */
        enabled: boolean;
        /**
         * 트리거가 최대일 때 라인 기준점을 상대 골 쪽으로 미는 거리(m).
         * 0 = **플라시보 팔**(판정·라벨은 그대로, 이동만 0 — 라벨이 아니라 기제가 원인임을 가른다).
         */
        stepUpM: number;
        /** 공↔우리 골이 이 거리 **이상**일 때만 건다(위 표의 거리 게이트). */
        minBallDistM: number;
        /** 라인 **바로 앞**(온사이드) 이 거리 안의 상대를 "어깨 위 러너"로 센다. */
        shoulderBandM: number;
        /** 어깨 위 러너가 이 인원 **이상**이어야 건다(잡을 사람이 없으면 안 건다). */
        minShoulder: number;
        /**
         * 거리 게이트 근방의 **완충 폭(m)** — `minBallDistM` 에서 이 거리에 걸쳐 세기가 0→1 로
         * 선형으로 열린다. 0 이면 계단(= 이진 스위치 = #178 플리커 재현).
         */
        releaseSmooth: number;
      };
      /**
       * 밴드를 어느 쪽으로 무는가.
       *  - `"holdBack"`(출하) = **너무 나간 선수만 되돌린다.** 뒤에 있는 선수는 그대로 둔다.
       *  - `"both"` = 뒤처진 선수도 라인으로 **밀어 올린다**(양방향 클램프).
       *
       * ⚠️ `"both"` 가 직관적이지만 실측이 반대를 말한다. 양방향이면 라인이 통째로 올라간다
       * (수비 라인 높이 27.40 → 28.19m, `lineDiscipline` 사다리에서 26.89 → 28.98m 단조 상승) —
       * **가장 깊은 수비수는 보통 커버 때문에 깊은 것**이라 그를 끌어올리면 뒤 공간이 열린다.
       * 60시드 골이 5.58 → 6.97 로 오른 것의 기제가 이것이다(잡음 바닥 0.82 를 넘는 부분).
       * 응집 자체는 한쪽만 물어도 성립한다 — 라인을 흩뜨리는 것은 **앞으로 튀어나간 선수**이지
       * 뒤에 남은 선수가 아니기 때문이다.
       */
      bandMode: "holdBack" | "both";
    };
    /**
     * **레스트 디펜스(오픈플레이)**(#377 S3-B · 로드맵 W5-8). 공격할 때 뒤에 남을 사람이 정해져 있다.
     *
     * ⚠️ 지금 `attackLinePush`(0.56)에는 **역할 게이트가 없다** — `tx += (ball.x − base.x) × 0.56`
     * 이 센터백에게도 그대로 걸린다. 실측(8시드): 공격 중 CB가 **하프라인을 넘은 틱 18.53%**,
     * 평균 진행도 37.91m(슬롯 16.8m보다 21m 앞), 경기당 최고 **91.21m**(상대 골라인 14m 앞).
     * 잔류 로직은 **코너 분기 안에만** 있었다(`decision.ts:cornerHolderRank`).
     *
     * ⚠️ `attackLinePush` 를 내리는 것은 해법이 아니다 — 같은 사다리에서 공격 시 팀 산포가
     * 17.18 → 13.20 으로 같이 눌린다(팀 업필드 이동은 의도된 동역학). **역할 조건부 상한**이라야 한다.
     *
     * 인원 매핑·전순서 선정은 코너와 **같은 관용구**를 쓴다(`teamplan.ts:holderRank` 로 일반화,
     * 코너가 그 특수화). 계수는 전부 러프 기본값이고 조정은 트랙 T — hero 가 "더 공격적으로"를
     * 원하면 `countMin/countMax` 만, "덜 올라가게"를 원하면 `lineCapProgress` 만 움직이면 된다.
     */
    restDefence: {
      /** false = 0.38.0 동작. **롤백 스위치** — 비트 동일이어야 한다. */
      enabled: boolean;
      /** 가담도 최대일 때 남는 인원. */
      countMin: number;
      /** 가담도 최소일 때 남는 인원. */
      countMax: number;
      /** 가담도 = 0.5 + (defensiveLineHeight − 0.5)×이것 + (tempo − 0.5)×아래(코너와 동형). */
      commitLineWeight: number;
      commitTempoWeight: number;
      /** 잔류 선정 시 선수 성향(forwardRunFreq) 가중. 크면 슬롯 순서를 뒤집는다(코너와 동형). */
      playerOverrideWeight: number;
      /** 잔류 인원의 진행도 상한(0..1, 0.5 = 하프라인). **1 = 상한 없음 = 플라시보 팔**. */
      lineCapProgress: number;
    };
    ...
    /**
     * **박스 도착런**(#407 N2) — "박스에 ST 말고 아무도 없다"를 여는 유일한 측정된 기제.
     *
     * ## 무엇이 없었나 (구조 사실 — 측정이 아니다)
     * `decision.ts:decideOffBall` 의 인포제션 폭 항은 `widthDir = base y 의 부호` 라
     * **언제나 자기 자리에서 더 바깥으로** 민다. **안쪽으로 미는 항이 한 줄도 없다.**
     * 그래서 오프더볼 전진은 base y 를 유지한 x 보간뿐이고, 박스 안 수신은 ST 가 90% 를
     * 독점한다(팀-경기당 ST 4.93 vs LW 0.10 / RW 0.07 — `research/e407-goal-centrality.md` C3).
     *
     * ## ⚠️ 왜 **조건부**여야 하나 (전역은 이미 실패로 확정됐다)
     * `attackWidthReach` 를 전역으로 음수화(0.10→−0.10)하면 축 B 는 실제로 열린다 —
     * 비ST 박스수신 0.54 → **7.60**(14배) · ST 슛 점유 95.3 → 89.0%. 그러나 풀백·센터백까지
     * 안으로 접혀 **팀 폭 46.8 → 37.1**(밴드 40–50 이탈) · **스로인 10.15 → 1.09**(공이
     * 터치라인에 안 간다) 로 경기가 부서진다(`research/e407-volume-diversity.md` §7-1, 60시드).
     * → 그래서 이 항은 **파이널서드 + 자기팀 소유 + 박스 밖**이라는 세 게이트를 통과한
     * 틱에만, **선택된 소수**에게만 걸린다. 비소유·비파이널서드 구간의 배치는 한 줄도 안 바뀐다.
     *
     * ## ⚠️ 지시는 **지속**돼야 한다 (초판 실패의 기제 — 실측으로 확정)
     * 초판은 게이트가 열린 틱의 목표를 직접 당겼다. 그런데 게이트 ①(자기팀 소유)은
     * **패스·슛이 비행하는 동안 닫힌다**(`ball.owner === null`). 박스까지 25m ≈ 10틱을 뛰어야
     * 하는데 당김이 1~3틱마다 끊기니 러너가 **왕복만 하고 도착하지 못했다** — 20시드 진단
     * (`research/e407-probe/e407-boxpop.ts`): 게이트 틱당 박스 안 비ST 인원 0.46 → **0.50**
     * (전역 팔은 0.97). 그래서 이 기제는 `holdTicks` 동안 유지되는 **`SimPlayer.runOrder`**
     * (#314 B)를 발행한다 — 새 상태를 만들지 않고, 만료·데드볼 취소·수비의 러너 예측
     * (`vision.runReadFrac`)이 전부 공짜로 따라온다.
     *
     * ## 규율 (재발명 금지 — 기존 부품 재사용)
     *  - 지시 자료구조 = `SimPlayer.runOrder` · **소비(당김) = `applyRunOrders` 의 pull 루프**.
     *    당김 세기는 그래서 `movement.runOrder.pull` 이고 여기 별도 노브를 두지 않는다.
     *    (대가: 런 오더를 끄면 박스 도착런도 안 움직인다 — 롤백 경로라 의도된 결합.)
     *  - "누가 뛰어드나" = 역할 하드코딩이 아니라 **위치 판정**이다: 지금 전진해 있고(진행도)
     *    아직 박스 밖인 아웃필더. 그래서 오버래핑 풀백(`variety.defenderOverlapProb`)이 자동으로
     *    포함되고, 이미 박스 안인 ST 는 자동으로 빠진다.
     *  - `SimState`·`RunOrder` 에 필드를 늘리지 않는다 — `SimStateSchema` 미선언 키 무음 유실
     *    (#154/#241)을 피하고, 그 스키마는 이 모듈의 owned-glob 밖이다.
     */
    boxArrival: {
      /** false = 0.42.0 동작. **롤백 스위치** — 비트 동일이어야 한다. */
      enabled: boolean;
      /**
       * **동시에** 박스로 뛰는 최대 인원(이미 뛰고 있는 러너까지 세어 정원을 지킨다).
       * 전역화 방지의 1차 방어선이다 — 이 수를 키우면 팀 폭이 따라 내려간다(전역 팔이 부서진 그 축).
       */
      maxRunners: number;
      /**
       * 발행한 런의 수명(틱). **이 웨이브의 핵심 노브**다 — 박스까지 ~10틱이 걸리는데 게이트는
       * 패스 비행마다 닫히므로, 이 값이 짧으면 러너가 왕복만 하고 도착하지 못한다(초판 실패).
       */
      holdTicks: number;
      /**
       * 발화 문턱: 공의 공격 진행도가 이 값 이상일 때만 런이 걸린다.
       *
       * ⚠️ **`setPiece.finalThirdLine`(0.66) 과 값이 같아도 별도 노브다.** 그건 "어디가
       * 파이널서드인가"(패스·세트피스 판정의 자)이고 이건 "언제 뛰기 시작하나"(도착 시간의 자)라
       * 축이 다르다 — 런이 완주하려면 ~10틱이 필요하므로 문턱을 내리면 도착률이 오르고 대신
       * 팀 폭·스로인이 내려간다. 그 트레이드오프를 사다리로 잰다.
       */
      triggerProgress: number;
      /**
       * 후보 자격: **지금 위치**의 공격 진행도가 이 값 이상. base 가 아니라 실위치인 것이
       * 설계다 — 오버랩으로 올라온 풀백은 포함되고, 뒤에 남은 CB 는 자동으로 빠진다.
       */
      minRunnerProgress: number;
      /**
       * 후보 자격 ②: **base 의 횡편차**가 이 값(m) 이상 = "원래 밖에 서 있는 사람"만 뛰어든다.
       *
       * 이 항의 존재 이유는 "폭 항이 바깥으로만 밀어서 아무도 안 들어온다"인데, 자격을 열어 두면
       * 이미 중앙에 있는 미드필더(LCM, base 횡편차 12.2m)가 전진도에서 앞서 먼저 뽑히고
       * **박스 앞 슈팅 자리에 있던 비ST 슈터를 박스 안으로 치운다**. 20시드 짝 대조(정렬키·나머지
       * 노브 고정) lat0 → lat16: **비ST 슛 0.73 → 0.90 · 슛 HHI 0.914 → 0.891 · 박스ST% 82.7 → 80.7**.
       * ⚠️ 실재하지만 **작다**(8시드에서는 부호가 뒤집힌다) — 그래서 계약이 아니라 측정 기록이다.
       * 4-3-3 횡편차: LW/RW·LB/RB **20.4m** · LCM/RCM 12.2m · CB 6.8m · CM/ST 0.
       */
      minBaseLatM: number;
      /** 도착 슬롯의 골라인으로부터 깊이(m). 첫 러너가 가장 깊다(니어포스트 쪽). */
      arrivalDepthM: number;
      /** 러너 1명당 슬롯을 뒤로 미는 양(m) — 두 명이 같은 점에 겹치지 않게(코너 `jitterX` 관용구). */
      slotSpreadM: number;
      /**
       * 도착 슬롯의 최대 횡편차(m). base y 의 **부호를 보존**한 채 이 폭으로 접는다 —
       * 왼쪽 윙어는 왼쪽 포스트, 오른쪽은 오른쪽. 0 으로 두면 전원이 골 정면에 겹친다.
       */
      arrivalHalfWidthM: number;
    };
  };
  ...
  vision: {
    ...
    /**
     * **수비 레인 예측**(#379 — W0 §2-B). 수비수가 공 위치만 쫓는 대신 **패스 레인**
     * (캐리어 → 위협적인 리시버 후보)을 읽고 그 선상으로 **미리** 이동한다.
     *
     * ## ⚠️ 정보 출처가 이 기제의 정의다 — **게시판을 안 읽는다**
     * M3-A(#369)가 만든 `intents`(`pass_plan`)는 **아군 전용 게시판**이다. 수비가 그걸 읽으면
     * 그 순간 텔레파시가 되고, W0 이 A 와 B 를 다른 축으로 그은 이유가 사라진다. 그래서 여기
     * 입력은 **기하뿐**이다 — 공(=캐리어) 위치 · **이 수비수가 실제로 인지한**(`vision` 기억,
     * 0.17.0) 상대의 마지막 본 위치 · 우리 골대. `readLane` 은 `state.intents` 도 `runOrder` 도
     * 보지 않고, 계약(`realism/lane-read.test.ts`)이 그것을 **소스 수준으로 박제**한다.
     *
     * ## 왜 확률인가(연속 강도가 아니라)
     * `readPassPlan`(#369)과 **같은 관용구**: 능력 비례 확률로 읽고, 판정은 `varietyNoise` 라
     * **RNG 스트림을 한 번도 소비하지 않는다**. 연속 강도로 만들면 전원이 조금씩 읽어
     * "이 수비수가 읽었나"를 가를 수 없고 — 즉 **출하 config 안에 대조군이 없어진다**.
     * 확률이면 같은 기하에서 READ/UNREAD 가 갈리므로 계약이 반사실 팔 없이 광고한 동작을 잰다
     * (M3-A 독립검증 m1 의 교훈).
     *
     * ## ⚠️ 세기는 `pull` 하나다 (hero 게임설계 항목 W0 §5-4)
     * *"'스루패스가 나오는 것'과 '수비가 똑똑한 것' 중 어디에 무게를 둘까?"* — 회신이 없어
     * **약하게**(스루패스가 살아 있는 쪽) 잡았다. 지금도 레인 점유가 39.5% 라 더 조이면 패스가
     * 더 죽고(W0 §3 이 "B 먼저"를 기각한 근거), M3-C 스루패스가 경기당 1.5건뿐이라 여유가 얇다.
     * hero 가 **반대로 정하면 `pull` 만 올리면 된다** — 코드는 안 바뀐다.
     */
    laneRead: {
      /** false = 레인 예측 없음(#379 이전 동작). 롤백 스위치·변이체 킬 대조군. */
      enabled: boolean;
      /** 읽기 확률의 바닥(능력 50 기준). */
      readBase: number;
      /** 능력((positioning+mental)/2)이 0→100 일 때 읽기 확률에 주는 가감폭. */
      readAttrSwing: number;
      /**
       * 읽기 판정 버킷 길이(틱). 매 틱 새로 굴리면 같은 레인을 읽었다 말았다 해서 목표가 튄다
       * (= #185 제자리 진동). 이 길이 동안은 판정을 유지한다.
       */
      readPeriodTicks: number;
      /**
       * 레인까지 거리의 몇 %를 이번 틱 선점에 쓰나(0..1). **이 기제의 세기 노브**.
       * 거리의 비율이라 투영점을 **넘어가지 않는다** — #178 이 겪은 "고정 길이 스텝이 마크를
       * 지나쳐 반대편을 목표로 잡는" 오버슛이 구조적으로 불가능하다.
       */
      pull: number;
      /** 한 틱 선점량 상한(m). 레인을 쫓아 자기 블록을 통째로 버리지 않게 한다. */
      maxStepM: number;
      /** 이 거리(m) 밖의 레인은 읽지 않는다(닿지 않는 레인을 향해 걸어나가지 않게). */
      reachM: number;
      /** 대상 레인 선택의 도달비용 가중(레인까지 거리 계수). 클수록 가까운 레인만 본다. */
      laneCostWeight: number;
      /**
       * 위협 하한(m) — 그 리시버가 공을 받았을 때 공이 우리 골에 **이만큼은 더 가까워져야**
       * 막을 값이 있는 레인이다. 뒤로 빼는 패스 레인을 쫓지 않게 한다.
       */
      minThreatM: number;
      /**
       * **이미 막힌 레인**의 기준(m) — 그 레인에 우리 팀 최근접(`laneDangerOn`)이 이 안이면
       * 후보에서 뺀다. 겹쳐 서는 것은 선점이 아니다.
       *
       * ⚠️ 이 게이트는 장식이 아니다. 없이 만들었더니 가치식의 −도달비용 항 때문에 **자기가
       * 이미 서 있는 레인**이 언제나 이겼고, 그래서 세기를 4배로 올리고 전원이 읽게 해도
       * 레인 점유 집계가 **한 톨도 안 움직였다**(6 rung 사다리 전 구간 57.9~58.2%).
       * 실축의 "레인 차단"도 같은 판단이다 — 이미 닫힌 선에 두 명이 서지 않는다.
       */
      coveredM: number;
    };
  };
...
export const defaultEngineConfig: EngineConfig = {
  ...
  // 0.32.0 = **입력 소생**(#377 M2). ①#346 피로 경제 재설계 — 부하를 **실제 이동량**으로 재고
  //   **회복 항**(누진: 지쳤을수록 빨리 회복)·데드볼 숨고르기·하프타임 회복을 넣었다. 구 모델은
  //   경기의 45%가 **전원 fatigue = 1.0**(산포 0.000)이라 교체·컨디션·stamina 가 닿을 자리가
  //   없었다 → 포화 0.0% · 종료 산포 0.214. ②#361 죽은 슬라이더 3종 배선(`team.width` ·
  //   `pressingScheme.triggerLine` · `passDirectness`) ③#366 `duty` 배선(결판 = (a) 배선).
  //   ④#338 죽은 노브 **레지스트리 계약**(주석은 스테일해진다 — INERT/LIVE 를 해시로 박제).
  //   ⚠️ 볼륨이 오른다. **8시드 아블레이션(최종 커밋 콜드)**: ①전부 off 7.44 → ②배선만 8.19 →
  //   ③피로만 10.06 → ④전부 on **11.38**. 즉 배선 단독 **+0.75** · 피로 단독 **+2.62** ·
  ...
  // 0.34.0 = **배포 발차 빌드**(#377 트랙 D 누적 + #370 원인값 되돌림, hero GO 2026-08-02).
  //   `contest.shootXgThreshold` **0.197 → 0.07** — 0.197 은 라이브 슛 79% 붕괴의 원인값이고
  //   (절대 xG 컷인데 xG 분포가 덱마다 다르다) 픽스처로는 원리적으로 못 잡는 부류다(#370).
  //   ⚠️ 픽스처 총량이 크게 오른다(팀당 슛 11.93 → **16.79** · 경기당 골 5.65 · 유효슛 6.24 ·
  //   전환 16.85% · 슛당 xG 0.18). §2-4 상 총량은 참고 지표이고 재보정은 **트랙 T** 소관이다 —
  //   이 커밋은 **정확성 수정**이지 밸런스 조정이 아니다.
  //   범프 필수: 구 `resumeState` 는 거부돼야 한다(진행 중 매치 처리 = #241).
  // 0.42.0 = **오프사이드 콜 빈도 복구**(#407 ⑦, config-only). `rules.offside.callProb`
  //   0.013 → **0.045** — 판정 로직은 무변경(FP 0 확정)이고 빈도 게이트 하나만 재보정했다.
  //   n60: 오프사이드 0.425 → **1.175**(벤치 1–3 진입). 부작용은 전부 밴드 쪽 또는 중립 —
  //   슛 17.48 → 17.02 · 파울 4.83 → 5.09 · 코너 5.89 → 5.60 · 패스% 82.8 → 82.6.
  //   ⚠️ 이 블록은 **출하값 0.045 의 실측**이다(`:2302` 와 같은 값). 초판이 rung 실험값
  //   0.055 의 수치를 실었던 것을 #407 ⑦ 독립 검증(B1)에서 정정했다 — 다른 조건에서 잰
  //   수치를 이 릴리스의 증거로 싣지 않는다(`gate-report-cold-rerun` 규율). 사다리 전 rung 의
  //   값은 `:2287` 주석에 따로 있고, 그건 **탐색 기록**이지 출하 증거가 아니다.
  ...
  //   슛 HHI 0.904 → **0.895** · 슈터 수 1.68 → **1.76** 를 **팀 폭 45.9(밴드) · 스로인 11.8(생존)**
  //   으로 얻는다 — 같은 효과의 전역 우회(`attackWidthReach` −0.10)가 폭 37.1 · 스로인 1.09 로
  //   경기를 부순 그 자리다. **조건부 설계가 성립한다는 것이 이 웨이브의 성과**다.
  //   **왜 못 켰나**: 파울 5.09 → **4.10~4.70**(hero AC = "파울을 악화시키지 않을 것") ·
  //   1대1 5.19 → 4.24~4.84 · 패스 82.6 → 79.0~80.2 · 전환 18.5 → 16.9. 그리고 정작 노린
  //   **비ST 슛/tm 은 0.85 → 0.87 로 평평**하다(ST 골 비중 96.3 → 96.4~96.6 · 와이드 슛 0.0% 유지).
  //   **기제로 규명된 것**: 박스에 *사람을 넣는 것*(N2 가 함)과 그 사람에게 *공이 가는 것*(안 됨)은
  //   다른 문제다. 배급·마무리가 여전히 ST 로 수렴하고, 그 벽은 ⑨ 옵션 B(B1 크로스 · B2 컷백
  //   EV 예외) 소관이다. **그리고 N1(거리 감쇠)은 박스를 채운 뒤에도 축 B 를 판다** —
  //   N2+N1 n60 에서 슛top1 94.5 → 97.3 · HHI 0.895 → 0.948 · 비ST 슛 0.87 → 0.46.
  //   감쇠를 세게 걸면(f8 p.25) 박스 안 슛 **100%** · HHI **1.000** 으로 Phase 2-A 기각 패턴이
  //   그대로 재현된다. 즉 "N2 가 박스를 채운 뒤 N1 을 켠다"는 이 프로그램의 가설은 **반증됐다**.
  version: "engine@0.43.0",
  msPerTick: 1000,
  ...
  contest: {
    ...
    // ── 🔴 0.197 → **0.07** 되돌림 (#370, hero 배포 GO 2026-08-02) ─────────────────────
    // **0.197 은 라이브 사고의 원인값이다.** 픽스처에서는 여섯 밴드를 전부 통과하는 유일한
    // 지점이었지만(아래 옛 주석), 실덱에서 슛이 **79% 붕괴**했다(56 → 12, 골 0). 이유는
    // 이 값이 **절대 xG 컷**인데 **xG 분포가 덱마다 다르기** 때문이다 — 문제 실덱에서는
    // 슛 후보의 **90%가 게이트 아웃**됐다. 픽스처 60시드를 아무리 늘려도 시드 분산만 넓히고
    // **입력 분포는 하나로 고정**이라 이 부류는 원리적으로 안 잡힌다(#377 §1-3 의 출발점).
    //
    // ⚠️ **그래서 이 값은 볼륨 레버가 아니다.** 위 사다리(0.001 당 팀당 슛 ~0.5)는 픽스처
    // 한 점에서의 감도이고, 그 감도 자체가 "덱이 바뀌면 어디로 튈지 모른다"는 뜻이다.
    // 볼륨은 `chain.goalValue` 같은 **덱 상대적** 레버로 잡는다(트랙 T 소관).
    //
    // 0.07 = 사고 이전 값이자 0.29.5 브랜치에서 검증된 값(hero 하네스 비교 확인).
    // 픽스처 총량은 올라간다(밴드 이탈) — §2-4 상 총량은 참고 지표이고, 재보정은 트랙 T 다.
    // 아래 옛 주석은 **0.197 시절 서술**이라 남겨 두되 현행이 아니다.
  ...
  rules: {
    ...
    offside: {
      enabled: true,
      toleranceM: 0.7,
      // #377 S3-C: 2.5 → **0**. 트랩 기하는 이제 `movement.defLine.trap` 이 물리로 만든다 —
      // 여기 값을 더하면 이중 계상이고, 0 보다 크면 위 선언부의 라인 불일치가 되살아난다.
      trapBiasM: 0,
      // #407 ⑦ (0.42.0): **0.013 → 0.045**. 구 값은 0.5.0 도입 이후 한 번도 재보정되지 않았고
      // (`git log -S"callProb"` = 도입 커밋 1건) 그 사이 패스 구성·수비 형태·행동 코어가 전부
      // 바뀌면서 콜 빈도가 1.88(0.16.0) → 1.4(0.19.0) → **0.425**(0.41.0, n60) 로 내려앉았다
      // (벤치 1–3, ▼0.575). 판정 로직에는 결함이 없다 — 20시드 패스 결정 9,850건 전량을 Law 11
      // 로 독립 재판정해 **오심 0건**이고 `callProb=1` 반사실에서 384/384 일치다
      // (`research/e407-offside.md`). 즉 고칠 것은 **빈도 하나**였다.
      // 사다리(n60 = 팀-경기 120): 0.013→0.425 · 0.03→0.825 · **0.045→1.175** · 0.05→1.267 ·
      // 0.055→1.392 · 0.06→1.492 · 0.065→1.592 · 0.07→1.708 · 0.075→1.800 · 0.08→1.992 ·
      // 0.10→2.817 (엄격 단조, 기울기 ≈ 선형).
      // ⚠️ **밴드 중앙(1.75 = `callProb` 0.07)을 안 골랐다.** 근거 둘:
      //  ① `bench.ts` 의 `offsides 1–3` 은 **90분 실축 벤치**이고 #365(45분화) 이후 재도출되지
      //     않았다 — 카운트형 지표 전반의 스테일은 `research/e407-volume-recalibration.md` §6-2 가
      //     이미 기록했다(코너·스로인·파울·주행). 길이보정(×0.5)이면 밴드는 0.5–1.5 이고,
      //     1.175 는 **두 읽는 법에 모두 든다**(90분 환산 2.35/팀 = 실축 EPL 1.5–2.5 한가운데).
      //     밴드를 넓히지도 좁히지도 않고 **교집합**을 택한 것이다.
      //  ② 임계 근처 계약이 값에 따라 흩어진다 — 같은 5개 계약을 rung 별로 재면 부작용 red 가
      //     0.045 **1건** · 0.055 3건 · 0.07 4건이다(전부 0.013 에서는 통과). 그 계약들은 마진이
      //     mm 단위이거나(input-wiring T1: low 9.1159 vs high 9.1165) 표본이 퇴화해 있어
      //     (press-unit A6: 비참여자 표본 0) **값을 고르는 기준으로 쓰면 노이즈에 맞추는 것**이다.
      //     그래서 ①이 1차 근거이고 ②는 확인일 뿐이다. 상세 = 구현 노트.
      // 계약 = `realism/offside-call.test.ts`(밴드 + 사다리 단조 + `callProb=0` 이면 깃발 0).
      callProb: 0.045,
      trapCallMult: 1.8,
    },
  ...
  chain: {
    // #361 T1 롤백 스위치. 이것만 없으면 "M2 배선 전부 off"를 config 로 재현할 수 없다
    // (나머지 3종은 `press.trigger.enabled`·`duty.enabled`·`fatigue.recoveryEnabled` 가 있다).
    passDirectnessEnabled: true,
    ...
    goalValue: 22,
    holdPenalty: 0.08,
    // ── #407 N1 (0.41.0) — 기제는 들어왔고 **기본은 off** 다. 근거는 실측이다 ───────────────
    // 27개 지점 · 20시드(팀-경기 40) 스윕 결과: 감쇠 축은 **축 A(거리 분포)를 확실히 개선**하지만
    // **축 B(선수 다양성)를 예외 없이 악화**시킨다. hero 가 축 B 악화를 하드 제약으로 걸었으므로
    // 기본값을 켤 수 없다. 상세표 = `issues/2026-08-02-engine-shot-gate-decay.md`.
    //
    //  ⓐ **축 A 는 이긴다(설계대로).** 같은 볼륨에서 하드 게이트 축보다 박스 편중이 훨씬 낮다 —
    //     팀당 슛 9.1 에서 하드(`shootRange` 14) 박스 **98.6%** vs 감쇠(g24·u.36) 슛 9.95 에서 **59.8%**.
    //     Phase 2-A 3안(박스 89.9~100%)이 기각당한 그 축은 이걸로 실제로 풀린다.
    //  ⓑ **그런데 축 B 가 무너진다.** 게이트를 넓히는 순간 ST 가 27m 에서 쏘기 시작해
    //     사슬이 미드필더에게 닿기 전에 끝난다: 슛 top1(=ST) 95.8% → **97.3~100%**,
    //     1대1 비중 7.17% → **0~4.3%**(#316). 가장 부드러운 가장자리(g24·f16.5·p.16)에서도
    //     top1 98.5 · 1대1 3.84 로 base 보다 나쁘다.
    //  ⓒ 볼륨 밴드(7.2–8.4)는 감쇠만으로 **닿지 않는다** — 밴드에 들어가려면 박스 편중이
    //     60~100% 로 튄다(축 A 제약 51.2% 초과).
    // → 이 노브들은 **N2(조건부 박스 도착런, 규모 M)와 같은 웨이브에서** 켜야 한다. 그게 축 B 를
    //   실제로 여는 유일한 측정된 기제이기 때문이다(Phase 2-B §8-2). 켤 때의 권장 출발점이
    //   아래 값이다(축 A 최선 · 볼륨 ≈baseline: 슛 18.88 · 박스 35.0% · 거리 p50 18.0).
    shootDistance: {
      enabled: false,
      genMaxM: 24,
      freeM: 16.5,
    ...
    hold: {
      ...
      tightPenalty: 0.3,
      minKeep: 0.25,
      // ── #407 N4 (0.41.0) — **켠다.** 실측으로 목적을 달성했다 ──────────────────────────
      // ⚠️ 아래 "거의 중립" 판단은 20시드로 내렸고, **60시드로 갱신됐다**(#407 독립 검증 major-1).
      //    60시드(팀-경기 120) 짝 대조에서 출하 기본의 1대1 은 **6.08% → 5.05%**
      //    (−17% 상대 · 절대 126→106건) 로 **내려간다** — 20시드의 7.17→7.11 은 표본이
      //    작아 못 본 것이다. 볼륨·거리 축은 여전히 중립(슛 17.27→17.48 · 박스 41.2→41.5)이지만
      //    **1대1 축은 중립이 아니다**. 원인 미규명 = #407 후속 과제. 감시는 60시드 프로브
      //    (`research/e407-probe/e407-diversity.ts`)가 하고, `realism/shot-distance-decay.test.ts`
      //    의 중립 계약은 그 지표를 매 실행 로그로 뱉고 붕괴(−50%)만 무는 바닥 가드다.
      // 20시드(팀-경기 40) 실측 — 이력:
      //  · 출하 기본에서는 거의 중립이다 — 팀당 슛 17.43→17.23 · 1대1 7.17→7.11 · 박스슛 41.6→40.9.
      //    (1대1 상황은 어차피 슛으로 끝나는 편이라 기본 트리에서 바뀔 결정이 적다.)
      //    ⚠️ **1대1 7.17→7.11 은 위 60시드 값으로 대체됐다**(20시드 판단 → 60시드 갱신).
      //  · **효과는 hold 를 볼륨 레버로 밀 때 나온다** — `holdPenalty` −2.0 에서
      //    1대1 비중 **2.67% → 5.45%**(base 7.17 의 76% 보존). Phase 2-A 안 A 는 여기서
      //    팀-경기의 95.8%가 0건이 되어 기각당했다. 그 실패 모드가 이 한 줄로 닫힌다.
      //    (60시드 재측: **3.33% → 5.33%**, base 6.08 의 88% — 방향·크기 모두 재확인.)
      //  · 그 hold 축은 축 A·B 를 **둘 다** 지킨다(hp−1.0: 박스 45.0% · 슛HHI 0.850 · top1 92.0 ·
      //    슈터수 1.73 — 전부 base 보다 좋다). 남은 유일한 블로커는 **파울 폭증**
      //    (4.47 → 11.7@hp−1.0 / 16.1@hp−2.0)이고 그건 이 웨이브의 범위 밖이다(#407 후속).
      // ⚠️ **사실상 불리언이다.** 1대1 에서 hold 의 우위는 작아서 2.0 만 빼도 결정이 뒤집힌다 —
      //    2.0 / 6.0 / 20.0 의 20시드 집계가 **완전히 동일**했다. 그래서 dead-knobs 레지스트리에는
      //    "0 으로 바꾸면 달라진다"(= 롤백 스위치)로 등록한다. 값을 키우는 튜닝은 의미가 없다.
      oneOnOnePenalty: 4.0,
    },
    // #377 M3-C 스루패스 — 러프 기본값(총량 재보정은 트랙 T). 값의 근거는 이 웨이브의 실측:
    //  · behindLineM 1.5 / minLeadM 10 / maxLeadM 25 = AC 가 요구한 리드 밴드(10~25m) 그대로.
    //  · minRunGainM 2.0 = "전진 중"의 문턱. 선수 속도가 3~7 m/tick 이라 2m 는 "한 걸음 앞을
    //    보고 있다" 수준이고, 예고(#369)를 읽고 라인 뒤로 출발한 러너는 훨씬 크게 잡힌다.
    //  · minMarginTicks 1 = **보수적 시작**(hero 미회신 항목 — W0 §5-3). 0/음수로 내리면
    //    "실패해 골킥이 되는 그림"을 감수하는 공격적 설정이 된다.
    //  · raceBase 0.55 + raceGainPerTick 0.15 = 여유 1틱에서 0.70 · 3틱에서 1.00 배.
    //    즉 아슬아슬한 스루패스는 성공확률이 3할 깎이고, 확실히 앞선 러너는 안 깎인다.
    throughPass: {
      enabled: true,
      behindLineM: 1.5,
      minRunGainM: 2.0,
  ...
  softCap: 0.25,
  duty: {
    // #366 러프 기본값 — 이중 계상(자연어 우회) 때문에 얕게. "연결고리(support)"는 전진이 아니라
    // **지원**이 강한 축이라 두 표가 서로 다른 방향으로 기운다(그게 이 열거형의 게임적 의미다).
    enabled: true,
    forwardRunMult: { defend: 0.7, support: 1.0, attack: 1.25 },
    supportPullMult: { defend: 0.8, support: 1.25, attack: 1.0 },
  },
  press: {
    // #361 러프 기본값. margin 0.05 = 피치 5.25m — 걷기 한 틱(2.5m)의 두 배라 채터링을 흡수한다.
    trigger: { enabled: true, marginProgress: 0.05 },
    // S3-A 러프 기본값 — 정밀 보정은 트랙 T. 산정 근거:
    //  · `dangerNearM` 25 ≈ 우리 페널티박스 앞선(박스 깊이 16.5m + 여유). 여기가 hero 제보의
    //    "골문 앞" 이다. `dangerFarM` 60 ≈ 하프라인 너머 — 거기선 총원 1(구동작)로 수렴한다.
    //  · `countNear` 3 = 압박 담당 1 + 커버 2. 실축의 "붙는 1명 + 받치는 2~3명"(#350 본문).
    //  · `wideWeight` 0.35 는 프리킥 벽(`wallWideWeight`)과 **같은 값**을 의도적으로 쓴다 —
    //    같은 질문("각이 나오는 위협인가")을 같은 자로 재는 것이 정합이다.
    //  · `coverLaneReachM` 12 = 커버가 압박 담당을 받치는 거리대(레인 전체가 아니라 공 쪽 구간).
    //  · `coveredM` 3 은 M3-B `vision.laneRead.coveredM` 과 같은 값이다. ⚠️ 그 둘을 서로의
    //    **독립 증거로 인용하면 안 된다**(`realism/lane.ts:LANE_NEAR_M` 주석의 독립검증 m2 와
    //    같은 부류) — 같은 함수의 출력을 같은 임계와 비교하는 것이라 정합이지 확인이 아니다.
    unit: {
      enabled: true,
      wideWeight: 0.35,
      dangerNearM: 25,
      ...
      countFar: 1,
      rangeM: 22,
      intensityCountGain: 0.8,
      intensityRangeGain: 0.8,
      coverLanePull: 0.6,
      coverLaneReachM: 12,
      // 35m ≈ 페널티 아크 언저리. 그 안으로 받는 리시버는 거리에 비례해 위험해진다.
      dangerRefM: 35,
      minThreatM: 2,
      reachM: 18,
      coveredM: 3,
      laneCostWeight: 0.35,
      // 7m = 압박 담당(공 위)보다 한 겹 뒤 · 6m 횡간격 = 서로 겹치지 않는 최소치. 전부 러프.
      supportGapM: 7,
      supportSpreadM: 6,
      supportSlotPull: 0.6,
    },
  ...
  fatigue: {
    // #346 러프 기본값 — 정밀 보정은 트랙 T. 산정 근거(45분 = 2700틱 기준):
    //   많이 뛴 선수(이동비 ~0.25) 평형 f* = (0.0060×0.25 − 0.00025)/(0.00025×6) ≈ 0.83
    //   덜 뛴 선수(이동비 ~0.10) 평형 f* = (0.0006 − 0.00025)/(0.0015) ≈ 0.23
    //   실측(8시드, **최종 커밋 콜드 재측정**): 평균 **0.623** · 개인 최고 평균 **0.857** ·
    //   **전원 포화 0.0%** · 종료 산포 **0.214** · 회복 틱 72.1% · 하프타임 −0.250.
    //   (대조군 `recoveryEnabled=false`: 포화 **43.2%** · 평균 0.717 · 개인 최고 **1.000** ·
    //    종료 산포 **0.000** · 회복 틱 **0.0%**.)
    //   → 평균 부하를 구 모델(0.728) 근처에 두면서 **아무도 1.0 에 박히지 않는다**.
    //   상수 회복(gain=0)으로는 이 둘을 동시에 못 만든다(실측: 평균 0.727 이면 종료 산포 0.000
    //   = 전원이 다시 1.0). ⚠️ 그래도 **볼륨은 올라간다** — 구 밸런스는 "79% 구간 전원 1.0"이라는
    //   상수 위에서 맞춰져 있었다(#346 본문). 재보정은 트랙 T 소관이고 여기서 하지 않는다.
    // 즉 **선수마다 다른 곡선**이 나오고 아무도 상수 1.0 에 박히지 않는다.
    recoveryEnabled: true,
    exertionPerTick: 0.0060,
    recoverPerTick: 0.00025,
    recoverFatigueGain: 6,
    deadBallRecoverMult: 2.5,
    activeMult: 1.6,
    gkMult: 0.3,
    halfTimeRecover: 0.25,
    floor: 0,
  },
  movement: {
    passPlan: {
      // #369 러프 기본값. read 0.35±0.3 = 능력 0 이면 20%, 100 이면 50% 가 읽는다
      // (전원이 항상 읽으면 텔레파시다). 수명 6틱 = 캐리어가 마음을 바꾸면 낡은 예고가
      // 안 남는 길이. 정밀 조정은 트랙 T.
      //
      // ⚠️ `pull` 은 이 웨이브에서 **두 번** 움직였고, 둘 다 튜닝이 아니라 **"노브가 광고한 동작을
      // 하는가"** 의 문제였다(§2.5 새 노브 레지스트리 기준). 이력: 0.45 → 0.75 → 0.7 → **0.75 복귀**.
      //  ① 초판 0.45 — "런 오더보다 약하게"라는 *말*에서 고른 값이다. 변이체 킬(끄면 경기가
      //     달라진다)은 green 인데 **읽은 리시버가 도착 예정 지점에서 오히려 멀어지고 있었다**.
      //     교훈은 유효하다: **"경기가 달라진다"는 "광고한 동작이 난다"를 뜻하지 않는다.**
      //  ② 0.75 — 광고한 동작이 처음 난 값.
      //  ③ 2R 수습에서 잠깐 **0.7** 로 내렸다. 이유는 "헤더 골이 60경기 표본에서 0 이 되니까"였는데
      //     **그 근거가 틀렸다**(아래). 그래서 ④ **0.75 로 되돌린다.**
      //
      // ── 고른 근거는 **두 축뿐**이다 (신호가 있는 축) ─────────────────────────────────
      //  ⓐ READ vs UNREAD 좁힌%(8시드, 출하 config) — `pull` 에 **단조**. 게다가 UNREAD 팔은
      //     평평하다(4.1 → 6.4, 추세 없음) = 깨끗한 이중차분이고, 게시 시점 거리 `d0` 가 두 팔에서
      //     같아(3.13 vs 3.01) 선택 편향도 배제된다.
      //  ⓑ mark-jitter nearOwner(백스톱 26) — 세게 당길수록 목표가 튀어 제자리 진동(#178/#185)이 는다.
      //   pull | READ vs UNREAD 좁힌% | mark-jitter nearOwner (백스톱 26)
      ...
      // → **0.75**: 계약 임계(>25%)에서 여유가 가장 크고(76%), 진동 백스톱 여유도 0.7(0.8%)보다
      //   **넓다**(3.2%). 0.8 은 백스톱 밖이다. 관찰량을 반사실 팔(`readBase=1`)이 아니라
      //   **출하 config 안의 READ vs UNREAD 대조**로 바꾼 것이 이 선택을 가능하게 했다(독립검증 m1).
      //
      // ── ⚠️ 헤딩·롱패스·걷어내기는 **근거로 쓰지 않는다** ─────────────────────────────
      // 2R 에서 0.7 을 고른 근거였던 "세게 당길수록 패스가 이어져 공중볼이 마른다"는 **반증됐다**.
      // 같은 자(커밋된 `HEADER_SEEDS` 구성 · `countHeaders` 그대로)로 n=120 사다리를 재면
      //   헤더 슛/골:  off 49/8 · 0.45 43/5 · 0.6 40/5 · 0.65 43/5 · 0.7 33/6 · 0.75 29/3 · **0.8 48/10**
      // — **추세가 없다.** 가장 세게 당기는 0.8 이 예고 off 와 같은 수준이다. 롱패스 시도율
      // (11.45/11.44/10.87/11.73/11.82)·걷어내기(4.91/5.25/4.75/5.38/5.00)도 같다.
      // 즉 이 세 축은 `pull` 의 함수가 아니라 **그 표본의 잡음**이다. 신호가 없는 게이트에 값을
      // 맞추는 것은 메모리 `balance-measure-multiseed`("계약은 확률 지표 말고 결정론 성질로")가
      // 금지하는 방식이고, 실제로 그렇게 고른 0.7 은 ⓐⓑ 두 축에서 0.75 보다 **나쁘다**.
      // (헤더 표본의 검정력·근본 원인은 `ball-physics.test.ts` 의 헤더 블록 주석 참조 — #316/S5.)
      enabled: true,
      readBase: 0.35,
      readAttrSwing: 0.3,
      pull: 0.75,
    ...
    defendCompactX: 0.32,
    defendCompactY: 0.16,
    // #377 S3-B: 여기까지 이 값은 **아무도 안 읽었다**(0/0.5/1.0 이 3시드 해시 동일).
    //
    // 값 선택의 근거는 **`heightRangeX` 와 짝**이다(아래 참조). 20시드 실측:
    //   k=0.35 hRX=0.50 → 골60 5.85 · 멤버산포p90 18.22 · 슬라이더권한 8.90m
    //   k=0.50 hRX=0.50 → 골60 6.97 · 16.68 · 9.29m   ← 골 폭주 상한(16시드 ≤8)을 8.06 으로 넘김
    //   k=0.65 hRX=0.35 → 골60 **5.58**(롤백과 동일) · **16.62**(가장 촘촘) · 6.38m  ← 출하
    // 즉 라인 세기(k)는 올리고 **라인 높이 권한(hRX)** 을 낮추는 조합이 세 축에서 동시에 낫다.
    // hero 가 더 강한 라인을 원하면 **이 값만 올리면 된다**(사다리 0 → 1 전 구간 검증됨).
    // ⚠️ 단, 올리면 `ball-physics` 무소유 급정지가 따라 오른다(60시드 25.27 → k=1.0 **27.03**,
    //    상한 25). 성질·수치는 위 선언부 주석 — 올릴 때 그 축을 같이 재라.
    lineDiscipline: 0.65,
    defLine: {
      enabled: true,
      // 0.30 = 백4(0.16·0.22)는 담고 미드(0.40·0.44)는 안 담는 유일한 자연 절단.
      memberProgressMax: 0.3,
      // 2 = 압박 유닛이 백4에서 1~2명을 데려간 뒤에도 "남은 사람끼리 라인을 지킨다"가 성립하는 하한.
      minMembers: 2,
      roleOffsetKeep: 0.35,
      // 5m = 실측 멤버 위치 산포(평균 8.7m · p90 21m)에 실제로 무는 값이면서, 근거리 마킹을
      // 흔들지 않는 폭. 러프 기본값이고 조정은 트랙 T.
      blockLineRangeM: 5,
      // 0.2(구 하드코딩) → **0.35**. 슬라이더 권한 3.71m → **6.38m**(1.7배).
      //
      // ⚠️ 0.5 도 시험했고(권한 9.29m) **되돌렸다**: 라인이 통째로 올라가면 뒤 공간이 열려
      // 60시드 골이 5.58 → 6.97 로 오르고 폭주 상한(16시드 ≤8)을 넘긴다. 라인 높이는 이 웨이브가
      // 회수 장치 없이 파는 대가라(오프사이드 0.67/경기 = 하이라인을 벌하는 장치가 사실상 없다)
      // **보수적으로 잡고 S3-C(오프사이드 트랩) 이후에 다시 올리는 것**이 맞다.
      // hero 가 "하이라인/로우블록이 더 확 느껴졌으면" 하면 **이 값만 올리면 된다**.
      heightRangeX: 0.35,
      refMode: "members",
      // #377 S3-C. 전부 러프 기본값 — 조정은 트랙 T(위 선언부 주석의 버킷 표가 근거다).
      trap: {
        enabled: true,
        // 2.5m = 어깨 밴드(4m)의 절반 남짓. 밴드 안 러너를 넘기되 라인을 통째로 올리지 않는 폭.
        stepUpM: 2.5,
        // 35m = 뚫린 비율이 30.39%(<25m) → 10.35%(25–40m) 로 꺾이는 구간의 안쪽 경계.
        minBallDistM: 35,
        // 4m = 어깨 밴드 사이징(2/4/6m 에서 점유 27.3/43.1/56.8%)의 중간 — 기회가 충분하면서
        // "라인에 붙어 있는 사람"으로 읽히는 폭.
        shoulderBandM: 4,
        minShoulder: 1,
        // 8m = 거리 게이트를 계단이 아니라 경사로 만드는 폭(#178 플리커 회피).
        releaseSmooth: 8,
      },
      bandMode: "both",
    },
    restDefence: {
      enabled: true,
      // 1~3 → 중립 슬라이더에서 **잔류 2명 = 두 센터백**(실축 레스트디펜스의 최소 형태).
      // ⚠️ 2~4(잔류 3명)도 시험했고 되돌렸다: R2/R3 은 똑같이 만족하지만(CB 하프초과 0.00%,
      // 최고 52.50m) **공격을 더 묶는다** — 롱패스 시도 11.82 → 11.51%(밴드 하한 12), 상대 진영
      // 인원 6.14 → 5.53. R4("공격을 죽이지 않는다")가 이 웨이브의 성립 조건이라 덜 공격적인
      // 쪽을 기본값으로 잡는다. 코너의 stayBackMin/Max 와 같은 형태.
      countMin: 1,
      countMax: 3,
      commitLineWeight: 0.6,
      commitTempoWeight: 0.4,
      playerOverrideWeight: 0.25,
      // 0.5 = 하프라인. 실측 CB 하프라인 초과 18.53% 를 겨냥한 값이고, 1 로 두면 배정만 하고
      // 상한이 없다(= 플라시보). hero 가 "CB도 올라가도 된다"면 이 값만 올리면 된다.
      lineCapProgress: 0.5,
    },
    pressRange: 22,
    markGap: 2.5,
    ...
    /**
     * #407 N2 — **출하 기본은 `enabled: false`** 다. 값은 20시드 스윕 + 60시드 확정으로 골랐고
     * 근거·전체 표는 `issues/2026-08-03-engine-box-arrival-runs.md` §3.
     *
     * ## ⚠️ 왜 켜지 않았나 (60시드 짝 대조, 팀-경기 120)
     * 기제는 **설계대로 작동한다** — 조건부라서 전역 우회가 부순 축을 지키면서 박스를 연다:
     *   비ST 박스수신 **0.72 → 1.04~1.22**(+45~70%) · 박스ST% 86.1 → **80.4** ·
     *   슛 HHI 0.904 → **0.892~0.895** · 슈터 수 1.68 → **1.76** ·
     *   **팀 폭 45.9~46.4(밴드 유지) · 스로인 11.8~12.3(살아 있음)**
     *   ↔ 전역 팔(`attackWidthReach` −0.10)은 같은 효과에 폭 **37.1** · 스로인 **1.09**.
     * 그런데 대가가 hero AC 의 하드 제약을 **넘는다**:
     *   **파울 5.09 → 4.10~4.70**(모든 팔에서 악화. AC = "이 웨이브가 파울을 악화시키지 않는 것") ·
     *   1대1 5.19 → 4.24~4.84(구조 밴드 목록에 명시) · 패스 82.6 → 79.0~80.2 · 전환 18.5 → 16.9.
     * 그리고 **정작 노린 지표는 안 움직인다**: 비ST 슛/tm 0.85 → 0.80~0.87(사실상 평평),
     * ST 골 비중 96.3 → 96.4~96.6, 와이드 슛 0.0% 그대로.
     *
     * ## 그래서 무엇이 남았나 (다음 웨이브의 입력)
     * 박스에 **사람을 넣는 것**과 그 사람에게 **공이 가는 것**은 다른 문제다. N2 는 전자를
     * 해결했고(게이트 틱당 비ST 박스 인구 0.46 → 0.70), 후자에서 막힌다 — 배급(패스 선택)과
     * 마무리(슛 독점)가 여전히 ST 로 수렴한다. 그 벽은 ⑨ 옵션 B 의 **B1 크로스 생성기 ·
     * B2 컷백 EV 예외**이고 이 웨이브의 스코프 밖이다.
     */
    boxArrival: {
      enabled: false,
      maxRunners: 2,
  ...
  vision: {
    ...
    runReadFrac: 0.5,
    runReadMaxM: 8,
    // #379(M3-B) 러프 기본값 — **약하게 시작한다**(위 주석 "세기는 pull 하나다" 참조).
    // read 0.3±0.3 = 능력 0 이면 0%, 50 이면 30%, 100 이면 60% 가 읽는다(전원이 읽으면
    // 수비 지능이라는 개념이 없다). 판정 버킷 4틱 = 한 레인을 몇 초 붙잡는 길이.
    // pull 0.3 · 상한 2.5m = 걸음 하나 분량(속도 3~7 m/tick)의 선점. 정밀 조정은 트랙 T.
    laneRead: {
      enabled: true,
      readBase: 0.3,
      readAttrSwing: 0.3,
