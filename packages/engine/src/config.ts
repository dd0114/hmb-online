...
export interface EngineConfig {
  ...
  contest: {
    ...
    /**
     * 슛 시도 최소 xG — **절대 하한(안전장치)**. 명백한 쓰레기 슛만 거른다.
     * ⚠️ **볼륨 레버로 쓰지 말 것**(#370). 이 값은 `xgBase × attrFactor(shooting) × 위치품질 ×
     * (1−0.3·fatigue)` 과 비교되는데, `attrFactor(shooting)` 은 **덱마다 다르다**(라이브 실측
     * 0.998~1.367). 그래서 같은 절대값이 덱마다 **다른 위치품질**에서 잘린다 — 0.28.0 배포에서
     * 라이브 슛이 79% 붕괴한 원인이 정확히 이것이다. 볼륨은 `shootPosQualityMin` 이 잡는다.
     */
    shootXgThreshold: number;
    /**
     * 슛 시도 최소 **위치 품질**(#370) — 볼륨 레버. `거리계수 × 중앙계수` 의 하한이다.
     *
     * `xgAtPoint` 는 `xgBase × attrFactor(shooting) × 거리계수 × 중앙계수 × (1−0.3·fatigue)` 인데,
     * 이 중 **선수 고유 스케일**(`attrFactor` · 피로)을 나눠 없애고 **어디에 서 있는가**만 남긴 값이
     * 위치 품질이다(0..1). 슈터의 능력치·피로와 무관하므로 **덱을 가로질러 같은 뜻**이다 —
     * 절대 xG 임계가 전이되지 않던 구조 결함(#370)의 근본 수정.
     *
     * 실효 임계 = `max(shootXgThreshold, xgBase × attrFactor × (1−0.3·fatigue) × shootPosQualityMin)`.
     * **0 이면 꺼진다 = 순수 절대 임계(구 동작) 롤백 스위치.**
     */
    shootPosQualityMin: number;
...
export const defaultEngineConfig: EngineConfig = {
  ...
  contest: {
    ...
    // ── #370 (0.197 → 0.07): **볼륨 레버에서 은퇴**시킨다 ─────────────────────────
    // 위 사다리는 전부 **픽스처 입력** 위에서 잰 것이다. 라이브 배포에서 같은 0.197 이
    // 실유저 경기 슛을 **56 → 12 로 79% 붕괴**시켰다(문제 경기 01KYVBW7…, 골 0).
    // 기전: 임계가 비교되는 xg 에는 `attrFactor(shooting)` 이 곱해져 있고 그 값은 **덱마다 다르다**
    // (라이브 14표본 슈터 평균 shooting 49.8~95.9 = attrFactor 0.998~1.367). 즉 하나의 절대값이
    // 덱마다 **다른 위치품질**에서 자른다. 실측(라이브 14표본, 게이트 off 기준 슛 xG 분포):
    //   p90 이 0.197 **아래**인 표본이 14 중 9 개 — 그 덱들에서는 슛이 원리적으로 0 에 수렴한다.
    // 0.07 = 분리 전 오랜 기본값이자 "명백한 쓰레기 슛만 거르는" 안전 하한. 볼륨은
    // `shootPosQualityMin` 이 잡는다(덱 불변). ⚠️ 다시 볼륨 레버로 쓰지 말 것.
    shootXgThreshold: 0.07,
    // ── #370 볼륨 레버 (신규 · **기본 off**) ─────────────────────────────────────
    // ⚠️ **0 = 꺼짐**(= 순수 절대 안전 하한 `shootXgThreshold`). hero 지시로 0.29.5 는
    // "팀당 슛 12–14 밴드를 판정에서 뺀다 — 슛이 20이든 25든 그대로 둔다"가 됐으므로,
    // 슛을 밴드로 되돌리는 **볼륨 조임 자체를 하지 않는다.** 그래서 이 노브는 자산으로만 남긴다.
    //
    // 이 노브가 무엇인가: 절대 xG 컷 대신 **위치품질**(거리계수×중앙계수)에 하한을 거는 게이트.
    // 선수 고유 스케일(`attrFactor(shooting)`·피로)을 나눠 없애므로 **덱을 가로질러 같은 뜻**이다
    // (`shotGateXg` 주석에 기전). 나중에 볼륨을 조여야 할 때 **절대 xG 임계 대신 이걸 쓴다.**
    // 60시드 사다리(팀당 슛, 이 노브만 켠 채): 0.66→8.17 · 0.63→11.75 · 0.62→13.06 · 0.60→15.54
    //   (`realism/shot-frequency.test.ts` 가 사다리로 박제 — 죽은 노브가 되지 않게).
    // 0.62 로 켜면 픽스처 여섯 밴드가 구 절대 게이트(0.197)와 등가이면서 품질이 같거나 낫다
    //   (60시드 슛 13.06 · 유효슛 4.90 · 골/경기 5.03 · hold 40.0% · 1v1 0.667 · 파울 12.08).
    //   ⚠️ 그러나 **라이브 붕괴는 그걸로 안 풀린다** — 문제 경기 L01 이 팀당 1.0 슛에 머문다
    //   (위치품질 분포 자체가 덱·매치업마다 다르고, 질량의 60% 를 자르는 임계는 어디서든 가파르다).
    //   그래서 0.29.5 는 **켜지 않는다**. 라이브 실측은 아래 `shootXgThreshold` 주석.
  ...
  chain: {
    ...
    // ── #370: **볼륨 레버로 재시도했고, 안 된다**(22 유지) ────────────────────────────
    // #357 이 gv 단독 인하로 볼륨을 잡으려다 실패한 것은 그때 `threatWeight`(18) > `goalValue`(9.4)
    // 였기 때문이고 그 역전은 해소됐으므로 재시도했다. 실측(안전 임계 0.07 고정, 20시드
    // 픽스처 / 라이브 14표본 평균 · 라이브 최소 경기합):
    //   gv 22 → 픽스처 32.05 · 라이브 24.18(최소 19) | gv 16 → 31.50 · 21.29(12)
    //   gv 12 → 28.60 · 16.54(9)   | gv 10 → 25.50 · 10.61(2) | **gv 8 → 12.40 · 3.04(1)**
    // 픽스처 밴드(12–14)에 들어가는 지점은 gv≈8 하나뿐인데 **거기서 라이브가 죽는다**
    // (평균 3.04 · 최소 경기합 1). gv 는 22→12 구간에서 볼륨을 거의 안 움직이다가 10 아래에서
    // 절벽이고, **그 절벽 위치가 덱마다 다르다** — 절대 xG 임계와 같은 병이다. 그래서 볼륨은
    // `contest.shootPosQualityMin`(덱 스케일 불변)이 잡고 gv 는 질(r=tw/gv<1)만 지킨다.
    goalValue: 22,
    holdPenalty: 0.08,
