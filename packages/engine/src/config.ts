...
export const defaultEngineConfig: EngineConfig = {
  ...
  movement: {
    passPlan: {
      ...
      // 안 남는 길이. 정밀 조정은 트랙 T.
      //
      // ⚠️ `pull` 초판은 0.45 였다("런 오더보다 약하게"라는 **말**에서 고른 값이다). 관전 증거를
      // 뽑아 보니 그 값에서는 **광고한 동작이 관측되지 않는다** — 읽은 리시버가 공이 떠나기 전에
      // 도착 예정 지점으로 좁힌 거리가 평균 **−2.27m**(전원 읽기 팔 = 안 읽은 리시버가 안 섞인
      // 팔, 즉 오히려 멀어진다)이고 좁힌 장면은 8.5% 뿐이었다. 역할·런·폭 항이 이겨서 예고가
      // 목표를 못 기울인 것이다. 이건 밸런스 튜닝이 아니라 **"노브가 실제로 발화하는가"** 의
      // 문제다(§2.5 새 노브 레지스트리 기준: 광고한 동작이 출하값에서 한 번은 나야 한다).
      //
      // 두 축을 같이 봐야 한다 — 세게 당길수록 예고는 보이지만 목표가 튀어 **제자리 진동**
      // (#178/#185)이 는다. 사다리(seed 1618033988 전원 읽기 팔 / mark-jitter 4시드 nearOwner,
      // 절대 백스톱 26 · 예고 off 기준선 23.56):
      ...
      // (진동 쪽 사다리는 단조가 아니다 — 0.6 이 0.8 보다 높다. 4시드 카오스 잡음이라
      //  임계 근처에서 값을 고르는 근거로는 약하고, 그래서 **관계식**(≤1.3× 시야off)이 주(主)
      //  계약이고 26 은 백스톱이다. 정밀 조정은 트랙 T.)
      enabled: true,
      readBase: 0.35,
      readAttrSwing: 0.3,
      pull: 0.75,
