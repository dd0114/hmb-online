...
test("corner → 재생 중 '코너킥!' 상황카드로 정지 (선행 정지 skip 착지에도 누락 없음)", async ({ page }) => {
  ...
  const caps = await playUntilSituationContains(page, corner.tick - 8, "코너킥");
  expect(caps.situation).toContain("코너킥");
...
test("throw_in → 재생 중 '스로인!' 상황카드로 정지 (배너만이 아님)", async ({ page }) => {
  ...
  const caps = await playUntilSituationContains(page, throwIn.tick - 3, "스로인");
  expect(caps.situation).toContain("스로인");
  expect(caps.flash).toBe("");
});
