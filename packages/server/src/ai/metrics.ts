/**
 * 캐시 계측(에픽 #32 · W3 AC1): "캐시 최대 활용" 을 수치로 증빙.
 * 2층 — L1 결과캐시(같은 지시 = AI 호출 자체 스킵) · L2 프롬프트 캐시(하네스 자동, cacheRead 토큰).
 * 인메모리 카운터(프로세스 수명). 영구 리포트는 W3 운영에서 스냅샷.
 */

/** 잡 1건의 토큰 usage(claude-code executor 봉투에서 파싱). */
...
export class CacheMetrics {
  ...
  /** AiService.request 결과 1건 기록: cached = L1 히트(AI 스킵), 그 외 = 미스(잡 발화). */
