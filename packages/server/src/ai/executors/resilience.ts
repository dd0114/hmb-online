...
/**
 * 회복력 데코레이터(에픽 #32 · W3 AC4) — 구독 캡·일시장애 대응.
 ...
 * 큐 프로토콜 덕에 잡은 큐에 남아, executor 만 바꿔도 무중단으로 소진된다.
 */
 ...
 * 운영자가 구독 캡을 만났을 때 메터드 API 나 stub 으로 무중단 전환하는 지점(에픽 §리스크).
 */
export function withFallback(
  primary: AiExecutor,
  fallback: AiExecutor,
  onFallback?: (error: unknown, job: AiJob) => void,
): AiExecutor {
  return {
