/**
 * C6 델타 패치 계측 (#193) — "이미 베이스 TacticalInput 이 있는 유저가 프롬프트 일부를 수정"한
 * 시나리오의 지연을 잰다. 수정 전/후 지시를 나란히 주고 **변경이 유발하는 변화만** TacticalPatch 로
 * 받아, applyPatch(base, patch) 정적 머지 + clamp 로 최종 TacticalInput 을 만든다.
 *
 * 실행: npx tsx packages/server/scripts/measure-delta-patch.ts --n 2 --model sonnet --effort low --dump <dir>
 * (계측 전용 — 서버/데모 스택 무접촉. 구독 세션 사용: ANTHROPIC_API_KEY unset 강제)
 */
import { spawn } from "node:child_process";
import { writeFileSync } from "node:fs";
import { TacticalPatch, applyPatch, clampTacticalInput } from "@hmb/shared";
import {
  PATCH_SYSTEM,
...
if (process.env["ANTHROPIC_API_KEY"]) {
  console.warn("[measure-delta] ANTHROPIC_API_KEY unset 강제(구독 세션 유지)");
...
function buildDeltaPrompt(): { prompt: string; base: ReturnType<typeof makeBaseTacticalInput>; ctx: ReturnType<typeof makeTeamInputContext>; fwId: string } {
  const ctx = makeTeamInputContext();
  const base = makeBaseTacticalInput(ctx.seed);
  const roster = [...ctx.roster].sort((a, b) => a.slotIndex - b.slotIndex);

  // 개인 지시 대상: H9 가 로스터에 있고 FW 계열이면 H9, 아니면 첫 FW.
...
async function main(): Promise<void> {
  ...
  console.log(`[measure-delta] model=${MODEL} n=${N} effort=${EFFORT || "(session default)"} promptChars=${prompt.length} fw=${fwId}`);
  const args = [
    "-p",
    "--output-format",
    "json",
  ...
  for (let i = 1; i <= N; i++) {
    ...
    // 산출 패치 → applyPatch 정적 머지 → clamp → 로스터 정합.
    let mergeOk = false;
    let mergeNote = "";
    let finalChars = 0;
    try {
