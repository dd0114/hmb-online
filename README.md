...
### ▶ **[https://hmb-online.pages.dev](https://hmb-online.pages.dev)**

링크를 누르면 브라우저에서 바로 시작된다. **설치도 계정도 결제도 필요 없다** — 게스트로 들어간다.

게스트 시작 → 스타터 팩 지급 → **선수별 프롬프트 작성** → 덱 구성 → 연습 경기 →
전반 → 감독시간(교체·후반 지시) → 후반 → 결과. 경기 중 `⏩ 스킵`으로 하프를 건너뛸 수 있어
**한 판을 1~2분**에 끝까지 볼 수 있다.

여기서는 **AI 가 실제로 동작한다** — 감독이 쓴 문장이 Claude 를 거쳐 그 선수의 전술 파라미터가 되고,
그 파라미터로 결정론 엔진이 경기를 돌린다. 이 게임의 차별점을 보려면 이 경로로 보는 것이 맞다.

---

## 1-B. 소스를 직접 받아서 돌리기 (두 줄)
...
**매치 엔진이 브라우저 안에서 직접 돈다.** 설정·로그인·계정 아무것도 필요 없다.

⚠️ 이 경로에서는 **AI 가 동작하지 않는다.** 프롬프트는 결정론 규칙으로 해석되고, 화면 하단 배너가
현재 어느 경로인지 알려 준다(플레이는 어느 경우에도 막지 않는다). 로컬에서 AI 까지 쓰려면
`npm run play:ai` — Claude Code 로그인이 있으면 §1 링크와 같은 AI 경로를 탄다(§2).
...
§1-B 와 같은 것이다(**스태틱 모드**: 목데이터 + 브라우저 내 엔진, AI 없음).
