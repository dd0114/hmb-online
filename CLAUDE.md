...
| 0.17.0 | **죽은 로밍 변주 복구 + 슛 재튜닝**(#147 W2): 오프더볼 로밍 노이즈가 `floor(tick/25)` 계단식이라 **버킷 안 25틱 동안 상수 → 틱별 개인 움직임 기여 0**, 경계에서만 전 선수가 **동시에** 방향 전환 — 변주 기능이 사실상 죽어 있던 결함. `variety.roamContinuous`(smoothstep 보간) + `noisePhasePerPlayer`(idHash 위상)로 복구. **성과는 이산 동시전환 제거에 한정**: 완전 동조(전원이 *정확히* 같은 방향) 팀-틱 **40.9%→19.9%**(독립 QA 가 R=1.00 스파이크 소멸로 육안 확인). **블록 행진은 무변**(변위가중 R 0.825→0.823 · 큰움직임 R 0.843→0.838 · 전환후 피크 0.94) — 그건 W3/W4 몫. ⚠️ 초판 지표(단위벡터 R)는 5cm 표류와 5m 질주를 동일 가중해 **표본구성 아티팩트**에 취약했다(0.823→0.729 '개선'은 렌더 2px 미만 변화의 산물, 검증 세션이 반증) → 지표를 **임계 없는** weightedR·rigidPct·bigMoveR·lockstepPct 로 교체. 동반 재튜닝: 로밍 복구로 슛 14.00→14.15·골 1.65→1.78(벤치 이탈)이라 `decisionWeights.shoot` 0.34→0.32 + `contest.xgBase` 0.19→0.178 → **슛 13.23·골 1.65**(벤치 복귀), 슛 레버 효과크기도 20시드 Δ 2.65→3.47 로 강화. 기각된 노브 3개는 기본 off 유지(`movement.transition`·`arriveToleranceM`·`targetLerp` — 전부 동기 지표 악화). 계약=`realism/movement-synchrony.test.ts`. 새 골든. |
