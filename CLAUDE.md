...
| 0.35.0 | **예고 패스**(#377 M3-A · #369). hero: *"받는 쪽이 패스하는 사람의 생각을 예측해도 될 것 같아 — 훈련을 했기 때문에 미리 한 단계 앞서 움직인다."* **없던 것은 측정이 아니라 구조**다: `intents` 는 **찬 그 틱에만** 게시됐고(#314), 틱 순서상 오프더볼 결정이 볼 소유자 결정보다 **앞**이라 리시버는 언제나 *반응*만 했다(패스가 발밑으로만 갔다 — 리드 p50 3.48m). 이제 캐리어가 아직 안 찼을 때(`hold`/`dribble`) 사슬이 **이미 계산해 둔** 최상위 패스 후보를 팀 게시판에 예고로 올리고(`Intent.kind="pass_plan"`), 동료가 **능력만큼**(`(mental+positioning)/2`) 읽어 도착 예정 지점으로 미리 출발한다. **새 예측기를 만들지 않는다** — 리시버가 같은 탐색을 다시 돌리면 비용만 22배고 결과는 같다. **아군만 읽는다**(상대 예측은 #379 의 몫이고 정보 출처가 게시판이 아니라 **기하**여야 텔레파시가 아니다). 읽기 판정은 `varietyNoise` **시드 노이즈**라 RNG 스트림을 **한 번도 소비하지 않는다**. ⚠️ **초판이 세 번 헛돌았고 세 번 다 계약·독립검증이 잡았다**: ⓐ 읽기 결과를 오프더볼 분기 **중간에서** `targetFx` 로 썼는데 그 분기는 맨 끝에서 로컬 tx/ty 를 대입해 **덮어썼다**(완전한 no-op — tsc 는 통과한다) → 순수 조회로 바꿔 **대입 직전에** 섞는다. ⓑ `pull` 0.45 에서 **변이체 킬은 green 인데 광고한 동작이 안 났다**(읽은 리시버가 도착 예정 지점에서 오히려 멀어졌다) — *"경기가 달라진다"는 "광고한 동작이 난다"를 뜻하지 않는다*(§2.5 새 노브 레지스트리 기준이 자기 웨이브를 잡았다). ⓒ 그 다음 `pull` 0.75 는 효과는 냈지만 **헤딩을 침식**해 헤더 골이 0 이 됐고, 그걸 **표본 확대(60→120)로 넘기려 한 것**을 독립검증이 blocker 로 잡았다 — 근거로 적은 "120경기로 재면 헤더 슛이 오히려 늘었다(39→48)"가 **다른 시드 집합으로 잰 값**이라 실제로는 43→29(−33%)로 **부호가 반대**였다. 표본 확대를 되돌리고 `pull` 을 **0.7** 로 내려 원래 표본에서 green 이 되게 했다. 세 축 사다리(READ vs UNREAD 좁힌% / mark-jitter nearOwner(백스톱 26) / 헤더 슛·골 60경기): 0.45 `10.8vs4.1`·25.11·21/3 · 0.6 `22.3vs5.5`·27.61✗·21/4 · **0.7 `36.8vs5.1(7.2배)`·25.80·15/2** · 0.75 `44.1vs5.2`·25.18·18/**0**✗. 기각: "최신 예고를 읽는다"(직관상 캐리어의 지금 생각) — 아블레이션에서 **두 축 다** 나빠졌다(좁힌 44.2→33.1% · 진동 26.18→27.9): 최신 예고는 매 틱 새로 계산돼 목표가 계속 움직이므로 리시버가 어느 지점에도 수렴하지 못한다. ⚠️ **재개가 깨져 있었다** — `packages/server` 의 `IntentSchema` 가 엔진 `Intent["kind"]` 를 **손으로 베낀 사본**이라 `pass_plan` 이 따라오지 않았고, **하프 경계에 그 의도가 실린 시드에서만** `resumeState` 가 거부됐다(#241 계열). 더 나쁜 것은 그게 **운**이라 두 번의 전체 게이트를 green 으로 통과했다는 점이다. 사본을 없앴다 — 엔진이 `INTENT_KINDS` 를 단일 출처로 export 하고 스키마가 `z.enum(INTENT_KINDS)` 로 그대로 쓴다(#63 owned-glob 한 줄 additive, main 통보 = #326 선례). **선언된 회귀**: 헤더 슛 24 → 15(−37%) · 롱패스 시도 비율 10.87%(밴드 12–15, 열차에선 밴드 안이었다) — 둘 다 같은 뿌리("패스가 잘 이어져 걷어내기·롱볼이 준다")이고 #316/S5·#372/트랙 T 소관이라 계수를 안 건드렸다. 대신 **헤더 슛 래칫을 조였다**(하한 10 → 12 — 10 은 기준선 24 의 0.42배라 37% 침식이 조용히 통과했다). 밸런스: `npm test` 8 red = 열차(44dd3e3) 콜드 재실행도 8 red 인데 **1건이 교체**됐다(해소: 슛→골 전환율 / 유입: 롱패스 시도율). 계약 인프라 4건: 쇼케이스 가드를 **의존 타입 전체**로(one_on_one 만 보다 `tackle` 0 으로 두 번째로 물렸다) · 헤더 슛 래칫 조임 · **`qa-match` 측정 창 가드 + 그 가드의 계약**(출하 데모에선 그 분기가 안 타서 조용히 깨져도 몰랐다 — 합성 로그로 양방향을 태운다) · 의도 종류 단일 출처. 골든·시드: 스냅샷 3종 + dead-knobs + vision 롤백 해시 2(`5fdcb658`/`347d54a5`) + hold-pressure 3 + PK 시드 28→**18**(1~80 에 후보가 하나뿐이라 1~200 으로 확대) + e2e `fixture-real` …446→**…388** + 쇼케이스 시드 27182818→**27706463**. 증거 = `evidence/377/M3-A.md`. |
