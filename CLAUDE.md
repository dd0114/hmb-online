...
```bash
npm test              # = T1. 항상 도는 계약 (사다리 제외)
npm run test:ladder   # 사다리·단조성 — engine config 노브를 만졌으면 필수
npm run test:full     # = T2. 전량 (T1 + 사다리)
```
...
### 검증 티어 T0/T1/T2 (#376 / #377 M0-3)

`HMB_LADDER` 와 **별개 축**이다 — 사다리는 "어떤 계약을 켜나", 티어는 "얼마나 비싼 것까지 도나".

```bash
npm run test:t0   # ≤1분 · 매 커밋. 결정론(축약 반복)·하이진·계약 + 실덱 붕괴 케이스 1경기
npm test          # = T1 · 노브를 만진 웨이브. 위 전량 + 다시드 집계 밴드 + 실덱 전량 스모크
npm run test:t2   # 릴리스 전. T1 + 사다리. (+ 독립검증 + hero 눈 QA 는 사람 게이트)
```

- **기본은 T1** — `npm test` 가 도는 것은 종전과 같다. T0 은 **명시적으로** 고른다.
- **결정론은 어느 티어에도 면제가 없다.** 티어가 정하는 것은 반복 횟수뿐이다(T0 8회 / T1+ 80회).
- T0 제외 목록·근거·정합성 = `packages/engine/src/realism/tier.ts` + `tier.test.ts`(**항상** 도는
  커버리지 가드 — 목록이 조용히 자라면 red). 제외는 "검증을 뺀 것"이 아니라 "언제 도느냐"다.
- 왜 T0 이 먼저인가: #370 때 가장 싼 핵심 검증이 4.8분짜리 사다리 뒤에 있었다. 싼 것을 앞에 둔다.
  ⚠️ 다만 **T0 의 실덱 1경기가 잡는 것은 "하프가 죽었나"까지**다(#231/#239 부류). #370 의 **슛 붕괴
  판정은 T1** 에 있고 지금은 `it.fails` 로 박제돼 있다 — 트랙 T 가 계수를 확정해 박제가 뒤집히면
  그때 슛 하한 래칫을 T0 로 올린다.

### ⚠️ 무거운 검증은 세션 간 직렬화된다 (#376)

`engdeep`·`testcost`·`formgate`… 가 각자 "내 테스트는 4분"이어도 동시에 돌면 **load 328** 로 머신이
죽는다(실제로 죽였다). 그래서 `npm test`·`test:t*`·`test:ladder`·`npm run e2e` 는
`tools/run-gate.mjs` 를 거친다 — 전역 슬롯 1개(`HMB_GATE_SLOTS`), playwright 는 **배타**.

```bash
node tools/run-gate.mjs --status     # 누가 점유 중인가
HMB_NO_GATE=1 npm test               # 우회(필요할 때만)
```

죽은 홀더는 PID liveness 로 자동 회수하고, 30분 대기 후에는 **막지 않고 경고 후 진행**한다
(게이트가 작업을 영구히 세우는 것이 부하보다 나쁘다).

⚠️ **협조적 락이라 게이트를 거치지 않은 실행은 막을 수 없다.** 실제로 겪었다 — `--status` 는 슬롯
0/1 인데 load 가 330 이었고, 다른 세션이 `npm test` 를 게이트 밖에서 돌리고 있었다. 그래서 **표준 경로
(`npm test`·`test:t*`·`npm run e2e`)를 쓰고 `npx vitest` 직접 실행은 피한다.**

게이트는 그런 실행을 **감지해 알리기만 한다(기본 대기 0)** — 강제는 슬롯 락이 한다. 감지는 `ps`
휴리스틱이라 틀릴 수 있고, 그걸 권위로 쓰면 틀리는 순간 fleet 전체가 선다(실제로 유휴 워처 하나에
모든 게이트가 5분씩 멈춘 적이 있다). 판정축은 이름이 아니라 **트리 CPU 합**이다 — 유휴 `vitest --watch`
는 실행 중인 것과 **타이틀·구조가 완전히 같고**(둘 다 `node (vitest)` + 워커), `npm run test:watch` 는
`--watch` 문자열이 **어디에도 안 남는다**. 이름으로는 원리적으로 못 가른다.
⚠️ **한계**: 절대 CPU 임계라 머신이 바쁠수록 프로세스별 %cpu 가 눌려 미탐이 커진다(한산할 때 78%인
같은 실행이 load 130 에서 11~35%). 임계를 낮게(10) 잡아 완화했지만 완전하지 않고, 기동 램프 ~2.5초도
미탐이다. **부하는 계약으로 못 걸고 감지는 부하에서 약해진다** — 그래서 게이트 수치는 항상
`uptime` 과 함께 읽는다. 기다리려면 `HMB_GATE_FOREIGN_WAIT_MS=300000` 으로 **명시적으로 켠다**.

### 실덱 판정 픽스처 (#374) — 밴드는 픽스처 전용이면 붕괴를 못 잡는다

`packages/engine/src/realism/real-decks/` = 라이브 DB 사본에서 뽑은 **실입력 10조합**(#370 붕괴
케이스 필수 포함). `tools/extract-real-decks.mjs` 가 **서버가 만든 것을 그대로 옮긴다**(TS 재구현
금지 — 재구현하면 검증이 구현과 같은 실수를 공유한다. `league-difficulty-sweep.ts` 선례).

**판정은 평균이 아니라 최악 케이스다.** 실측(현 main): 실덱 평균 슛은 벤치마크와 비슷한데
**팀-하프의 27% 가 슛 0회**다 — 평균만 보면 "정상"으로 읽힌다. 그게 라이브 24하프 평균으로
"붕괴 없음"이라 오판했던 그 실수다. 계약 = `realism/real-deck-smoke.test.ts`.

### 무리빌드 튜닝 하네스 (#377 M0-1)

```bash
npm run harness      # http://127.0.0.1:8310 — config 편집 → 재시뮬 → N경기 재생, 리빌드 0
```

계수를 만질 때 `config.ts` 를 편집하지 않는다. 엔진을 1회 로드해 두고 **config 를 데이터로 주입**해
재시뮬하고 viewer-core 로 바로 본다(4경기 ≈ 1초). 상세 = `tools/tuning-harness/README.md`.
⚠️ 하네스에서 나온 계수는 **커밋하지 않는다** — 확정값 커밋은 트랙 T 소관(#377 P v3).
