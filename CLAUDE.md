...
| 0.35.0 | **예고 패스**(#377 M3-A · #369). hero: *"받는 쪽이 패스하는 사람의 생각을 예측해도 될 것 같아 — 훈련을 했기 때문에 미리 한 단계 앞서 움직인다."* **없던 것은 측정이 아니라 구조**다: `intents` 는 **찬 그 틱에만** 게시됐고(#314), 틱 순서상 오프더볼 결정이 볼 소유자 결정보다 **앞**이라 리시버는 언제나 *반응*만 했다(패스가 발밑으로만 갔다 — 리드 p50 3.48m). 이제 캐리어가 아직 안 찼을 때(`hold`/`dribble`) 사슬이 **이미 계산해 둔** 최상위 패스 후보를 팀 게시판에 예고로 올리고(`Intent.kind="pass_plan"`), 동료가 **능력만큼**(`(mental+positioning)/2`) 읽어 도착 예정 지점으로 미리 출발한다. **새 예측기를 만들지 않는다** — 리시버가 같은 탐색을 다시 돌리면 비용만 22배고 결과는 같다. **아군만 읽는다**(상대 예측은 #379 의 몫이고 정보 출처가 게시판이 아니라 **기하**여야 텔레파시가 아니다). 읽기 판정은 `varietyNoise` **시드 노이즈**라 RNG 스트림을 **한 번도 소비하지 않는다**(#369 가 경고한 "후보 수 비례 소비 → 재개 계약 취약"의 구조적 회피). ⚠️ **초판이 두 번 헛돌았다**: ⓐ 읽기 결과를 오프더볼 분기 **중간에서** `targetFx` 로 썼는데 그 분기는 맨 끝에서 로컬 tx/ty 를 대입해 **덮어썼다**(완전한 no-op — tsc 는 통과한다) → 순수 조회로 바꿔 **대입 직전에** 섞는다. ⓑ `pull` 0.45 에서 **변이체 킬은 green 인데 광고한 동작이 안 났다** — 전원 읽기 팔에서 읽은 리시버가 도착 예정 지점에서 평균 **2.27m 멀어졌다**(좁힌 장면 8.5%). 즉 "경기가 달라진다"는 "광고한 동작이 난다"를 **뜻하지 않는다**(§2.5 새 노브 레지스트리 기준이 자기 웨이브를 잡았다). 두 축 사다리(gain / mark-jitter nearOwner, 백스톱 26): 0.45 −2.27m·25.11 · 0.6 −0.63·27.61✗ · 0.7 −0.72·25.80 · **0.75 +0.08m/41.4%·25.18** · 0.8 +0.42·26.18✗ → **0.75**. 기각: "최신 예고를 읽는다"(직관상 캐리어의 지금 생각) — 아블레이션에서 **두 축 다** 나빠졌다(gain +0.42→−0.77 · 진동 26.18→27.9). 최신 예고는 매 틱 새로 계산돼 목표가 계속 움직이므로 리시버가 어느 지점에도 수렴하지 못한다. 밸런스: 열차(44dd3e3)에서 이미 red 이던 총량 밴드 5건은 그대로 red(트랙 T 소관), 대신 **red 3건이 해소됐다** — `behaviour` ⓐ 걷어내기 · ⓑ 러너 마크 거리 · shot-frequency 전환율(별도 워크트리 콜드 대조). 계약 인프라 3건: **쇼케이스 가드를 의존 타입 전체로**(one_on_one 만 보다가 이번엔 `tackle` 0 으로 `log.spec` 이 빨개졌다 — 같은 부류 2회) · **헤더 골 표본 60 → 120**(임계 `>0` 무변경, 포아송 P(0) 5~14% → 1%대) · **`qa-match` 빗나감 측정 창**(다음 틱이 하프 경계면 공은 이미 킥오프 중앙 — 선방 검사가 갖고 있던 `whistleCuts` 와 같은 부류의 오탐). 골든·시드: 스냅샷 3종 + dead-knobs + vision 롤백 해시 2(`cc6ade0f`/`d103da24`) + hold-pressure 3 + PK 시드 28→**10** + e2e `fixture-real` …446→**…570** + 쇼케이스 시드 27182818→**27706463**. 증거 = `evidence/377/M3-A.md`. |
