...
| 0.16.0 | **좌우대칭 픽스**: 공격 폭 확장의 중앙 선수 좌/우 분배를 `idHash` 패리티로 — 구 `<center?-1:1` 은 중앙 선수(ST·CM)를 항상 +y 로 밀어 공격이 하프 아래로 쏠렸다(슛 96%·코너 98.6% 편중). 회귀가드=`realism/lateral-balance.test.ts`. |
