...
| 0.32.0 | **입력 소생**(#377 M2). ①**#346 피로 경제 재설계** — 구 모델은 `fatigue += fatiguePerTick × exertion` **단조 증가**뿐이고 회복 항도 하프타임 리셋도 없었다. 게다가 `exertion` 의 `active` 는 매 틱 **소유자 1명 + 압박자 1명**뿐이라 사실상 전원 1.0 = **누가 얼마나 뛰었는지가 피로에 안 들어갔다**. 결과: 경기의 **44.5%가 전원 fatigue = 1.0**(종료 시점 선수 간 산포 **0.000**). fatigue 는 이동속도(×0.55)·xG(×0.70)·드리블(×0.60)·공중경합(×0.75)·사슬 EV 에 **곱해지는 승수**라, 경기의 절반이 **상수** 위에서 돌았고 하프타임 교체·컨디션(Phase 2)·`stamina`(#337)가 닿을 자리가 구조적으로 없었다. 새 모델: 부하 = **그 틱에 실제로 이동한 거리**(최대속도 대비) · **누진 회복**(회복량 = `recoverPerTick × (1 + recoverFatigueGain × fatigue)`) · **데드볼 숨 고르기**(정지 틱도 갱신 — 구 모델은 정지가 통째로 피로 밖이었다) · **하프타임 회복**. ⚠️ **누진이 핵심이다** — 상수 회복으로는 "평균 부하는 옛 모델만큼 무겁게"와 "아무도 1.0 에 안 박히게"를 **동시에 못 만든다**(실측: 평균 0.727 로 맞추면 종료 산포가 0.000 = 전원 다시 1.0). 누진이면 평형 `f* = (부하−r)/(r·gain)` 이 1 아래에 생겨 많이 뛴 선수와 덜 뛴 선수가 **다른 평형**에 앉는다. 실측(8시드): 포화 **0.0%** · 평균 0.62 · 개인 최고 0.86 · 종료 산포 **0.227** · 회복 틱 72.7% · 하프타임 −0.250. ②**#361 죽은 슬라이더 3종 배선** — `team.width`(공수 폭 항에 `(0.5+width)` 배수, 선수 성향과 **곱**) · `pressingScheme.triggerLine`(`assignPresser` 게이트 = 로우블록/하이프레스) · `passDirectness`(`chain.ts:candidateEv` 의 **롱 옵션** EV 배수 — 유일한 소비자였던 `scoreOption` 이 weighted 전용이라 사슬 기본에서 죽어 있었다). ③**#366 `duty` 결판 = (a) 배선**(오프더볼 전진 런·지원 당김 배수). ⚠️ 자연어 우회와 **이중 계상**되므로 배수를 1.0±0.3 으로 얕게. ④**#338 죽은 노브 = 레지스트리 계약**(주석은 스테일해진다 — INERT 17 / LIVE 4 / **조건부 LIVE 3** / 롤백 1 을 해시로 박제). #338 본문 **2건 정정**: `contest.oneOnOneClearM`·`oneOnOneXgMult` 와 `clearance.maxProgress`·`minPressers` 는 **살아 있다**(0.27.0 이 `oneOnOneShot` 을, `clearanceEligible` 은 원래부터 두 코어 공유). ⑤ `packages/server` 광고↔실효 감사(#367) 죽은 필드 **5개 → 1개**(남은 것 = `mentalModifier` #363, hero 게임설계 게이트). ⚠️ **볼륨이 오른다 — 그리고 그건 버그가 아니다**: 팀당 슛 7.44 → **13.44**(기여 = 피로 **+5.25** · 배선 +0.75, 8시드 아블레이션). 구 밸런스가 "경기의 44.5%가 전원 최대 피로"라는 **상수** 위에 맞춰져 있었던 것이 드러난 것이다(#346 본문이 예고). 총량 밴드 7건이 red 이지만 §2-4 가 참고 지표로 강등한 축이고 **구조·비율 지표는 전부 green** — 재보정은 **트랙 T** 소관이라 계수는 안 건드렸다. 사다리 red 도 단조성이 아니라 **비율**(구간별 엄격 단조 전부 통과 · 절대 효과 2.75 슛 유지 = 기저 볼륨 상승의 크기 아티팩트). **계약 3종 신설**: `realism/fatigue.test.ts`(`action.setFatigueObserver` 신설 — fatigue 는 스냅샷·이벤트 어디에도 안 나와서 이 결함이 오래 안 잡혔다) · `realism/input-wiring.test.ts`(입력 4종 **변이체 킬 + 방향 관계식 + 롤백**) · `realism/dead-knobs.test.ts`. ⚠️ **관찰량을 두 번 갈아탔다**(input-wiring 에 기록): triggerLine 방향을 "공 5m 안 인원 **수**"로 재면 **선택 편향**으로 역전하고(하이프레스는 그 구간에서 공을 빨리 뺏어 표본 자체가 줄고 남는 틱은 압박 실패분뿐), "라인 높이 **평균**"은 10명 평균이라 희석된다 → **공↔최근접 홈 선수 거리**. 같은 부류로 mark-jitter 관계식이 1.311 로 넘쳤는데 **처리군은 비트 동일(4.51→4.51)이고 대조군만 4.13→3.44 로 조용해진 것**이었다(분모가 움직이는 자[尺]) → 관계식 1.35 + **처리군 자체 비회귀**를 같이 건다. **골든·시드**: 스냅샷 3종 + 롤백 해시 5개 재기록 · e2e `fixture-real` …748→**…364** · **쇼케이스 전용 시드 신설**(`SHOWCASE_SEED = 27182818`) — `demoSeed` 는 리얼 증거용이라 분리했고, 1대1 빈도(#316 잔여 0.5~1/경기)가 얇아 전개가 바뀔 때마다 우연히 0 이 되던 것을 **생성 시점 가드**(`generate-demo.test.ts`)로 잡는다. |
