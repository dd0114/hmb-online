...
| 0.17.0 | **동기 이동 해소 W2 — 로밍 연속화**(#147): 오프더볼 로밍 노이즈가 `floor(tick/25)` 계단식이라 **버킷 안 25틱 동안 상수 → 틱별 개인 움직임 기여 0**(팀 형태 병진만 남음), 경계에서만 전 선수가 **동시에** 방향 전환(경계 틱 평균 전환각 46.6° vs 평시 30°) — 변주 기능이 사실상 죽어 있던 결함. `variety.roamContinuous`(버킷 간 smoothstep 보간) + `noisePhasePerPlayer`(idHash 위상) 로 개인 방향 성분을 상시화. **정렬도 R 0.823→0.729, 전원 동조(R>0.9) 팀-틱 57.2%→41.2%, 소유권 전환 후 피크 0.94→0.85.** 밸런스 밴드 전부 유지(패스 80.0%·롱 14.4%·슛 14.2·xG/슛 0.11·코너 5.4). 실측으로 **기각된** 대안 3개는 config 노브로 남기되 기본 off(`movement.transition` 전환 반응 지연 R 0.729→0.756 악화 · `arriveToleranceM` →0.830 악화 · `targetLerp` 병진·잔차가 동률로 줄어 병진비 무변). 계약=`realism/movement-synchrony.test.ts`(스냅샷 골든 + W2 래칫 + 목표 밴드 `it.fails`). 새 골든. |
