...
0/1 인데 load 가 330 이었고, 다른 세션이 `npx vitest` 를 직접 돌리고 있었다. 그래서 **표준 경로
(`npm test`·`test:t*`·`npm run e2e`)를 쓰고 `npx vitest` 직접 실행은 피한다.** 게이트는 그런 실행을
감지하면 목록을 찍고 **최대 5분만 양보**한 뒤 진행한다(`HMB_GATE_FOREIGN_WAIT_MS`).
